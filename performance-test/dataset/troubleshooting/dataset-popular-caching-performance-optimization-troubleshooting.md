...
5초는 너무 오래 걸리는 것 같았습니다. 매 요청마다 복잡한 QueryDSL 쿼리를 실행하여 실시간으로 인기 점수를 계산하는 방식으로 구현되어 있었는데, 이게 문제인 것 같았습니다.
...
이 문제를 어떻게 해결할 수 있을까 여러 방안을 고민해봤습니다:
...
### 4. 최종 해결방안: 하이브리드 캐싱 시스템

캐싱과 배치 처리를 함께 사용하면 어떨까 하는 아이디어가 떠올랐습니다. 이 방법이 가장 균형 잡힌 해결책인 것 같았습니다.
...
처음에는 간단하게 생각했습니다. 일단 동작하게 만들고 나중에 정리하자고 생각했는데, 나중에 테스트할 때 문제가 생겼습니다.
...
**캐시 로직이 실행이 안 됨**이라는 순간이 있었는데, 이때 애플리케이션을 재시작해야 한다는 것을 깨달았습니다.
...
│ 지표            │ Before   │ After    │ 개선율  │
├─────────────────┼──────────┼──────────┼─────────┤
│ 평균 응답시간   │ 17.73ms  │ 11.01ms  │ 37.9% 감소│
│ 95% 응답시간    │ 24.59ms  │ 14.50ms  │ 41.0% 감소│
│ 최대 응답시간   │ 199.73ms │ 18.63ms  │ 90.7% 감소│
│ DB 쿼리 수      │ 5-10개   │ 0-1개    │ 90%+ 감소 │
│ 안정성          │ 변동폭 큼 │ 안정적   │ 개선    │
└─────────────────┴──────────┴──────────┴─────────┘
