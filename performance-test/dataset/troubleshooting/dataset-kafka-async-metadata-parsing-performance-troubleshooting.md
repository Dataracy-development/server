...
## 🚨 문제 발견: 동기 메타데이터 파싱으로 인한 응답시간 지연

### 초기 상황

데이터 분석 커뮤니티 플랫폼의 **데이터셋 업로드 기능**에서 파일 업로드 후 메타데이터 파싱(행 수, 열 수, 미리보기 데이터 추출)을 동기적으로 처리하고 있었습니다.

기존 시스템에서는 업로드 완료 후 즉시 메타데이터를 파싱하여 사용자에게 응답하는 방식으로 구현되어 있었습니다.

### 문제 인식 과정

포트폴리오 트러블슈팅 스토리 작성을 위해 성능 테스트를 진행하면서 동기 처리 방식의 문제점을 발견했습니다. 실제 구현된 코드를 분석해보니 메타데이터 파싱이 업로드 응답을 지연시키는 주요 원인임을 파악했습니다.
...
public UploadDataResponse uploadDataSync(Long userId, MultipartFile dataFile, MultipartFile thumbnailFile, UploadDataRequest requestDto) {
    ...
    // ⚠️ 문제: 동기적으로 메타데이터 파싱 처리 (성능 문제 지점)
...
## 🔧 해결 방안: Kafka 비동기 메타데이터 파싱 시스템 도입

### 개선 방향 설정

문제 분석 결과, 기존 시스템에 이미 구현된 **Kafka 이벤트 드리븐 아키텍처**를 활용하여 메타데이터 파싱을 비동기로 처리하기로 결정했습니다.
...
│ 지표            │ Before   │ After    │ 개선율  │
├─────────────────┼──────────┼──────────┼─────────┤
│ 평균 응답시간   │ 5.96초   │ 4.00초   │ 33% ⬇️  │
│ 95% 응답시간    │ 8.55초   │ 4.86초   │ 43% ⬇️  │
│ 업로드 성공률   │ 100%     │ 100%     │ 유지    │
│ 사용자 경험     │ 블로킹   │ 논블로킹 │ 향상    │
└─────────────────┴──────────┴──────────┴─────────┘
```
