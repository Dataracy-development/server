/**
 * ========================================
 * 인기 데이터셋 조회 성능 테스트 시나리오 (DDD + 헥사고날 아키텍처)
 * ========================================
 *
 * 🎯 테스트 목적: DataQueryController.getPopularDatasets() API의 성능 및 인기도 기반 조회 최적화 검증
 *
 * 🏗️ 아키텍처 계층별 테스트 대상:
 * - Web Adapter: DataQueryController.getPopularDatasets() (Primary Adapter)
 * - Application Layer: GetPopularDataUseCase.getPopularDatasets() (Inbound Port)
 * - Domain Layer: Data 도메인 모델의 인기도 계산 및 랭킹 로직
 * - Infrastructure: JPA Repository, QueryDSL, Redis 캐시, Elasticsearch 검색, 통계 집계
 *
 * 🔍 테스트 시나리오별 목적:
 * - smoke: 기본 인기도 조회 검증 (5 VU, 30초) - CI/CD 파이프라인용
 * - load: 일반 인기도 조회 테스트 (10-100 VU, 8분) - 일상적 인기 데이터셋 조회 시뮬레이션
 * - stress: 고부하 인기도 조회 테스트 (50-300 VU, 10분) - 인기 데이터셋 집중 조회 시나리오
 * - soak: 장시간 인기도 조회 안정성 테스트 (100 VU, 1시간) - 캐시 효율성 및 랭킹 정확성 검증
 * - spike: 급격한 인기도 조회 폭증 테스트 (20-800 VU, 2분 30초) - 갑작스러운 인기 데이터셋 조회 대응
 ...
 *
 * 🎯 실무적 필요성:
 * - 인기 데이터셋 조회는 사용자 참여도가 높은 핵심 기능으로 성능이 중요
 ...
 * k6 run --env SCENARIO=load performance-test/dataset/scenarios/dataset-popular.test.js
 * k6 run --env SCENARIO=stress performance-test/dataset/scenarios/dataset-popular.test.js
 * k6 run --env SCENARIO=soak performance-test/dataset/scenarios/dataset-popular.test.js
 * k6 run --env SCENARIO=spike performance-test/dataset/scenarios/dataset-popular.test.js
 * k6 run --env SCENARIO=capacity performance-test/dataset/scenarios/dataset-popular.test.js
 */
...
// Custom metrics for popular dataset read operations
const popularReadSuccessRate = new Rate("popular_read_success_rate");
const popularReadResponseTime = new Trend("popular_read_response_time");
const popularReadAttempts = new Counter("popular_read_attempts");
const cacheHitRate = new Rate("popular_cache_hit_rate");
const rankingCalculationTime = new Trend("popular_ranking_calculation_time");
const statisticsAggregationTime = new Trend(
  "popular_statistics_aggregation_time"
);
const elasticsearchQueryTime = new Trend("popular_elasticsearch_query_time");
const dataMappingTime = new Trend("popular_data_mapping_time");
const popularityScoreAccuracy = new Rate("popularity_score_accuracy");
const rankingConsistency = new Rate("ranking_consistency");
...
export let options = {
  ...
  thresholds: {
    ...
    popular_read_success_rate: ["rate>0.95"],
    popular_read_response_time: ["p(95)<400"],
    popular_cache_hit_rate: ["rate>0.8"],
    popular_ranking_calculation_time: ["p(95)<100"],
    popular_statistics_aggregation_time: ["p(95)<150"],
    popular_elasticsearch_query_time: ["p(95)<80"],
    popular_data_mapping_time: ["p(95)<50"],
    popularity_score_accuracy: ["rate>0.9"],
    ranking_consistency: ["rate>0.95"],
  },
};
  ...
  if (AUTH_MODE === "login") {
    ...
    if (loginRes.status === 200) {
      ...
      return {
        ...
        "User-Agent": "k6-popular-dataset-test/1.0",
      };
    }
  }
  ...
  if (success) {
    ...
    try {
      ...
      for (let i = 1; i < datasets.length; i++) {
        if (datasets[i - 1].popularityScore < datasets[i].popularityScore) {
      ...
      datasets.forEach((dataset) => {
        if (!dataset.popularityScore || dataset.popularityScore < 0) {
          scoreAccuracy = false;
        }
      });
    } catch (e) {
