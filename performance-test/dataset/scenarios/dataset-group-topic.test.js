// dataset-group-topic.test.js
// k6 test for Group by Topic 조회
...
function scenarioExec() {
    // TODO: implement API call for Group by Topic 조회
    const url = `${BASE_URL}/api/v1/datasets?nocache=${Math.random()}`;
    const res = http.get(url, headers({ endpoint: 'group-topic' }));
    check(res, { 'group-topic 200': (r) => r.status === 200 });
    sleep(1);
}
