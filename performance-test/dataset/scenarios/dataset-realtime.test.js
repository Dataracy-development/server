// dataset-realtime.test.js
// k6 test for Real-time Search (자동완성)
...
function scenarioExec() {
    // TODO: implement API call for Real-time Search (자동완성)
    const url = `${BASE_URL}/api/v1/datasets?nocache=${Math.random()}`;
    const res = http.get(url, headers({ endpoint: 'realtime' }));
