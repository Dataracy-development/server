// dataset-mixed-read-heavy.test.js
// k6 test for 혼합 워크로드 (조회 90%+)
...
function scenarioExec() {
    // TODO: implement API call for 혼합 워크로드 (조회 90%+)
    const url = `${BASE_URL}/api/v1/datasets?nocache=${Math.random()}`;
    const res = http.get(url, headers({ endpoint: 'mixed-read' }));
    check(res, { 'mixed-read 200': (r) => r.status === 200 });
    sleep(1);
}
