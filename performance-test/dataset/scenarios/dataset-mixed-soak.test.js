// dataset-mixed-soak.test.js
// k6 test for 혼합 워크로드 Soak Test
...
function scenarioExec() {
    // TODO: implement API call for 혼합 워크로드 Soak Test
    const url = `${BASE_URL}/api/v1/datasets?nocache=${Math.random()}`;
    const res = http.get(url, headers({ endpoint: 'mixed-soak' }));
    check(res, { 'mixed-soak 200': (r) => r.status === 200 });
    sleep(1);
}
