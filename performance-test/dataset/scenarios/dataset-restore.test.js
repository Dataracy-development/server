// dataset-restore.test.js
// k6 test for Dataset Restore (짧은 주기 요청)
...
function scenarioExec() {
    // TODO: implement API call for Dataset Restore (짧은 주기 요청)
    const url = `${BASE_URL}/api/v1/datasets?nocache=${Math.random()}`;
    const res = http.get(url, headers({ endpoint: 'restore' }));
