// dataset-recent.test.js
// k6 test for Recent Datasets 조회
...
function scenarioExec() {
    // TODO: implement API call for Recent Datasets 조회
    const url = `${BASE_URL}/api/v1/datasets?nocache=${Math.random()}`;
    const res = http.get(url, headers({ endpoint: 'recent' }));
    check(res, { 'recent 200': (r) => r.status === 200 });
    sleep(1);
}
