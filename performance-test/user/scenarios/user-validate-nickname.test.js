...
export let options = {
    ...
    thresholds: {
        'http_req_failed{scenario:smoke,endpoint:user-validate-nick}':   ['rate<0.01'],
        'http_req_duration{scenario:smoke,endpoint:user-validate-nick}': ['p(95)<450'],

        'http_req_failed{scenario:load,endpoint:user-validate-nick}':    ['rate<0.02'],
        'http_req_duration{scenario:load,endpoint:user-validate-nick}':  ['p(95)<550'],

        'http_req_failed{scenario:stress,endpoint:user-validate-nick}':  ['rate<0.05'],
        'http_req_duration{scenario:stress,endpoint:user-validate-nick}':['p(99)<1200'],

        'http_req_failed{scenario:soak,endpoint:user-validate-nick}':    ['rate<0.02'],
        'http_req_duration{scenario:soak,endpoint:user-validate-nick}':  ['avg<700'],

        'http_req_failed{scenario:spike,endpoint:user-validate-nick}':   ['rate<0.05'],
        'http_req_duration{scenario:spike,endpoint:user-validate-nick}': ['p(99)<1400'],

        'http_req_failed{scenario:capacity,endpoint:user-validate-nick}':['rate<0.05'],
        'http_req_duration{scenario:capacity,endpoint:user-validate-nick}':['p(95)<1700'],
    },};
...
const NICK = __ENV.NICK || `nick_${randStr(6)}`;

function execOnce(wait=0){
    const res = http.get(`${BASE_URL}/api/v1/users/validate/nickname?value=${encodeURIComponent(NICK)}&nocache=${Math.random()}`, headers({ endpoint: 'user-validate-nick' }));
...
// 실행 예시:
// k6 run performance-test/user/scenarios/user-validate-nickname.test.js -e SCENARIO=smoke -e NICK=alice123
