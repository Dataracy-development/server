...
export let options = {
    ...
    thresholds: {
        'http_req_failed{scenario:smoke,endpoint:user-me}':   ['rate<0.01'],
        'http_req_duration{scenario:smoke,endpoint:user-me}': ['p(95)<500'],

        'http_req_failed{scenario:load,endpoint:user-me}':    ['rate<0.02'],
        'http_req_duration{scenario:load,endpoint:user-me}':  ['p(95)<600'],

        'http_req_failed{scenario:stress,endpoint:user-me}':  ['rate<0.05'],
        'http_req_duration{scenario:stress,endpoint:user-me}':['p(99)<1200'],

        'http_req_failed{scenario:soak,endpoint:user-me}':    ['rate<0.02'],
        'http_req_duration{scenario:soak,endpoint:user-me}':  ['avg<800'],

        'http_req_failed{scenario:spike,endpoint:user-me}':   ['rate<0.05'],
        'http_req_duration{scenario:spike,endpoint:user-me}': ['p(99)<1500'],

        'http_req_failed{scenario:capacity,endpoint:user-me}':['rate<0.05'],
        'http_req_duration{scenario:capacity,endpoint:user-me}':['p(95)<1800'],
    },};
...
function execOnce(wait=0){
    const res = http.get(`${BASE_URL}/api/v1/users/me?nocache=${Math.random()}`, headers({ endpoint: 'user-me' }));
...
// 실행 예시:
// k6 run performance-test/user/scenarios/user-read-me.test.js -e SCENARIO=smoke -e ACCESS_TOKEN=...
