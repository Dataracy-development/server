...
export let options = {
    ...
    thresholds: {
        'http_req_failed{scenario:smoke,endpoint:popular}':   ['rate<0.01'],
        'http_req_duration{scenario:smoke,endpoint:popular}': ['p(95)<600'],

        'http_req_failed{scenario:load,endpoint:popular}':    ['rate<0.02'],
        'http_req_duration{scenario:load,endpoint:popular}':  ['p(95)<800'],

        'http_req_failed{scenario:stress,endpoint:popular}':  ['rate<0.05'],
        'http_req_duration{scenario:stress,endpoint:popular}':['p(99)<1400'],

        'http_req_failed{scenario:soak,endpoint:popular}':    ['rate<0.02'],
        'http_req_duration{scenario:soak,endpoint:popular}':  ['avg<900'],

        'http_req_failed{scenario:spike,endpoint:popular}':   ['rate<0.05'],
        'http_req_duration{scenario:spike,endpoint:popular}': ['p(99)<1800'],

        'http_req_failed{scenario:capacity,endpoint:popular}':['rate<0.05'],
        'http_req_duration{scenario:capacity,endpoint:popular}':['p(95)<2100'],
    },
};
...
function execOnce(wait=0) {
  const url = `${BASE_URL}/api/v1/projects/popular?size=20&nocache=${Math.random()}`;
  const res = http.get(url, headers({ endpoint: 'popular' }));
  check(res, { '인기 프로젝트 조회 200': (r) => r.status === 200 });
