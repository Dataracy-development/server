...
export let options = {
    ...
    thresholds: {
        'http_req_failed{scenario:smoke,endpoint:continue}':   ['rate<0.01'],
        'http_req_duration{scenario:smoke,endpoint:continue}': ['p(95)<700'],

        'http_req_failed{scenario:load,endpoint:continue}':    ['rate<0.02'],
        'http_req_duration{scenario:load,endpoint:continue}':  ['p(95)<900'],

        'http_req_failed{scenario:stress,endpoint:continue}':  ['rate<0.05'],
        'http_req_duration{scenario:stress,endpoint:continue}':['p(99)<1500'],

        'http_req_failed{scenario:soak,endpoint:continue}':    ['rate<0.02'],
        'http_req_duration{scenario:soak,endpoint:continue}':  ['avg<1000'],

        'http_req_failed{scenario:spike,endpoint:continue}':   ['rate<0.05'],
        'http_req_duration{scenario:spike,endpoint:continue}': ['p(99)<1900'],

        'http_req_failed{scenario:capacity,endpoint:continue}':['rate<0.05'],
        'http_req_duration{scenario:capacity,endpoint:continue}':['p(95)<2200'],
    },
};
...
function execOnce(wait=0) {
  const url = `${BASE_URL}/api/v1/projects/1/continue&nocache=${Math.random()}`;
  const res = http.get(url, headers({ endpoint: 'continue' }));
  check(res, { '연속 프로젝트 조회 200': (r) => r.status === 200 });
