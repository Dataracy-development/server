...
export let options = {
    ...
    thresholds: {
        'http_req_failed{scenario:smoke,endpoint:detail}':   ['rate<0.01'],
        'http_req_duration{scenario:smoke,endpoint:detail}': ['p(95)<500'],

        'http_req_failed{scenario:load,endpoint:detail}':    ['rate<0.02'],
        'http_req_duration{scenario:load,endpoint:detail}':  ['p(95)<700'],

        'http_req_failed{scenario:stress,endpoint:detail}':  ['rate<0.05'],
        'http_req_duration{scenario:stress,endpoint:detail}':['p(99)<1300'],

        'http_req_failed{scenario:soak,endpoint:detail}':    ['rate<0.02'],
        'http_req_duration{scenario:soak,endpoint:detail}':  ['avg<800'],

        'http_req_failed{scenario:spike,endpoint:detail}':   ['rate<0.05'],
        'http_req_duration{scenario:spike,endpoint:detail}': ['p(99)<1700'],

        'http_req_failed{scenario:capacity,endpoint:detail}':['rate<0.05'],
        'http_req_duration{scenario:capacity,endpoint:detail}':['p(95)<2000'],
    },
};
...
function execOnce(wait=0) {
  const url = `${BASE_URL}/api/v1/projects/1&nocache=${Math.random()}`;
  const res = http.get(url, headers({ endpoint: 'detail' }));
  check(res, { '프로젝트 상세 조회 200': (r) => r.status === 200 });
