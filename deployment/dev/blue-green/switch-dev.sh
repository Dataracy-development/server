...
# 레지스트리에서 항상 최신 이미지를 받아서 기동 (서버 재빌드 방지)
...
echo "[INFO] Nginx upstream 설정 갱신: $BACKEND_NAME → localhost:80"

# Swagger 관련 경로는 캐시 무효화 헤더 추가 (브라우저/프록시 캐시 방지)
...
server {
  ...
  # ── OpenAPI JSON (그룹 포함: /v3/api-docs, /v3/api-docs/*, /swagger-config) ──
  ...
  # (선택) webjars도 최신 리소스 보장을 원하면 캐시 무효화
  location /webjars/ {
...
# NGINX 컨테이너 완전 재시작 (설정 확실 반영)
echo "[INFO] NGINX 컨테이너 완전 재시작 중 (CORS/캐시 문제 방지)"
