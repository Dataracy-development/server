...
public class VisitSourceController implements VisitSourceApi {
  ...
  public ResponseEntity<SuccessResponse<AllVisitSourcesWebResponse>> findAllVisitSources() {
    Instant startTime =
        LoggerFactory.api().logRequest("[FindAllVisitSources] 전체 방문 경로 목록을 조회 API 요청 시작");
    AllVisitSourcesWebResponse allVisitSourcesWebResponse;

    try {
      AllVisitSourcesResponse allVisitSourcesResponse =
          findAllVisitSourcesUseCase.findAllVisitSources();
    ...
    return ResponseEntity.status(HttpStatus.OK)
        .body(
            SuccessResponse.of(
                ReferenceSuccessStatus.OK_TOTAL_VISIT_SOURCE_LIST, allVisitSourcesWebResponse));
  }
}
