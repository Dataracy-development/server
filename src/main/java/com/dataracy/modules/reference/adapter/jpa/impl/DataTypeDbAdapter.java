...
public class DataTypeDbAdapter implements DataTypePort {
  ...
  public List<DataType> findAllDataTypes() {
    Instant startTime =
        LoggerFactory.db().logQueryStart(DATA_TYPE_ENTITY, "[findAll] 데이터 유형 목록 조회 시작");
    List<DataTypeEntity> dataTypeEntities = dataTypeJpaRepository.findAll();
    List<DataType> dataTypes =
        dataTypeEntities.stream().map(DataTypeEntityMapper::toDomain).toList();
    LoggerFactory.db().logQueryEnd(DATA_TYPE_ENTITY, "[findAll] 데이터 유형 목록 조회 종료", startTime);
    return dataTypes;
  }

  /**
   * 주어진 ID에 해당하는 데이터 유형 도메인 객체를 Optional로 반환한다. ID가 null이거나 해당 데이터 유형이 존재하지 않으면 빈 Optional을 반환한다.
   *
  ...
  public Optional<DataType> findDataTypeById(Long dataTypeId) {
    Instant startTime =
        LoggerFactory.db()
            .logQueryStart(DATA_TYPE_ENTITY, "[findById] 데이터 유형 목록 조회 시작 dataTypeId=" + dataTypeId);
    if (dataTypeId == null) {
      return Optional.empty();
    }
    Optional<DataType> dataType =
        dataTypeJpaRepository.findById(dataTypeId).map(DataTypeEntityMapper::toDomain);
    LoggerFactory.db()
        .logQueryEnd(
            DATA_TYPE_ENTITY, "[findById] 데이터 유형 목록 조회 종료 dataTypeId=" + dataTypeId, startTime);
    return dataType;
  }
  ...
  public boolean existsDataTypeById(Long dataTypeId) {
    ...
    LoggerFactory.db()
        .logExist(
            DATA_TYPE_ENTITY,
            "[existsById] 데이터 유형 존재 유무 확인 dataTypeId=" + dataTypeId + ", isExists=" + isExists);
    return isExists;
  }
  ...
  public Optional<String> getLabelById(Long dataTypeId) {
    Instant startTime =
        LoggerFactory.db()
            .logQueryStart(
                DATA_TYPE_ENTITY, "[findLabelById] 데이터 유형 라벨 조회 시작 dataTypeId=" + dataTypeId);
    if (dataTypeId == null) {
      return Optional.empty();
    }
    ...
    LoggerFactory.db()
        .logQueryEnd(
            DATA_TYPE_ENTITY,
            "[findLabelById] 데이터 유형 라벨 조회 종료 dataTypeId=" + dataTypeId + ", label=" + label,
            startTime);
    return label;
  }
  ...
  public Map<Long, String> getLabelsByIds(List<Long> dataTypeIds) {
    Instant startTime =
        LoggerFactory.db().logQueryStart(DATA_TYPE_ENTITY, "[findAllById] 데이터 유형 라벨 목록 조회 시작");
    Map<Long, String> labels =
        dataTypeJpaRepository.findAllById(dataTypeIds).stream()
