...
public class OccupationController implements OccupationApi {
  ...
  public ResponseEntity<SuccessResponse<AllOccupationsWebResponse>> findAllOccupations() {
    Instant startTime =
        LoggerFactory.api().logRequest("[FindAllOccupations] 전체 직업 목록을 조회 API 요청 시작");
    AllOccupationsWebResponse allOccupationsWebResponse;

    try {
      AllOccupationsResponse allOccupationsResponse =
          findAllOccupationsUseCase.findAllOccupations();
    ...
    return ResponseEntity.status(HttpStatus.OK)
        .body(
            SuccessResponse.of(
                ReferenceSuccessStatus.OK_TOTAL_OCCUPATION_LIST, allOccupationsWebResponse));
  }
}
