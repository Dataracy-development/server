...
/** occupation 웹 DTO와 occupation 애플리케이션 DTO를 변환하는 매퍼 */
@Component
public class OccupationWebMapper {
  /**
   * 단일 직업 애플리케이션 응답 DTO를 웹 응답 DTO로 변환합니다.
   *
   * @param occupationResponse 변환할 직업 애플리케이션 응답 DTO
  ...
  public OccupationWebResponse toWebDto(OccupationResponse occupationResponse) {
    return new OccupationWebResponse(
        occupationResponse.id(), occupationResponse.value(), occupationResponse.label());
  }

  /**
   * 애플리케이션 전체 직업 응답 DTO를 웹 전체 직업 응답 DTO로 변환합니다.
   *
   * @param allOccupationsResponse 변환할 애플리케이션 전체 직업 응답 DTO
  ...
  public AllOccupationsWebResponse toWebDto(AllOccupationsResponse allOccupationsResponse) {
    ...
    return new AllOccupationsWebResponse(
        allOccupationsResponse.occupations().stream().map(this::toWebDto).toList());
  }
}
