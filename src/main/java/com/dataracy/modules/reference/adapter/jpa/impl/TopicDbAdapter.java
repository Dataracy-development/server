...
public class TopicDbAdapter implements TopicPort {
    ...
    public Optional<Topic> findTopicById(Long topicId) {
        Instant startTime = LoggerFactory.db().logQueryStart(TOPIC_ENTITY, "[findById] 토픽 목록 조회 시작 topicId=" + topicId);
        if (topicId == null) {
            return Optional.empty();
        }
        Optional<Topic> topic = topicJpaRepository.findById(topicId)
                .map(TopicEntityMapper::toDomain);
        LoggerFactory.db().logQueryEnd(TOPIC_ENTITY, "[findById] 토픽 목록 조회 종료 topicId=" + topicId, startTime);
        return topic;
    }
    ...
    public boolean existsTopicById(Long topicId) {
        ...
        LoggerFactory.db().logExist(TOPIC_ENTITY, "[existsById] 토픽 존재 유무 확인 topicId=" + topicId + ", isExists=" + isExists);
        return isExists;
    }
    ...
    public Optional<String> getLabelById(Long topicId) {
        Instant startTime = LoggerFactory.db().logQueryStart(TOPIC_ENTITY, "[findLabelById] 토픽 라벨 조회 시작 topicId=" + topicId);
        if (topicId == null) {
            return Optional.empty();
        }
        Optional<String> label = topicJpaRepository.findLabelById(topicId);
        LoggerFactory.db().logQueryEnd(TOPIC_ENTITY, "[findLabelById] 토픽 라벨 조회 종료 topicId=" + topicId + ", label=" + label, startTime);
        return label;
    }
    ...
    public Map<Long, String> getLabelsByIds(List<Long> topicIds) {
        Instant startTime = LoggerFactory.db().logQueryStart(TOPIC_ENTITY, "[findAllById] 토픽 라벨 목록 조회 시작");
