...
public class DataSourceDbAdapter implements DataSourcePort {
  ...
  public List<DataSource> findAllDataSources() {
    Instant startTime =
        LoggerFactory.db().logQueryStart(DATA_SOURCE_ENTITY, "[findAll] 데이터 출처 목록 조회 시작");

    List<DataSourceEntity> dataSourceEntities = dataSourceJpaRepository.findAll();
    List<DataSource> dataSources =
        dataSourceEntities.stream().map(DataSourceEntityMapper::toDomain).toList();
  ...
  public Optional<DataSource> findDataSourceById(Long dataSourceId) {
    Instant startTime =
        LoggerFactory.db()
            .logQueryStart(
                DATA_SOURCE_ENTITY, "[findById] 데이터 출처 목록 조회 시작 dataSourceId=" + dataSourceId);

    if (dataSourceId == null) {
      return Optional.empty();
    }
    Optional<DataSource> dataSource =
        dataSourceJpaRepository.findById(dataSourceId).map(DataSourceEntityMapper::toDomain);

    LoggerFactory.db()
        .logQueryEnd(
            DATA_SOURCE_ENTITY,
            "[findById] 데이터 출처 목록 조회 종료 dataSourceId=" + dataSourceId,
            startTime);
    return dataSource;
  }
  ...
  public boolean existsDataSourceById(Long dataSourceId) {
    ...
    LoggerFactory.db()
        .logExist(
            DATA_SOURCE_ENTITY,
            "[existsById] 데이터 출처 존재 유무 확인 dataSourceId=" + dataSourceId + ", isExists=" + isExists);
    return isExists;
  }
  ...
  public Optional<String> getLabelById(Long dataSourceId) {
    Instant startTime =
        LoggerFactory.db()
            .logQueryStart(
                DATA_SOURCE_ENTITY, "[findLabelById] 데이터 출처 라벨 조회 시작 dataSourceId=" + dataSourceId);
    if (dataSourceId == null) {
      return Optional.empty();
    }
    ...
    LoggerFactory.db()
        .logQueryEnd(
            DATA_SOURCE_ENTITY,
            "[findLabelById] 데이터 출처 라벨 조회 종료 dataSourceId=" + dataSourceId + ", label=" + label,
            startTime);
    return label;
  }
  ...
  public Map<Long, String> getLabelsByIds(List<Long> dataSourceIds) {
    Instant startTime =
        LoggerFactory.db().logQueryStart(DATA_SOURCE_ENTITY, "[findAllById] 데이터 출처 라벨 목록 조회 시작");
    Map<Long, String> labels =
        dataSourceJpaRepository.findAllById(dataSourceIds).stream()
            .collect(Collectors.toMap(DataSourceEntity::getId, DataSourceEntity::getLabel));
    LoggerFactory.db()
        .logQueryEnd(DATA_SOURCE_ENTITY, "[findAllById] 데이터 출처 라벨 목록 조회 종료", startTime);
    return labels;
  }
}
