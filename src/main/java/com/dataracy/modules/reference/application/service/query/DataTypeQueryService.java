...
public class DataTypeQueryService
    implements FindAllDataTypesUseCase,
        FindDataTypeUseCase,
  ...
  private static final String GET_DATA_TYPE_LABEL_FROM_ID_USE_CASE =
      "GetDataTypeLabelFromIdUseCase";
  ...
  public AllDataTypesResponse findAllDataTypes() {
    Instant startTime =
        LoggerFactory.service().logStart(FIND_ALL_DATA_TYPES_USE_CASE, "모든 데이터 유형 정보 조회 서비스 시작");
    ...
    LoggerFactory.service()
        .logSuccess(FIND_ALL_DATA_TYPES_USE_CASE, "모든 데이터 유형 정보 조회 서비스 종료", startTime);
    return allDataTypesResponse;
  }

  /**
   * 주어진 ID에 해당하는 데이터 유형을 조회하여 DataTypeResponse DTO로 반환한다. 데이터 유형이 존재하지 않을 경우 ReferenceException이
   * 발생한다.
   *
  ...
  public DataTypeResponse findDataType(Long dataTypeId) {
    Instant startTime =
        LoggerFactory.service()
            .logStart(FIND_DATA_TYPE_USE_CASE, "주어진 ID로 데이터 유형 조회 서비스 시작 dataTypeId=" + dataTypeId);
    DataType dataType =
        dataTypePort
            .findDataTypeById(dataTypeId)
            .orElseThrow(
                () -> {
                  LoggerFactory.service()
                      .logWarning(
                          FIND_DATA_TYPE_USE_CASE, DATA_TYPE_NOT_FOUND_MESSAGE + dataTypeId);
    ...
    LoggerFactory.service()
        .logSuccess(
            FIND_DATA_TYPE_USE_CASE,
            "주어진 ID로 데이터 유형 조회 서비스 종료 dataTypeId=" + dataTypeId,
            startTime);
    return dataTypeResponse;
  }

  /**
   * 주어진 데이터 타입 ID의 존재 여부를 검증합니다. 데이터 타입이 존재하지 않을 경우 ReferenceException을 발생시킵니다.
   *
  ...
  public void validateDataType(Long dataTypeId) {
    Instant startTime =
        LoggerFactory.service()
            .logStart(
                VALIDATE_DATA_TYPE_USE_CASE,
                "주어진 ID에 해당하는 데이터 유형이 존재하는지 확인 서비스 시작 dataTypeId=" + dataTypeId);
    boolean isExist = dataTypePort.existsDataTypeById(dataTypeId);
    if (!isExist) {
      LoggerFactory.service()
          .logWarning(VALIDATE_DATA_TYPE_USE_CASE, DATA_TYPE_NOT_FOUND_MESSAGE + dataTypeId);
      throw new ReferenceException(ReferenceErrorStatus.NOT_FOUND_DATA_TYPE);
    }
    LoggerFactory.service()
        .logSuccess(
            VALIDATE_DATA_TYPE_USE_CASE,
            "주어진 ID에 해당하는 데이터 유형이 존재하는지 확인 서비스 종료 dataTypeId=" + dataTypeId,
            startTime);
  }

  /**
   * 주어진 데이터 타입 ID에 해당하는 라벨을 반환합니다. 데이터 타입이 존재하지 않으면 ReferenceException이 발생합니다.
   *
  ...
  public String getLabelById(Long dataTypeId) {
    Instant startTime =
        LoggerFactory.service()
            .logStart(
                GET_DATA_TYPE_LABEL_FROM_ID_USE_CASE,
                "주어진 데이터 유형 ID에 해당하는 라벨을 조회 서비스 시작 dataTypeId=" + dataTypeId);
    String label =
        dataTypePort
            .getLabelById(dataTypeId)
            .orElseThrow(
                () -> {
                  LoggerFactory.service()
                      .logWarning(
                          GET_DATA_TYPE_LABEL_FROM_ID_USE_CASE,
                          DATA_TYPE_NOT_FOUND_MESSAGE + dataTypeId);
                  return new ReferenceException(ReferenceErrorStatus.NOT_FOUND_DATA_TYPE);
                });
    LoggerFactory.service()
        .logSuccess(
            GET_DATA_TYPE_LABEL_FROM_ID_USE_CASE,
            "주어진 데이터 유형 ID에 해당하는 라벨을 조회 서비스 종료 dataTypeId=" + dataTypeId,
            startTime);
    return label;
  }
  ...
  public Map<Long, String> getLabelsByIds(List<Long> dataTypeIds) {
    Instant startTime =
        LoggerFactory.service()
            .logStart(
                GET_DATA_TYPE_LABEL_FROM_ID_USE_CASE, "데이터 유형 ID 목록에 대해 각 ID에 해당하는 라벨을 반환 서비스 시작");
    ...
    LoggerFactory.service()
        .logSuccess(
            GET_DATA_TYPE_LABEL_FROM_ID_USE_CASE,
            "데이터 유형 ID 목록에 대해 각 ID에 해당하는 라벨을 반환 서비스 종료",
            startTime);
    return labels;
  }
}
