...
public class VisitSourceQueryService
    implements FindAllVisitSourcesUseCase,
  ...
  private static final String GET_VISIT_SOURCE_LABEL_FROM_ID_USE_CASE =
      "GetVisitSourceLabelFromIdUseCase";

  // 메시지 상수 정의
  private static final String VISIT_SOURCE_NOT_FOUND_MESSAGE =
      "해당 방문 경로가 존재하지 않습니다. visitSourceId=";
  ...
  public AllVisitSourcesResponse findAllVisitSources() {
    Instant startTime =
        LoggerFactory.service().logStart(FIND_ALL_VISIT_SOURCES_USE_CASE, "모든 방문 경로 정보 조회 서비스 시작");
    List<VisitSource> visitSources = visitSourcePort.findAllVisitSources();
    AllVisitSourcesResponse allVisitSourcesResponse =
        visitSourceDtoMapper.toResponseDto(visitSources);
    LoggerFactory.service()
        .logSuccess(FIND_ALL_VISIT_SOURCES_USE_CASE, "모든 방문 경로 정보 조회 서비스 종료", startTime);
    return allVisitSourcesResponse;
  }

  /**
   * 주어진 ID에 해당하는 방문 경로 정보를 조회하여 VisitSourceResponse DTO로 반환한다. 방문 경로가 존재하지 않을 경우
   * ReferenceException이 발생한다.
   *
  ...
  public VisitSourceResponse findVisitSource(Long visitSourceId) {
    Instant startTime =
        LoggerFactory.service()
            .logStart(
                FIND_VISIT_SOURCE_USE_CASE,
                "주어진 ID로 방문 경로 조회 서비스 시작 visitSourceId=" + visitSourceId);
    VisitSource visitSource =
        visitSourcePort
            .findVisitSourceById(visitSourceId)
            .orElseThrow(
                () -> {
                  LoggerFactory.service()
                      .logWarning(
                          FIND_VISIT_SOURCE_USE_CASE,
                          VISIT_SOURCE_NOT_FOUND_MESSAGE + visitSourceId);
    ...
    LoggerFactory.service()
        .logSuccess(
            FIND_VISIT_SOURCE_USE_CASE,
            "주어진 ID로 방문 경로 조회 서비스 종료 visitSourceId=" + visitSourceId,
            startTime);
    return visitSourceResponse;
  }

  /**
   * 주어진 방문 출처 ID가 존재하는지 검증합니다. 방문 출처가 존재하지 않을 경우 ReferenceException을 발생시킵니다.
   *
  ...
  public void validateVisitSource(Long visitSourceId) {
    Instant startTime =
        LoggerFactory.service()
            .logStart(
                VALIDATE_VISIT_SOURCE_USE_CASE,
                "주어진 ID에 해당하는 방문 경로가 존재하는지 확인 서비스 시작 visitSourceId=" + visitSourceId);
    boolean isExist = visitSourcePort.existsVisitSourceById(visitSourceId);
    if (!isExist) {
      LoggerFactory.service()
          .logWarning(
              VALIDATE_VISIT_SOURCE_USE_CASE, VISIT_SOURCE_NOT_FOUND_MESSAGE + visitSourceId);
      throw new ReferenceException(ReferenceErrorStatus.NOT_FOUND_VISIT_SOURCE);
    }
    LoggerFactory.service()
        .logSuccess(
            VALIDATE_VISIT_SOURCE_USE_CASE,
            "주어진 ID에 해당하는 방문 경로가 존재하는지 확인 서비스 종료 visitSourceId=" + visitSourceId,
            startTime);
  }

  /**
   * 주어진 방문 출처 ID에 해당하는 라벨을 반환합니다. 방문 출처가 존재하지 않을 경우 ReferenceException이 발생합니다.
   *
  ...
  public String getLabelById(Long visitSourceId) {
    Instant startTime =
        LoggerFactory.service()
            .logStart(
                GET_VISIT_SOURCE_LABEL_FROM_ID_USE_CASE,
                "주어진 방문 경로 ID에 해당하는 라벨을 조회 서비스 시작 visitSourceId=" + visitSourceId);
    String label =
        visitSourcePort
            .getLabelById(visitSourceId)
            .orElseThrow(
                () -> {
                  LoggerFactory.service()
                      .logWarning(
    ...
    LoggerFactory.service()
        .logSuccess(
            GET_VISIT_SOURCE_LABEL_FROM_ID_USE_CASE,
            "주어진 방문 경로 ID에 해당하는 라벨을 조회 서비스 종료 visitSourceId=" + visitSourceId,
            startTime);
    return label;
  }

  /**
   * 주어진 방문 출처 ID 목록에 대해 각 ID와 해당 라벨을 매핑한 Map을 반환합니다. 입력된 ID 목록이 null이거나 비어 있으면 빈 Map을 반환합니다.
   *
  ...
  public Map<Long, String> getLabelsByIds(List<Long> visitSourceIds) {
    Instant startTime =
        LoggerFactory.service()
            .logStart(
                GET_VISIT_SOURCE_LABEL_FROM_ID_USE_CASE,
                "방문 경로 ID 목록에 대해 각 ID에 해당하는 라벨을 반환 서비스 시작");
    ...
    LoggerFactory.service()
        .logSuccess(
            GET_VISIT_SOURCE_LABEL_FROM_ID_USE_CASE,
            "방문 경로 ID 목록에 대해 각 ID에 해당하는 라벨을 반환 서비스 종료",
            startTime);
    return labels;
  }
}
