...
public class TopicQueryService
    implements FindAllTopicsUseCase,
        FindTopicUseCase,
  ...
  public AllTopicsResponse findAllTopics() {
    Instant startTime =
        LoggerFactory.service().logStart(FIND_ALL_TOPICS_USE_CASE, "모든 토픽 정보 조회 서비스 시작");
  ...
  public TopicResponse findTopic(Long topicId) {
    Instant startTime =
        LoggerFactory.service()
            .logStart(FIND_TOPIC_USE_CASE, "주어진 ID로 토픽 조회 서비스 시작 topicId=" + topicId);
    Topic topic =
        topicPort
            ...
            .orElseThrow(
                () -> {
                  LoggerFactory.service()
                      .logWarning(FIND_TOPIC_USE_CASE, TOPIC_NOT_FOUND_MESSAGE + topicId);
    ...
    LoggerFactory.service()
        .logSuccess(FIND_TOPIC_USE_CASE, "주어진 ID로 토픽 조회 서비스 종료 topicId=" + topicId, startTime);
    return topicResponse;
  }

  /**
   * 주어진 토픽 ID에 해당하는 토픽의 존재 여부를 검증합니다. 토픽이 존재하지 않을 경우 ReferenceException을 발생시킵니다.
   *
  ...
  public void validateTopic(Long topicId) {
    Instant startTime =
        LoggerFactory.service()
            .logStart(
                VALIDATE_TOPIC_USE_CASE, "주어진 ID에 해당하는 토픽이 존재하는지 확인 서비스 시작 topicId=" + topicId);
    boolean isExist = topicPort.existsTopicById(topicId);
    if (!isExist) {
      LoggerFactory.service()
          .logWarning(VALIDATE_TOPIC_USE_CASE, TOPIC_NOT_FOUND_MESSAGE + topicId);
      throw new ReferenceException(ReferenceErrorStatus.NOT_FOUND_TOPIC_NAME);
    }
    LoggerFactory.service()
        .logSuccess(
            VALIDATE_TOPIC_USE_CASE,
            "주어진 ID에 해당하는 토픽이 존재하는지 확인 서비스 종료 topicId=" + topicId,
            startTime);
  }
  ...
  public String getLabelById(Long topicId) {
    Instant startTime =
        LoggerFactory.service()
            .logStart(
                GET_TOPIC_LABEL_FROM_ID_USE_CASE,
                "주어진 토픽 ID에 해당하는 라벨을 조회 서비스 시작 topicId=" + topicId);
    String label =
        topicPort
            .getLabelById(topicId)
            .orElseThrow(
                () -> {
                  LoggerFactory.service()
                      .logWarning(
                          GET_TOPIC_LABEL_FROM_ID_USE_CASE, TOPIC_NOT_FOUND_MESSAGE + topicId);
                  return new ReferenceException(ReferenceErrorStatus.NOT_FOUND_TOPIC_NAME);
                });
    LoggerFactory.service()
        .logSuccess(
            GET_TOPIC_LABEL_FROM_ID_USE_CASE,
            "주어진 토픽 ID에 해당하는 라벨을 조회 서비스 종료 topicId=" + topicId,
            startTime);
    return label;
  }
  ...
  public Map<Long, String> getLabelsByIds(List<Long> topicIds) {
    Instant startTime =
        LoggerFactory.service()
            .logStart(GET_TOPIC_LABEL_FROM_ID_USE_CASE, "토픽 ID 목록에 대해 각 ID에 해당하는 라벨을 반환 서비스 시작");
    ...
    LoggerFactory.service()
        .logSuccess(
            GET_TOPIC_LABEL_FROM_ID_USE_CASE, "토픽 ID 목록에 대해 각 ID에 해당하는 라벨을 반환 서비스 종료", startTime);
    return labels;
  }
}
