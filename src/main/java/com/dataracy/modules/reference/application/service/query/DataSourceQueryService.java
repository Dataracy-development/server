...
public class DataSourceQueryService
    implements FindAllDataSourcesUseCase,
  ...
  private static final String GET_DATA_SOURCE_LABEL_FROM_ID_USE_CASE =
      "GetDataSourceLabelFromIdUseCase";

  // 메시지 상수 정의
  private static final String DATA_SOURCE_NOT_FOUND_MESSAGE = "해당 데이터 출처가 존재하지 않습니다. dataSourceId=";
  private static final String DATA_SOURCE_NOT_FOUND_MESSAGE_2 =
      "해당 데이터 소스가 존재하지 않습니다. dataSourceId=";
  ...
  public AllDataSourcesResponse findAllDataSources() {
    Instant startTime =
        LoggerFactory.service().logStart(FIND_ALL_DATA_SOURCES_USE_CASE, "모든 데이터 출처 정보 조회 서비스 시작");
    ...
    LoggerFactory.service()
        .logSuccess(FIND_ALL_DATA_SOURCES_USE_CASE, "모든 데이터 출처 정보 조회 서비스 종료", startTime);
    return allDataSourcesResponse;
  }

  /**
   * 주어진 ID에 해당하는 데이터 출처 정보를 조회하여 DataSourceResponse DTO로 반환한다. 데이터 출처가 존재하지 않을 경우
   * ReferenceException을 발생시킨다.
   *
  ...
  public DataSourceResponse findDataSource(Long dataSourceId) {
    Instant startTime =
        LoggerFactory.service()
            .logStart(
                FIND_DATA_SOURCE_USE_CASE, "주어진 ID로 데이터 출처 조회 서비스 시작 dataSourceId=" + dataSourceId);
        ...
        dataSourcePort
            ...
            .orElseThrow(
                () -> {
                  LoggerFactory.service()
                      .logWarning(
                          FIND_DATA_SOURCE_USE_CASE, DATA_SOURCE_NOT_FOUND_MESSAGE + dataSourceId);
    ...
    LoggerFactory.service()
        .logSuccess(
            FIND_DATA_SOURCE_USE_CASE,
            "주어진 ID로 데이터 출처 조회 서비스 종료 dataSourceId=" + dataSourceId,
            startTime);
    return dataSourceResponse;
  }

  /**
   * 주어진 데이터 소스 ID가 존재하는지 검증합니다. 데이터 소스가 존재하지 않을 경우 ReferenceException을 발생시킵니다.
   *
  ...
  public void validateDataSource(Long dataSourceId) {
    Instant startTime =
        LoggerFactory.service()
            .logStart(
                VALIDATE_DATA_SOURCE_USE_CASE,
                "주어진 ID에 해당하는 데이터 소스가 존재하는지 확인 서비스 시작 dataSourceId=" + dataSourceId);
    boolean isExist = dataSourcePort.existsDataSourceById(dataSourceId);
    if (!isExist) {
      LoggerFactory.service()
          .logWarning(
              VALIDATE_DATA_SOURCE_USE_CASE, DATA_SOURCE_NOT_FOUND_MESSAGE_2 + dataSourceId);
      throw new ReferenceException(ReferenceErrorStatus.NOT_FOUND_DATA_SOURCE);
    }
    LoggerFactory.service()
        .logSuccess(
            VALIDATE_DATA_SOURCE_USE_CASE,
            "주어진 ID에 해당하는 데이터 소스가 존재하는지 확인 서비스 종료 dataSourceId=" + dataSourceId,
            startTime);
  }

  /**
   * 주어진 데이터 소스 ID로 해당 데이터 소스의 라벨을 반환합니다. 데이터 소스가 존재하지 않을 경우 ReferenceException이 발생합니다.
   *
  ...
  public String getLabelById(Long dataSourceId) {
    Instant startTime =
        LoggerFactory.service()
            .logStart(
                GET_DATA_SOURCE_LABEL_FROM_ID_USE_CASE,
                "주어진 데이터 소스 ID에 해당하는 라벨을 조회 서비스 시작 dataSourceId=" + dataSourceId);
    String label =
        dataSourcePort
            .getLabelById(dataSourceId)
            .orElseThrow(
                () -> {
                  LoggerFactory.service()
                      .logWarning(
                          GET_DATA_SOURCE_LABEL_FROM_ID_USE_CASE,
                          DATA_SOURCE_NOT_FOUND_MESSAGE_2 + dataSourceId);
                  return new ReferenceException(ReferenceErrorStatus.NOT_FOUND_DATA_SOURCE);
                });
    LoggerFactory.service()
        .logSuccess(
            GET_DATA_SOURCE_LABEL_FROM_ID_USE_CASE,
            "주어진 데이터 소스 ID에 해당하는 라벨을 조회 서비스 종료 dataSourceId=" + dataSourceId,
            startTime);
    return label;
  }
  ...
  public Map<Long, String> getLabelsByIds(List<Long> dataSourceIds) {
    Instant startTime =
        LoggerFactory.service()
            .logStart(
                GET_DATA_SOURCE_LABEL_FROM_ID_USE_CASE,
                "데이터 소스 ID 목록에 대해 각 ID에 해당하는 라벨을 반환 서비스 시작");
    ...
    LoggerFactory.service()
        .logSuccess(
            GET_DATA_SOURCE_LABEL_FROM_ID_USE_CASE,
            "데이터 소스 ID 목록에 대해 각 ID에 해당하는 라벨을 반환 서비스 종료",
            startTime);
    return labels;
  }
}
