...
public class OccupationQueryService
    implements FindAllOccupationsUseCase,
  ...
  private static final String GET_OCCUPATION_LABEL_FROM_ID_USE_CASE =
      "GetOccupationLabelFromIdUseCase";
  ...
  public AllOccupationsResponse findAllOccupations() {
    Instant startTime =
        LoggerFactory.service().logStart(FIND_ALL_OCCUPATIONS_USE_CASE, "모든 직업 정보 조회 서비스 시작");
    ...
    LoggerFactory.service()
        .logSuccess(FIND_ALL_OCCUPATIONS_USE_CASE, "모든 직업 정보 조회 서비스 종료", startTime);
  ...
  public OccupationResponse findOccupation(Long occupationId) {
    Instant startTime =
        LoggerFactory.service()
            .logStart(
                FIND_OCCUPATION_USE_CASE, "주어진 ID로 직업 조회 서비스 시작 occupationId=" + occupationId);
    Occupation occupation =
        occupationPort
            .findOccupationById(occupationId)
            .orElseThrow(
                () -> {
                  LoggerFactory.service()
                      .logWarning(
                          FIND_OCCUPATION_USE_CASE, OCCUPATION_NOT_FOUND_MESSAGE + occupationId);
    ...
    LoggerFactory.service()
        .logSuccess(
            FIND_OCCUPATION_USE_CASE,
            "주어진 ID로 직업 조회 서비스 종료 occupationId=" + occupationId,
            startTime);
    return occupationResponse;
  }

  /**
   * 주어진 직업 ID에 해당하는 직업이 존재하는지 검증합니다. 직업이 존재하지 않을 경우 ReferenceException을 발생시킵니다.
   *
  ...
  public void validateOccupation(Long occupationId) {
    Instant startTime =
        LoggerFactory.service()
            .logStart(
                VALIDATE_OCCUPATION_USE_CASE,
                "주어진 ID에 해당하는 직업이 존재하는지 확인 서비스 시작 occupationId=" + occupationId);
    boolean isExist = occupationPort.existsOccupationById(occupationId);
    if (!isExist) {
      LoggerFactory.service()
          .logWarning(VALIDATE_OCCUPATION_USE_CASE, OCCUPATION_NOT_FOUND_MESSAGE + occupationId);
      throw new ReferenceException(ReferenceErrorStatus.NOT_FOUND_OCCUPATION);
    }
    LoggerFactory.service()
        .logSuccess(
            VALIDATE_OCCUPATION_USE_CASE,
            "주어진 ID에 해당하는 직업이 존재하는지 확인 서비스 종료 occupationId=" + occupationId,
            startTime);
  }
  ...
  public String getLabelById(Long occupationId) {
    Instant startTime =
        LoggerFactory.service()
            .logStart(
                GET_OCCUPATION_LABEL_FROM_ID_USE_CASE,
                "주어진 직업 ID에 해당하는 라벨을 조회 서비스 시작 occupationId=" + occupationId);
    String label =
        occupationPort
            .getLabelById(occupationId)
            .orElseThrow(
                () -> {
                  LoggerFactory.service()
                      .logWarning(
                          GET_OCCUPATION_LABEL_FROM_ID_USE_CASE,
                          OCCUPATION_NOT_FOUND_MESSAGE + occupationId);
                  return new ReferenceException(ReferenceErrorStatus.NOT_FOUND_OCCUPATION);
                });
    LoggerFactory.service()
        .logSuccess(
            GET_OCCUPATION_LABEL_FROM_ID_USE_CASE,
            "주어진 직업 ID에 해당하는 라벨을 조회 서비스 종료 occupationId=" + occupationId,
            startTime);
    return label;
  }
  ...
  public Map<Long, String> getLabelsByIds(List<Long> occupationIds) {
    Instant startTime =
        LoggerFactory.service()
            .logStart(
                GET_OCCUPATION_LABEL_FROM_ID_USE_CASE, "직업 ID 목록에 대해 각 ID에 해당하는 라벨을 반환 서비스 시작");
    ...
    LoggerFactory.service()
        .logSuccess(
            GET_OCCUPATION_LABEL_FROM_ID_USE_CASE,
            "직업 ID 목록에 대해 각 ID에 해당하는 라벨을 반환 서비스 종료",
            startTime);
    return labels;
  }
}
