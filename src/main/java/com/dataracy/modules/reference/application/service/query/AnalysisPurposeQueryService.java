...
public class AnalysisPurposeQueryService
    implements FindAllAnalysisPurposesUseCase,
  ...
  // Use Case 상수 정의
  private static final String FIND_ALL_ANALYSIS_PURPOSES_USE_CASE =
  ...
  private static final String GET_ANALYSIS_PURPOSE_LABEL_FROM_ID_USE_CASE =
      "GetAnalysisPurposeLabelFromIdUseCase";
  private static final String ANALYSIS_PURPOSE_NOT_FOUND_MESSAGE =
      "해당 분석 목적이 존재하지 않습니다. analysisPurposeId=";
  ...
  public AllAnalysisPurposesResponse findAllAnalysisPurposes() {
    Instant startTime =
        LoggerFactory.service()
            .logStart(FIND_ALL_ANALYSIS_PURPOSES_USE_CASE, "모든 분석 목적 정보 조회 서비스 시작");
    ...
    AllAnalysisPurposesResponse allAnalysisPurposesResponse =
        analysisPurposeDtoMapper.toResponseDto(analysisPurposes);
    LoggerFactory.service()
        .logSuccess(FIND_ALL_ANALYSIS_PURPOSES_USE_CASE, "모든 분석 목적 정보 조회 서비스 종료", startTime);
    return allAnalysisPurposesResponse;
  }

  /**
   * 주어진 ID에 해당하는 분석 목적의 상세 정보를 조회하여 반환한다. 분석 목적이 존재하지 않을 경우 ReferenceException이 발생한다.
   *
  ...
  public AnalysisPurposeResponse findAnalysisPurpose(Long analysisPurposeId) {
    Instant startTime =
        LoggerFactory.service()
            .logStart(
                FIND_ANALYSIS_PURPOSE_USE_CASE,
                "주어진 ID로 분석 목적 조회 서비스 시작 analysisPurposeId=" + analysisPurposeId);
    AnalysisPurpose analysisPurpose =
        analysisPurposePort
            .findAnalysisPurposeById(analysisPurposeId)
            .orElseThrow(
                () -> {
                  LoggerFactory.service()
                      .logWarning(
                          FIND_ANALYSIS_PURPOSE_USE_CASE,
                          ANALYSIS_PURPOSE_NOT_FOUND_MESSAGE + analysisPurposeId);
                  return new ReferenceException(ReferenceErrorStatus.NOT_FOUND_ANALYSIS_PURPOSE);
                });
    AnalysisPurposeResponse analysisPurposeResponse =
        analysisPurposeDtoMapper.toResponseDto(analysisPurpose);
    LoggerFactory.service()
        .logSuccess(
            FIND_ANALYSIS_PURPOSE_USE_CASE,
            "주어진 ID로 분석 목적 조회 서비스 종료 analysisPurposeId=" + analysisPurposeId,
            startTime);
    return analysisPurposeResponse;
  }

  /**
   * 주어진 ID로 분석 목적의 존재 여부를 검증합니다. 해당 ID의 분석 목적이 존재하지 않으면 ReferenceException을 발생시킵니다.
   *
  ...
  public void validateAnalysisPurpose(Long analysisPurposeId) {
    Instant startTime =
        LoggerFactory.service()
            .logStart(
                VALIDATE_ANALYSIS_PURPOSE_USE_CASE,
  ...
  public String getLabelById(Long analysisPurposeId) {
    Instant startTime =
        LoggerFactory.service()
            .logStart(
                GET_ANALYSIS_PURPOSE_LABEL_FROM_ID_USE_CASE,
                "주어진 분석 목적 ID에 해당하는 라벨을 조회 서비스 시작 analysisPurposeId=" + analysisPurposeId);
    String label =
        analysisPurposePort
            .getLabelById(analysisPurposeId)
            .orElseThrow(
                () -> {
                  LoggerFactory.service()
                      .logWarning(
    ...
    LoggerFactory.service()
        .logSuccess(
            GET_ANALYSIS_PURPOSE_LABEL_FROM_ID_USE_CASE,
            "주어진 분석 목적 ID에 해당하는 라벨을 조회 서비스 종료 analysisPurposeId=" + analysisPurposeId,
            startTime);
    return label;
  }
  ...
  public Map<Long, String> getLabelsByIds(List<Long> analysisPurposeIds) {
    Instant startTime =
        LoggerFactory.service()
            .logStart(
                GET_ANALYSIS_PURPOSE_LABEL_FROM_ID_USE_CASE,
                "분석 목적 ID 목록에 대해 각 ID에 해당하는 라벨을 반환 서비스 시작");
    ...
    LoggerFactory.service()
        .logSuccess(
            GET_ANALYSIS_PURPOSE_LABEL_FROM_ID_USE_CASE,
            "분석 목적 ID 목록에 대해 각 ID에 해당하는 라벨을 반환 서비스 종료",
            startTime);
    return labels;
  }
}
