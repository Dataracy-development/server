...
public abstract class AbstractKafkaProducerConfig<V> {
  ...
  /**
   * Kafka 프로듀서의 기본 설정 값을 담은 맵을 반환합니다. 반환되는 맵에는 bootstrap servers, acks, retries 설정이 포함되어 있습니다.
   *
  ...
  /**
   * Kafka 프로듀서의 키 직렬화에 사용할 클래스 타입을 반환합니다. 구체 서브클래스에서 반드시 구현해야 하며, 키 직렬화에 적합한 Serializer 클래스를 지정해야
   * 합니다.
   *
  ...
  /**
   * Kafka 프로듀서를 위한 ProducerFactory 인스턴스를 생성합니다. 기본 프로듀서 설정과 서브클래스에서 지정한 Key/Value 직렬화 클래스를 포함하여
   * ProducerFactory를 반환합니다.
   *
  ...
  /**
   * Kafka bootstrap-servers 설정이 비어 있는지 검증합니다. 설정 값이 null이거나 공백일 경우 IllegalStateException을 발생시킵니다.
   *
