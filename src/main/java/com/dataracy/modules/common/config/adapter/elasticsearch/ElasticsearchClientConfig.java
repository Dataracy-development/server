...
public class ElasticsearchClientConfig {
   ...
   *
   * <p>호스트, 포트, 프로토콜 등 설정 정보를 기반으로 연결을 구성하며, 연결 및 소켓 타임아웃(각각 5000ms, 60000ms)과 최대 연결 수(총 100개, 라우트당
   * 10개)를 지정한다. 애플리케이션 종료 시 자동으로 연결이 닫혀 리소스 누수를 방지한다.
   *
  ...
  public RestClient restClient() {
    return RestClient.builder(
        ...
        .setRequestConfigCallback(
            requestConfig -> requestConfig.setConnectTimeout(5000).setSocketTimeout(60000))
        .setHttpClientConfigCallback(
            httpClient -> httpClient.setMaxConnTotal(100).setMaxConnPerRoute(10))
        .build();
  }
