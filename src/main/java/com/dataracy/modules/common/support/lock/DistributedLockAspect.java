...
public class DistributedLockAspect {
   ...
   *
   * <p>분산 락 키는 {@link DistributedLock} 어노테이션의 SpEL 표현식을 기반으로 생성됩니다.
  ...
  public Object around(ProceedingJoinPoint joinPoint, DistributedLock distributedLock) throws Throwable {
    ...
    LoggerFactory.common()
        .logInfo(
            ...
            String.format(
                ...
                className, methodName, lockKey, distributedLock.waitTime(), distributedLock.leaseTime(),
    ...
    try {
      ...
      if (!acquired) {
        String message =
            String.format(
                "다른 사용자가 해당 자원에 접근 중입니다. (key: %s) 잠시 후 다시 시도해주세요.", lockKey);
        LoggerFactory.common()
            .logWarning(ASPECT_NAME, String.format("[분산락] 락 획득 실패 - class: %s method: %s key: %s",
      ...
      LoggerFactory.common()
          .logInfo(ASPECT_NAME,
              String.format("[분산락] 락 획득 성공, 원본 메서드 실행 시작 - class: %s method: %s key: %s",
                  className, methodName, lockKey));
      ...
      try {
        ...
        LoggerFactory.common()
            .logInfo(ASPECT_NAME,
                String.format("[분산락] 원본 메서드 실행 완료 - class: %s method: %s key: %s", className,
                    methodName, lockKey));
        return result;
      } catch (BusinessException | CommonException e) {
        // 비즈니스/공통 예외는 그대로 전파
        LoggerFactory.common()
            .logError(ASPECT_NAME,
                String.format(
      ...
      if (acquired && lock.isHeldByCurrentThread()) {
        try {
          ...
          LoggerFactory.common()
              .logInfo(ASPECT_NAME,
                  String.format("[분산락] 락 해제 성공 - class: %s method: %s key: %s", className, methodName,
                      lockKey));
        } catch (Exception e) {
          LoggerFactory.common()
              .logError(ASPECT_NAME,
                  String.format("[분산락] 락 해제 실패 - class: %s method: %s key: %s", className, methodName,
                      lockKey),
                  e);
        }
      }
  ...
  private String generateLockKey(ProceedingJoinPoint joinPoint, String keyExpression) {
    try {
      ...
      try {
        ...
        LoggerFactory.common()
            .logWarning(ASPECT_NAME, "파라미터 이름 추출 실패, 인덱스로 접근: " + e.getMessage());
      }
      ...
      if (result == null || result.toString().trim().isEmpty()) {
        LoggerFactory.common()
            .logError(ASPECT_NAME,
                String.format("SpEL 표현식 파싱 결과가 유효하지 않음 - 표현식: %s, 결과: %s", keyExpression,
                    result));
      ...
      LoggerFactory.common()
          .logInfo(ASPECT_NAME,
              String.format("락 키 생성 완료 - 표현식: %s, 키: %s", keyExpression, lockKey));
      return lockKey;

    } catch (org.springframework.expression.spel.SpelEvaluationException e) {
      LoggerFactory.common()
          .logError(ASPECT_NAME,
              String.format("SpEL 표현식 파싱 실패 - 표현식: %s", keyExpression), e);
      throw new LockAcquisitionException(
          "분산 락 키 SpEL 파싱 실패: " + keyExpression + " (method: " + joinPoint.getSignature().getName() + ")",
          e);
    } catch (Exception e) {
      LoggerFactory.common()
          .logError(ASPECT_NAME,
              String.format("락 키 생성 중 오류 발생 - 표현식: %s", keyExpression), e);
  ...
  private boolean tryLock(RLock lock, DistributedLock distributedLock) {
    ...
    for (int i = 0; i <= retryCount; i++) {
      try {
        ...
        LoggerFactory.common()
            .logError(ASPECT_NAME, "락 획득 중 오류 발생", e);
        if (i == retryCount) {
          return false;
        }
      }
