...
public class ReadCommentPortAdapter implements ReadCommentPort {
  ...
  public Optional<Comment> findCommentById(Long commentId) {
    Instant startTime =
        LoggerFactory.query()
            .logQueryStart(
                COMMENT_ENTITY, "[findCommentById] 주어진 ID에 해당하는 댓글 조회 시작. commentId=" + commentId);

    CommentEntity entity =
        queryFactory
            .selectFrom(comment)
            .where(CommentFilterPredicate.commentIdEq(commentId))
            .fetchOne();
    ...
    LoggerFactory.query()
        .logQueryEnd(
            COMMENT_ENTITY,
            "[findCommentById] 주어진 ID에 해당하는 댓글 조회 종료. commentId=" + commentId,
            startTime);
    return result;
  }
  ...
  public Page<FindCommentWithReplyCountResponse> findComments(Long projectId, Pageable pageable) {
    Instant startTime =
        LoggerFactory.query()
            .logQueryStart(
                COMMENT_ENTITY, "[findComments] 댓글당 답글 수를 포함한 댓글 목록 조회 시작. projectId=" + projectId);
    int queryCount = 0;
    ...
    // 3단계: DTO 조합 (메모리에서 처리)
    List<FindCommentWithReplyCountResponse> contents =
        commentEntities.stream()
            .map(
                entity ->
                    new FindCommentWithReplyCountResponse(
    ...
    LoggerFactory.query()
        .logQueryEnd(
            COMMENT_ENTITY,
            "[findComments] 댓글당 답글 수를 포함한 댓글 목록 조회 종료. projectId="
                + projectId
                + ", queryCount="
                + queryCount,
            startTime);
  ...
  public Page<Comment> findReplyComments(Long projectId, Long commentId, Pageable pageable) {
    Instant startTime =
        LoggerFactory.query()
            .logQueryStart(
                COMMENT_ENTITY,
                "[findReplyComments] 해당 댓글에 대한 답글 목록 조회 시작. projectId="
                    + projectId
                    + ", commentId="
                    + commentId);
    ...
    List<Comment> contents = entities.stream().map(CommentEntityMapper::toDomain).toList();
    ...
    LoggerFactory.query()
        .logQueryEnd(
            COMMENT_ENTITY,
            "[findReplyComments] 해당 댓글에 대한 답글 목록 조회 종료. projectId="
                + projectId
                + ", commentId="
                + commentId,
            startTime);
  ...
  private Map<Long, Long> getReplyCountsBatch(List<Long> commentIds) {
    ...
    return queryFactory
        ...
        .collect(
            Collectors.toMap(
                tuple -> tuple.get(child.parentCommentId), tuple -> tuple.get(child.id.count())));
  }
}
