...
public interface CommentCommandApi {
   ...
   *
   * <p>요청한 인증된 사용자(현재 로그인한 사용자)를 작성자로 하여 댓글을 추가합니다. 요청 본문에는 댓글 내용(content)과 선택적 부모 댓글
   * ID(parentCommentId)를 포함할 수 있으며, parentCommentId가 있으면 해당 댓글에 대한 답글로 처리됩니다.
   *
   ...
   */
  @Operation(summary = "프로젝트에 피드백 댓글을 작성한다.", description = "해당 프로젝트에 대한 피드백 댓글을 작성한다.")
  @Parameter(
   ...
   */
  @Operation(summary = "댓글을 수정한다.", description = "프로젝트의 해당하는 댓글을 수정한다.")
  @Parameter(
      in = ParameterIn.HEADER,
      name = "Authorization",
      required = true,
  ...
  @PutMapping(
      value = "/{projectId}/comments/{commentId}",
   ...
   */
  @Operation(summary = "피드백 댓글을 삭제한다.", description = "해당 피드백 댓글을 삭제한다.")
  @Parameter(
      in = ParameterIn.HEADER,
      name = "Authorization",
      required = true,
  ...
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
            description = "해당하는 피드백 댓글 삭제에 성공했습니다.",
            useReturnTypeSchema = true)
      })
  @DeleteMapping("/{projectId}/comments/{commentId}")
  ResponseEntity<SuccessResponse<Void>> deleteComment(
      @PathVariable @Min(1) Long projectId, @PathVariable @Min(1) Long commentId);
}
