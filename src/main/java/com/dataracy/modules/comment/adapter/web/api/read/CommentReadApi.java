...
public interface CommentReadApi {
   ...
   * @param pageable 페이지네이션 정보 (기본값: 페이지 0, 크기 5)
   * @return 프로젝트의 댓글 목록이 포함된 성공 응답 객체
   */
  @Operation(summary = "해당 프로젝트의 댓글 목록을 조회한다.", description = "해당 프로젝트의 댓글 목록을 조회한다.")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
            description = "해당 프로젝트의 댓글 목록 조회에 성공했습니다.",
  ...
  /**
   * 특정 프로젝트의 부모 댓글에 대한 답글 목록을 페이지네이션하여 조회한다.
   *
   * @param projectId 답글을 조회할 프로젝트의 ID (1 이상)
   * @param commentId 답글을 조회할 부모 댓글의 ID (1 이상)
   * @param pageable 페이지네이션 정보 (기본: 페이지 0, 크기 5)
   * @return 답글 목록이 포함된 성공 응답 객체
   */
  @Operation(summary = "해당 댓글 대한 답글 목록을 조회한다.", description = "해당 댓글 대한 답글 목록을 조회한다.")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
            description = "해당 댓글 대한 답글 목록 조회에 성공했습니다.",
            useReturnTypeSchema = true)
      })
