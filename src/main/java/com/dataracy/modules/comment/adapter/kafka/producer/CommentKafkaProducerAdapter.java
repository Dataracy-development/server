...
public class CommentKafkaProducerAdapter implements SendCommentEventPort {
    ...
    /**
         ...
         *
         * 이 메서드는 토픽 {@code topicUpload}에 키는 {@code String.valueOf(projectId)}, 값은 {@code projectId}로 메시지를 전송합니다.
    ...
    public void sendCommentUploadedEvent(Long projectId) {
        kafkaTemplate.send(topicUpload, String.valueOf(projectId), projectId)
                .whenComplete((result, ex) -> {
                    if (ex != null) {
                        LoggerFactory.kafka().logError(topicUpload, "댓글 작성 이벤트 발송 처리 실패: projectId=" + projectId, ex);
                        throw new CommonException(CommonErrorStatus.KAFKA_SEND_FAILURE);
                    } else {
                        LoggerFactory.kafka().logProduce(topicUpload, "댓글 작성 이벤트 발송됨: projectId=" + projectId);
                    }
                });
    }
     ...
     *
     * 비동기적으로 topicDelete 토픽에 메시지를 전송하며, Kafka 메시지의 키는 projectId의 문자열, 값은 projectId(Long)입니다.
    ...
    public void sendCommentDeletedEvent(Long projectId) {
        kafkaTemplate.send(topicDelete, String.valueOf(projectId), projectId)
                .whenComplete((result, ex) -> {
                    if (ex != null) {
                        LoggerFactory.kafka().logError(topicDelete, "댓글 삭제 이벤트 발송 처리 실패: projectId=" + projectId, ex);
                        throw new CommonException(CommonErrorStatus.KAFKA_SEND_FAILURE);
                    } else {
                        LoggerFactory.kafka().logProduce(topicDelete, "댓글 삭제 이벤트 발송됨: projectId=" + projectId);
                    }
                });
    }
}
