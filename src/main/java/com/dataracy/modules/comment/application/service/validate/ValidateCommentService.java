...
public class ValidateCommentService implements ValidateCommentUseCase {
     ...
     * 댓글이 존재하지 않을 경우 CommentException을 발생시킵니다.
     *
    ...
    public void validateComment(Long commentId) {
        Instant startTime = LoggerFactory.service().logStart(VALIDATE_COMMENT_USE_CASE, "주어진 댓글 ID에 해당하는 댓글 존재 서비스 시작 commentId=" + commentId);
        boolean isValidated = validateCommentPort.existsByCommentId(commentId);

        if (!isValidated) {
            LoggerFactory.service().logWarning(VALIDATE_COMMENT_USE_CASE, COMMENT_NOT_FOUND_MESSAGE + commentId);
            throw new CommentException(CommentErrorStatus.NOT_FOUND_COMMENT);
        }
        LoggerFactory.service().logSuccess(VALIDATE_COMMENT_USE_CASE, "주어진 댓글 ID에 해당하는 댓글 존재 서비스 종료 commentId=" + commentId, startTime);
    }
}
