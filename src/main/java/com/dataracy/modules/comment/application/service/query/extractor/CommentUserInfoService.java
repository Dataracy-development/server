...
public class CommentUserInfoService implements FindCommentUserInfoUseCase {
     ...
     * 
     * N+1 문제를 방지하기 위해 모든 사용자 정보를 배치로 조회합니다.
     * 
    ...
    public CommentLabelResponse findCommentUserInfoBatch(List<Long> userIds) {
        ...
        Map<Long, String> userThumbnailMap = findUserThumbnailUseCase.findUserThumbnailsByIds(distinctUserIds);
        Map<Long, String> userAuthorLevelIds = findUserAuthorLevelIdsUseCase.findUserAuthorLevelIds(distinctUserIds);
        ...
        CommentLabelResponse result = new CommentLabelResponse(
                usernameMap, 
                userThumbnailMap, 
                userAuthorLevelIds, 
