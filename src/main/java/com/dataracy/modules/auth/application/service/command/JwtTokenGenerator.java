...
public class JwtTokenGenerator {
  ...
  /** 공통 토큰 생성 로직 */
  private String generateTokenWithLogging(
      String tokenType, String identifier, TokenGenerationFunction generator) {
    try {
      Instant startTime =
          LoggerFactory.service()
              .logStart(
                  JWT_GENERATE_USE_CASE,
      ...
      LoggerFactory.service()
          .logSuccess(
              JWT_GENERATE_USE_CASE,
              String.format("%s 토큰 발급 서비스 성공 %s", tokenType, identifier),
              startTime);
      ...
      LoggerFactory.service()
          .logException(
              JWT_GENERATE_USE_CASE,
              String.format("[토큰 발급] %s 토큰 발급에 실패했습니다. %s", tokenType, identifier),
              e);
  ...
  public String generateRegisterToken(String provider, String providerId, String email) {
    return generateTokenWithLogging(
        "레지스터",
        "email=" + email,
  ...
  public String generateResetPasswordToken(String email) {
    return generateTokenWithLogging(
        "패스워드 재설정", "email=" + email, () -> jwtGeneratorPort.generateResetPasswordToken(email));
  }

  /** 액세스 토큰 생성 */
  public String generateAccessToken(Long userId, RoleType role) {
    return generateTokenWithLogging(
        "액세스", "userId=" + userId, () -> jwtGeneratorPort.generateAccessToken(userId, role));
  }

  /** 리프레시 토큰 생성 */
  public String generateRefreshToken(Long userId, RoleType role) {
    return generateTokenWithLogging(
        "리프레시", "userId=" + userId, () -> jwtGeneratorPort.generateRefreshToken(userId, role));
  }
  ...
  /** 토큰 생성 함수 인터페이스 RuntimeException을 사용하여 JWT 생성 실패를 표현한다. */
  @FunctionalInterface
