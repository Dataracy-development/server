...
public class JwtQueryService implements JwtValidateUseCase {
  ...
  public Long getUserIdFromToken(String token) {
    Instant startTime =
        LoggerFactory.service().logStart(JWT_VALIDATE_USE_CASE, "토큰으로부터 유저 아이디 추출 서비스 시작");
  ...
  public String getProviderFromRegisterToken(String token) {
    Instant startTime =
        LoggerFactory.service().logStart(JWT_VALIDATE_USE_CASE, "레지스터 토큰으로부터 소셜 제공자 추출 서비스 시작");
    String provider = jwtValidatorPort.getProviderFromRegisterToken(token);
    LoggerFactory.service()
        .logSuccess(JWT_VALIDATE_USE_CASE, "레지스터 토큰으로부터 소셜 제공자 추출 서비스 성공", startTime);
    return provider;
  }
  ...
  public String getProviderIdFromRegisterToken(String token) {
    Instant startTime =
        LoggerFactory.service().logStart(JWT_VALIDATE_USE_CASE, "레지스터 토큰으로부터 소셜 제공자 아이디 추출 서비스 시작");
    String providerId = jwtValidatorPort.getProviderIdFromRegisterToken(token);
    LoggerFactory.service()
        .logSuccess(JWT_VALIDATE_USE_CASE, "레지스터 토큰으로부터 소셜 제공자 아이디 추출 서비스 성공", startTime);
    return providerId;
  }
  ...
  public String getEmailFromRegisterToken(String token) {
    Instant startTime =
        LoggerFactory.service().logStart(JWT_VALIDATE_USE_CASE, "레지스터 토큰으로부터 이메일 추출 서비스 시작");
    String email = jwtValidatorPort.getEmailFromToken(token);
    LoggerFactory.service()
        .logSuccess(JWT_VALIDATE_USE_CASE, "레지스터 토큰으로부터 이메일 추출 서비스 성공", startTime);
    return email;
  }
  ...
  public String getEmailFromResetToken(String token) {
    Instant startTime =
        LoggerFactory.service().logStart(JWT_VALIDATE_USE_CASE, "비밀번호 재설정 토큰으로부터 이메일 추출 서비스 시작");
    String email = jwtValidatorPort.getEmailFromToken(token);
    LoggerFactory.service()
        .logSuccess(JWT_VALIDATE_USE_CASE, "비밀번호 재설정 토큰으로부터 이메일 추출 서비스 성공", startTime);
    return email;
  }
  ...
  public RoleType getRoleFromToken(String token) {
    Instant startTime =
        LoggerFactory.service().logStart(JWT_VALIDATE_USE_CASE, "토큰으로부터 유저 role 추출 서비스 시작");
    RoleType role = jwtValidatorPort.getRoleFromToken(token);
    LoggerFactory.service()
        .logSuccess(JWT_VALIDATE_USE_CASE, "토큰으로부터 유저 role 추출 서비스 성공", startTime);
    return role;
  }
  ...
  public long getRegisterTokenExpirationTime() {
    Instant startTime =
        LoggerFactory.service().logStart(JWT_VALIDATE_USE_CASE, "레지스터 토큰 유효기간 반환 서비스 시작");
  ...
  public long getAccessTokenExpirationTime() {
    Instant startTime =
        LoggerFactory.service().logStart(JWT_VALIDATE_USE_CASE, "어세스 토큰 유효기간 반환 서비스 시작");
  ...
  public long getRefreshTokenExpirationTime() {
    Instant startTime =
        LoggerFactory.service().logStart(JWT_VALIDATE_USE_CASE, "리프레시 토큰 유효기간 반환 서비스 시작");
  ...
  public String getRedirectOnboardingUrl() {
    Instant startTime =
        LoggerFactory.service()
            .logStart(JWT_VALIDATE_USE_CASE, "소셜 회원가입 시 추가정보 입력을 위한 온보딩 url 반환 서비스 시작");
    String onboardingUrl = jwtProperties.getRedirectOnboarding();
    LoggerFactory.service()
        .logSuccess(JWT_VALIDATE_USE_CASE, "소셜 회원가입 시 추가정보 입력을 위한 온보딩 url 반환 서비스 성공", startTime);
  ...
  public String getRedirectBaseUrl() {
    Instant startTime =
        LoggerFactory.service()
            .logStart(JWT_VALIDATE_USE_CASE, "로그인, 회원가입 완료 후 이동하는 메인 url 반환 서비스 시작");
    String baseUrl = jwtProperties.getRedirectBase();
    LoggerFactory.service()
        .logSuccess(JWT_VALIDATE_USE_CASE, "로그인, 회원가입 완료 후 이동하는 메인 url 반환 서비스 성공", startTime);
    return baseUrl;
  }
}
