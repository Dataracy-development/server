...
public class ManageRefreshTokenService implements ManageRefreshTokenUseCase {
  ...
  public void saveRefreshToken(String userId, String refreshToken) {
    Instant startTime =
        LoggerFactory.service().logStart(USE_CASE, "리프레시 토큰 레디스 저장 서비스 시작 userId=" + userId);
    manageRefreshTokenPort.saveRefreshToken(userId, refreshToken);
    LoggerFactory.service()
        .logSuccess(USE_CASE, "리프레시 토큰 레디스 저장 서비스 성공 userId=" + userId, startTime);
  }
  ...
  public String getRefreshToken(String userId) {
    Instant startTime =
        LoggerFactory.service().logStart(USE_CASE, "레디스에서 리프레시 토큰 추출 서비스 시작 userId=" + userId);
    ...
    LoggerFactory.service()
        .logSuccess(USE_CASE, "레디스에서 리프레시 토큰 추출 서비스 성공 userId=" + userId, startTime);
    return refreshToken;
  }
  ...
  public void deleteRefreshToken(String userId) {
    Instant startTime =
        LoggerFactory.service().logStart(USE_CASE, "레디스에서 리프레시 토큰 삭제 서비스 시작 userId=" + userId);
    manageRefreshTokenPort.deleteRefreshToken(userId);
    LoggerFactory.service()
        .logSuccess(USE_CASE, "레디스에서 리프레시 토큰 삭제 서비스 성공 userId=" + userId, startTime);
  }
}
