...
public class OAuth2LoginSuccessHandler extends SimpleUrlAuthenticationSuccessHandler {
  ...
  /**
   * OAuth2 로그인 성공 후 처리. 쿠키 설정 등 웹 관련은 컨트롤러 또는 핸들러에서 담당한다.
   *
  ...
  private void handleNewUser(
      OAuthUserInfo oAuthUserInfo, HttpServletRequest request, HttpServletResponse response)
      throws IOException {
    ...
    cookieUtil.setCookie(
        request, response, "registerToken", registerTokenResponse.registerToken(), maxAge);
  ...
  private void handleExistingUser(
    ...
    RefreshTokenResponse refreshTokenResponseDto =
        handleUserUseCase.handleExistingUser(oAuthUserInfo);
    ...
    // 리프레시 토큰을 쿠키에 저장한다.
    cookieUtil.setCookie(
        request, response, "refreshToken", refreshTokenResponseDto.refreshToken(), maxAge);
    // 메인페이지로 리다이렉션
