...
public enum AuthErrorStatus implements BaseErrorCode {
  ...
  BAD_REQUEST_TOKEN_TYPE(
      HttpStatus.BAD_REQUEST, "AUTH-003", "잘못된 토큰 유형입니다. REGISTER, ACCESS, REFRESH 중 하나여야 합니다."),
  ...
  EXPIRED_REGISTER_TOKEN(
      HttpStatus.UNAUTHORIZED, "AUTH-008", "회원가입을 위한 유효시간이 만료되었습니다. 다시 소셜 로그인을 시도해주세요."),
  ...
  FAILED_GENERATE_REGISTER_TOKEN(
      HttpStatus.INTERNAL_SERVER_ERROR, "AUTH-018", "레지스터 토큰 생성에 실패했습니다."),
  FAILED_GENERATE_RESET_PASSWORD_TOKEN(
      HttpStatus.INTERNAL_SERVER_ERROR, "AUTH-019", "비밀번호 재설정 토큰 생성에 실패했습니다."),
  SHORT_JWT_SECRET(HttpStatus.INTERNAL_SERVER_ERROR, "AUTH-020", "JWT Secret 키는 최소 32자 이상이어야 합니다."),

  // 소셜 타입 오류
  NOT_SUPPORTED_SOCIAL_PROVIDER_TYPE(
      HttpStatus.BAD_REQUEST, "AUTH-021", "지원되지 않는 소셜 로그인 유형입니다. (구글, 카카오만 가능)"),

  // 레이트 리미팅 오류
  RATE_LIMIT_EXCEEDED(HttpStatus.TOO_MANY_REQUESTS, "AUTH-022", "요청 횟수가 초과되었습니다. 잠시 후 다시 시도해주세요.");
