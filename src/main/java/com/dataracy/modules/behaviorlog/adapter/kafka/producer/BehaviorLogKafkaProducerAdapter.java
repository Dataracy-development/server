...
public class BehaviorLogKafkaProducerAdapter implements BehaviorLogSendProducerPort {
   ...
   *
   * <p>behaviorLog의 userId가 존재하면 이를 메시지 키로 사용하고, 없을 경우 anonymousId를 키로 사용합니다. 두 값 모두 null인 경우 로그를
   * 남기고 전송을 수행하지 않습니다.
   */
  @Override
  public void send(BehaviorLog behaviorLog) {
    String key =
        behaviorLog.getUserId() != null ? behaviorLog.getUserId() : behaviorLog.getAnonymousId();
    ...
    kafkaTemplate
        .send(topic, key, behaviorLog)
        .whenComplete(
            (result, ex) -> {
              if (ex != null) {
                log.error(
                    "Kafka 메시지 전송 실패 - topic={}, key={}, error={}",
                    topic,
                    key,
                    ex.getMessage(),
                    ex);
                ...
                log.trace(
                    "Kafka 메시지 전송 성공 - topic={}, partition={}, offset={}, key={}",
