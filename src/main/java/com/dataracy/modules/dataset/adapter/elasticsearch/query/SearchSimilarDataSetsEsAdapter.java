...
public class SearchSimilarDataSetsEsAdapter implements SearchSimilarDataSetsPort {
    ...
    try {
      Instant startTime =
          LoggerFactory.elastic()
              .logQueryStart(INDEX, "유사 데이터셋 검색 시작: dataId=" + data.getId() + ", size=" + size);
      ...
      List<SimilarDataResponse> similarDataResponses =
          response.hits().hits().stream().map(hit -> mapToSimilarResponse(hit.source())).toList();
      LoggerFactory.elastic()
          .logQueryEnd(
              INDEX, "유사 데이터셋 검색 종료: dataId=" + data.getId() + ", size=" + size, startTime);
      return similarDataResponses;
    } catch (IOException e) {
      LoggerFactory.elastic()
          .logError(INDEX, "유사 데이터셋 조회 실패: dataId=" + data.getId() + ", size=" + size, e);
   ...
   *
   * <p>이 쿼리는 제목과 설명을 기반으로 한 more_like_this 조건, topicId에 대한 가중치(term) 조건, 동일한 id의 문서 제외, 그리고 삭제되지
   * 않은(isDeleted=false) 문서만을 대상으로 하는 필터를 포함합니다.
   *
   * @param q 쿼리 빌더
                        ...
                        sb.moreLikeThis(
                            mlt ->
                                mlt.fields("title", "content")
                                    ...
                                    .minTermFreq(1)
                                    .minDocFreq(1)))
                .should(sb -> sb.term(t -> t.field("topicId").value(data.getTopicId()).boost(1.5f)))
                .mustNot(mn -> mn.term(t -> t.field("id").value(data.getId())))
                .filter(f -> f.term(t -> t.field("isDeleted").value(false))));
  }

  /**
   * DataSearchDocument를 SimilarDataResponse로 변환한다.
   *
   * <p>전달된 검색 문서의 필드를 대응하는 SimilarDataResponse 생성자 인자로 매핑해 새 응답 객체를 반환한다. 반환 객체에는 id, title,
   * userId, username, userProfileImageUrl, topicLabel, dataSourceLabel, dataTypeLabel, startDate,
   * endDate, description, dataThumbnailUrl, downloadCount, sizeBytes, rowCount, columnCount,
   * createdAt 등이 포함된다.
   *
