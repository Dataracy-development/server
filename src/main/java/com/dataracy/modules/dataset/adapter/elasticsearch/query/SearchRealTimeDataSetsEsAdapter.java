...
public class SearchRealTimeDataSetsEsAdapter implements SearchRealTimeDataSetsPort {
    ...
    /**
         ...
         *
         * 검색은 "title"(가중치 2)과 "description" 필드에 대해 자동 퍼지 멀티매치 쿼리로 수행되며,
         * "isDeleted"가 false인 문서만 포함됩니다. 결과는 생성일(createdAt) 기준 내림차순으로 정렬되고 요청한 개수로 제한됩니다.
         * 반환되는 각 RecentMinimalDataResponse에는 데이터셋의 id, title, 작성자(userId, username), 썸네일 URL 및 생성일이 포함됩니다.
         *
    ...
    public List<RecentMinimalDataResponse> searchRealTimeDataSets(String keyword, int size) {
        try {
            Instant startTime = LoggerFactory.elastic().logQueryStart(INDEX, "데이터셋 검색어 자동완성 실시간 검색 시작: keyword=" + keyword + ", size=" + size);
            ...
            LoggerFactory.elastic().logError(INDEX, "데이터셋 실시간 검색 실패: keyword=" + keyword + ", size=" + size, e);
