...
public interface DataSearchApi {
   ...
   */
  @Operation(summary = "유사한 데이터셋을 조회한다.", description = "제공받은 데이터와 유사한 데이터셋을 조회한다.")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
            description = "유사 데이터셋 조회에 성공했습니다.",
   ...
   */
  @Operation(summary = "필터링된 데이터셋 리스트를 조회한다.", description = "필터링된 데이터셋 리스트를 조회한다.")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
            description = "필터링된 데이터셋 리스트 조회에 성공했습니다.",
            useReturnTypeSchema = true)
      })
  @GetMapping("/filter")
  ResponseEntity<SuccessResponse<Page<FilteredDataWebResponse>>> searchFilteredDataSets(
      @Validated @ModelAttribute FilteringDataWebRequest webRequest,
   ...
   */
  @Operation(summary = "키워드 자동완성을 위한 데이터셋 검색 기능", description = "데이터셋 자동완성 검색 기능으로 데이터셋을 조회한다.")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
