...
public interface DataEsProjectionTaskRepository extends JpaRepository<DataEsProjectionTaskEntity, Long> {
     ...
     *
     * 조회된 행에는 PESSIMISTIC_WRITE 락이 적용되며 락이 걸린 행은 SKIP LOCKED 동작으로 건너뜁니다.
     * 주어진 상태 목록에 속하고 `now` 시각 이전 또는 같은 `nextRunAt`을 가진 엔티티를
     * `nextRunAt` 오름차순, 동일한 경우 `id` 오름차순으로 결정론적으로 정렬해 반환합니다.
     * 결과는 전달된 Pageable에 따라 페이징(제한/오프셋 및 정렬 적용 가능)됩니다.
     *
