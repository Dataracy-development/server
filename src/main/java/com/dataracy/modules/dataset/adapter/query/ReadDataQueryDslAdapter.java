...
public class ReadDataQueryDslAdapter
    implements FindDataPort,
   ...
   *
   * <p>데이터가 존재하면 도메인 객체를 담은 Optional을, 없으면 빈 Optional을 반환합니다.
   *
  ...
  public Optional<Data> findDataById(Long dataId) {
    Instant startTime =
        LoggerFactory.query()
            .logQueryStart(DATA_ENTITY, "[findDataById] 아이디를 통해 데이터셋 조회 시작. dataId=" + dataId);
    DataEntity entity =
        queryFactory
            .selectFrom(data)
            .where(DataFilterPredicate.notDeleted(), DataFilterPredicate.dataIdEq(dataId))
            .fetchOne();
    Optional<Data> result = Optional.ofNullable(entity).map(DataEntityMapper::toDomain);
    LoggerFactory.query()
        .logQueryEnd(DATA_ENTITY, "[findDataById] 아이디를 통해 데이터셋 조회 완료. dataId=" + dataId, startTime);
    return result;
  }
  ...
  public Optional<Data> findDataWithMetadataById(Long dataId) {
    Instant startTime =
        LoggerFactory.query()
            .logQueryStart(
                DATA_ENTITY,
                "[findDataWithMetadataById] 아이디를 통해 삭제된 데이터셋을 포함한 데이터셋 조회 시작. dataId=" + dataId);
    DataEntity entity =
        queryFactory
            .selectFrom(data)
            .leftJoin(data.metadata)
            .fetchJoin()
            .where(DataFilterPredicate.notDeleted(), DataFilterPredicate.dataIdEq(dataId))
            .fetchOne();
    Optional<Data> result = Optional.ofNullable(entity).map(DataEntityMapper::toDomain);
    LoggerFactory.query()
        .logQueryEnd(
            DATA_ENTITY,
            "[findDataWithMetadataById] 아이디를 통해 삭제된 데이터셋을 포함한 데이터셋 조회 완료. dataId=" + dataId,
            startTime);
    return result;
  }
  ...
  public List<DataGroupCountResponse> getDataGroupCount() {
    Instant startTime =
        LoggerFactory.query().logQueryStart(DATA_ENTITY, "[getDataGroupCount] 토픽별 데이터셋 개수 조회 시작.");

    List<DataGroupCountResponse> dataGroupCountResponses =
        queryFactory
            .select(
                Projections.constructor(
                    DataGroupCountResponse.class, topic.id, topic.label, data.count()))
            .from(data)
            .join(topic)
            .on(data.topicId.eq(topic.id), data.isDeleted.isFalse())
            .groupBy(topic.id, topic.label)
            .fetch();

    LoggerFactory.query()
        .logQueryEnd(DATA_ENTITY, "[getDataGroupCount] 토픽별 데이터셋 개수 조회 완료.", startTime);
   ...
   *
   * <p>결과는 최신순으로 정렬되며 데이터의 메타정보는 fetchJoin으로 함께 로드됩니다. 각 항목에는 도메인 Data와 해당 데이터에 연결된 프로젝트의 개수가
   * 포함됩니다. 전체 집계(total)는 EXISTS 기반 필터로 계산됩니다.
   *
   ...
   */
  @Override
  public Page<DataWithProjectCountDto> findConnectedDataSetsAssociatedWithProject(
      Long projectId, Pageable pageable) {
    Instant startTime =
        LoggerFactory.query()
            .logQueryStart(
                DATA_ENTITY,
                "[getConnectedDataSetsAssociatedWithProject] 지정된 프로젝트에 연결된 데이터셋 목록 조회 시작. projectId="
                    + projectId);
    int queryCount = 0;
    ...
    List<DataEntity> dataEntities =
        queryFactory
            .selectFrom(data)
            .leftJoin(data.metadata)
            .fetchJoin() // 1:1이면 fetchJoin OK
            .where(
                JPAExpressions.selectOne()
                    .from(projectData)
                    .where(projectData.project.id.eq(projectId).and(projectData.dataId.eq(data.id)))
                    .exists())
    ...
    // 3단계: DTO 조합 (메모리에서 처리)
    List<DataWithProjectCountDto> contents =
        dataEntities.stream()
            .map(
                entity ->
                    new DataWithProjectCountDto(
    ...
    LoggerFactory.query()
        .logQueryEnd(
            DATA_ENTITY,
            "[getConnectedDataSetsAssociatedWithProject] 지정된 프로젝트에 연결된 데이터셋 목록 조회 완료. projectId="
                + projectId
                + ", queryCount="
                + queryCount,
            startTime);
   ...
   */
  @Override
  public List<DataWithProjectCountDto> findConnectedDataSetsAssociatedWithProjectByIds(
      List<Long> dataIds) {
    ...
    Instant startTime =
        LoggerFactory.query()
            .logQueryStart(
                DATA_ENTITY, "[getConnectedDataSetsAssociatedWithProjectByIds] dataIds=" + dataIds);
    int queryCount = 0;
    ...
    List<DataEntity> dataEntities =
        queryFactory
            ...
            .join(data.metadata)
            .fetchJoin() // 1:1
            .where(DataFilterPredicate.dataIdIn(dataIds))
            .orderBy(DataSortBuilder.fromSortOption(DataSortType.LATEST, null))
            .fetch();
    queryCount++; // 메인 쿼리
    ...
    LoggerFactory.query()
        .logQueryEnd(
            DATA_ENTITY,
            "[getConnectedDataSetsAssociatedWithProjectByIds] 완료 dataIds="
                + dataIds
                + ", queryCount="
                + queryCount,
            startTime);
  ...
  public List<Data> getRecentDataSets(int size) {
    Instant startTime =
        LoggerFactory.query()
            .logQueryStart(DATA_ENTITY, "[getRecentDataSets] 최신 데이터셋 목록 조회 시작. size=" + size);
    ...
    List<Data> dataSets = dataEntities.stream().map(DataEntityMapper::toDomain).toList();
    LoggerFactory.query()
        .logQueryEnd(DATA_ENTITY, "[getRecentDataSets] 최신 데이터셋 목록 조회 완료. size=" + size, startTime);
    return dataSets;
  }
   ...
   *
   * <p>각 결과는 도메인 Data와 그 데이터에 연결된 고유 프로젝트 수를 포함합니다. 결과는 계산된 인기 점수(데이터 메타·연결 프로젝트 수 기반) 내림차순으로 정렬되어
   * 반환됩니다. 메타데이터는 함께 로드됩니다.
   *
  ...
  public List<DataWithProjectCountDto> getPopularDataSets(int size) {
    Instant startTime =
        LoggerFactory.query()
            .logQueryStart(DATA_ENTITY, "[searchPopularDataSets] 인기있는 데이터셋 목록 조회 시작. size=" + size);
    int queryCount = 0;
        ...
        dataEntities.stream()
            ...
            .sorted(
                (a, b) -> {
                  ...
                  double scoreA =
                      a.data().getDownloadCount() * 2.0 + a.countConnectedProjects() * 1.5;
                  double scoreB =
                      b.data().getDownloadCount() * 2.0 + b.countConnectedProjects() * 1.5;
    ...
    LoggerFactory.query()
        .logQueryEnd(
            DATA_ENTITY,
            "[searchPopularDataSets] 인기있는 데이터셋 목록 조회 완료. size="
                + size
                + ", queryCount="
                + queryCount,
            startTime);
  ...
  public Page<DataWithProjectCountDto> findUserDataSets(Long userId, Pageable pageable) {
    ...
    Instant startTime =
        LoggerFactory.query()
            .logQueryStart(
                DATA_ENTITY, "[findUserDataSets] 회원이 업로드한 데이터셋 목록 조회 시작. userId=" + userId);
    int queryCount = 0;
    ...
    List<DataEntity> dataEntities =
        queryFactory
            ...
            .leftJoin(data.metadata)
            .fetchJoin() // 1:1이면 fetchJoin OK
            .where(DataFilterPredicate.notDeleted(), DataFilterPredicate.userIdEq(userId))
            .orderBy(DataSortBuilder.fromSortOption(DataSortType.LATEST, null))
    ...
    LoggerFactory.query()
        .logQueryEnd(
            DATA_ENTITY,
            "[findUserDataSets] 회원이 업로드한 데이터셋 목록 조회 완료. userId="
                + userId
                + ", queryCount="
                + queryCount,
            startTime);
