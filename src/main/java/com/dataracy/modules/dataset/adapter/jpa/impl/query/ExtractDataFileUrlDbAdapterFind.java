...
public class ExtractDataFileUrlDbAdapterFind
    implements ExtractDataOwnerPort, FindDownloadDataFileUrlPort {
  ...
  /**
   * 주어진 데이터 ID에 해당하는 데이터의 업로드 유저 ID를 반환합니다. 데이터가 존재하지 않을 경우 DataException이 발생합니다.
   *
  ...
  public Long findUserIdByDataId(Long dataId) {
    Instant startTime =
        LoggerFactory.db()
            .logQueryStart(DATA_ENTITY, "[findById] 데이터셋의 작성자 아이디 조회 시작 dataId=" + dataId);
    ...
    LoggerFactory.db()
        .logQueryEnd(DATA_ENTITY, "[findById] 데이터셋의 작성자 아이디 조회 종료 dataId=" + dataId, startTime);
  ...
  public Long findUserIdIncludingDeleted(Long dataId) {
    Instant startTime =
        LoggerFactory.db()
            .logQueryStart(
                DATA_ENTITY,
                "[findIncludingDeletedData] 삭제된 데이터셋을 포함하여 데이터셋의 작성자 아이디 조회 시작 dataId=" + dataId);
    DataEntity dataEntity =
        dataJpaRepository
    ...
    LoggerFactory.db()
        .logQueryEnd(
            DATA_ENTITY,
            "[findIncludingDeletedData] 삭제된 데이터셋을 포함하여 데이터셋의 작성자 아이디 조회 종료 dataId=" + dataId,
            startTime);
    return dataEntity.getUserId();
  }
  ...
  private Optional<String> findDataFileUrl(Long dataId, String logPrefix) {
    Instant startTime =
        LoggerFactory.db().logQueryStart(DATA_ENTITY, logPrefix + " 시작 dataId=" + dataId);
