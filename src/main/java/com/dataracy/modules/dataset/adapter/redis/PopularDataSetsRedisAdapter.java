...
public class PopularDataSetsRedisAdapter implements PopularDataSetsStoragePort {
  ...
  public Optional<List<PopularDataResponse>> getPopularDataSets() {
    Instant startTime =
        LoggerFactory.redis().logQueryStart(POPULAR_DATASETS_KEY, "인기 데이터셋 캐시 조회 시작");
    ...
    try {
      ...
      List<PopularDataResponse> popularDataSets =
          objectMapper.readValue(cachedData, new TypeReference<List<PopularDataResponse>>() {});
  ...
  public void setPopularDataSets(List<PopularDataResponse> popularDataSets) {
    try {
      ...
      LoggerFactory.redis()
          .logSaveOrUpdate(
              POPULAR_DATASETS_KEY, "인기 데이터셋 캐시 저장 성공 count=" + popularDataSets.size());
  ...
  public boolean hasValidData() {
    return redisTemplate.hasKey(POPULAR_DATASETS_KEY)
        && redisTemplate.hasKey(POPULAR_DATASETS_METADATA_KEY);
  }
}
