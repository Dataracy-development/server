...
public class DataEntity extends BaseTimeEntity {
  ...
  @Column private String dataFileUrl;
  @Column private String dataThumbnailUrl;

  // 캐싱 필드
  @Column private int downloadCount;
   ...
   * 엔티티를 논리적으로 삭제 처리합니다.
   *
   * <p>이 메서드는 isDeleted 플래그를 true로 설정하여 데이터가 삭제된 것으로 표시합니다. 실제 데이터베이스에서 삭제되지는 않으며, @SQLRestriction에
   * 의해 쿼리 결과에서 제외됩니다.
   */
  public void delete() {
    this.isDeleted = true;
   ...
   *
   * <p>새 URL이 null이거나 공백이면 DataException(DataErrorStatus.INVALID_FILE_URL)을 던집니다. 전달된 URL이 현재와 동일하면
   * 아무 작업도 하지 않습니다. 유효한 새 URL일 경우 dataFileUrl을 갱신하고 sizeBytes를 dataFileSize로 설정합니다.
   *
   ...
   *
   * <p>참고: 이 메서드는 14개의 파라미터를 가지지만, DataEntity가 복잡한 도메인 엔티티이고 9개 파일에서 사용 중이며 Builder 패턴을 내부적으로 사용하므로
   * 허용됩니다. 새로운 코드에서는 DataEntity.builder()를 직접 사용하는 것을 권장합니다.
   *
   * @param title 데이터셋 제목
   * @param topicId 주제 ID
   * @param userId 생성자 ID
