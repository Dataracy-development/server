...
public class DataReadWebMapper {
   ...
   *
   * <p>원본 DTO의 각 필드(id, title, username, userProfileImageUrl, userIntroductionText, authorLabel,
   * occupationLabel, topicLabel, dataSourceLabel, dataTypeLabel, startDate, endDate, description,
   * analysisGuide, dataThumbnailUrl, downloadCount, sizeBytes, rowCount, columnCount, previewJson,
   * createdAt)를 그대로 전달하여 웹 응답 객체를 생성합니다.
   *
  ...
  /**
   * DataGroupCountResponse 객체를 DataGroupCountWebResponse 객체로 변환합니다. 주어진 주제별 데이터 개수 정보를 웹 응답 객체로
   * 매핑합니다.
   *
  ...
  public DataGroupCountWebResponse toWebDto(DataGroupCountResponse responseDto) {
    return new DataGroupCountWebResponse(
        responseDto.topicId(), responseDto.topicLabel(), responseDto.count());
  }

  /**
   * ConnectedDataResponse를 ConnectedDataWebResponse로 변환합니다.
   *
   * <p>응답 DTO의 기본 데이터셋 정보(작성자 프로필 이미지 포함)와 연결된 프로젝트 수를 웹 응답 객체로 매핑하여 반환합니다.
   *
   ...
   *
   * <p>변환된 웹 응답에는 다음 필드가 포함됩니다: id, title, creatorId, creatorName, userProfileImageUrl,
   * dataThumbnailUrl, createdAt.
   *
   ...
   *
   * <p>다음 필드를 PopularDataWebResponse로 매핑하여 새 인스턴스를 반환합니다: id, title, creatorId, creatorName,
   * userProfileImageUrl, topicLabel, dataSourceLabel, dataTypeLabel, startDate, endDate,
   ...
   * @return 웹 응답용 UserDataWebResponse (id, title, topicLabel, dataTypeLabel, startDate, endDate,
   *     dataThumbnailUrl, downloadCount, sizeBytes, rowCount, columnCount, createdAt,
   *     countConnectedProjects 포함)
   */
