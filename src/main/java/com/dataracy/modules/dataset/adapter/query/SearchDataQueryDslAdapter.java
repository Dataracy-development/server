...
public class SearchDataQueryDslAdapter implements SearchFilteredDataSetsPort {
   ...
   *
   * <p>각 데이터셋에 연관된 프로젝트 개수를 포함하여, 필터링, 정렬, 페이징이 모두 적용된 결과를 반환합니다.
   *
   ...
   */
  @Override
  public Page<DataWithProjectCountDto> searchByFilters(
      FilteringDataRequest request, Pageable pageable, DataSortType sortType) {
    Instant startTime =
        LoggerFactory.query()
            .logQueryStart(
                DATA_ENTITY, "[searchByFilters] 필터링된 데이터셋 목록 조회 시작. keyword=" + request.keyword());
    int queryCount = 0;
    ...
    LoggerFactory.query()
        .logQueryEnd(
            DATA_ENTITY,
            "[searchByFilters] 필터링된 데이터셋 목록 조회 완료. keyword="
                + request.keyword()
                + ", queryCount="
                + queryCount
                + ", dataSize="
