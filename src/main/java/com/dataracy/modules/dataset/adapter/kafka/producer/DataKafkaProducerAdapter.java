...
public class DataKafkaProducerAdapter implements DataUploadEventPort {
  ...
  public void sendUploadEvent(Long dataId, String dataFileUrl, String originalFilename) {
    ...
    kafkaTemplate
        .send(dataUploadTopic, String.valueOf(dataId), event)
        .whenComplete(
            (result, ex) -> {
              if (ex != null) {
                LoggerFactory.kafka()
                    .logError(dataUploadTopic, "데이터셋 업로드 이벤트 발송 처리 실패: dataId=" + dataId, ex);
              } else {
                LoggerFactory.kafka()
                    .logProduce(dataUploadTopic, "데이터셋 업로드 이벤트 발송됨: dataId=" + dataId);
              }
            });
  }
   ...
   *
   * <p>토픽 이름이 비어 있을 경우 애플리케이션 실행을 중단하기 위해 IllegalStateException을 발생시킵니다.
   */
  @PostConstruct
