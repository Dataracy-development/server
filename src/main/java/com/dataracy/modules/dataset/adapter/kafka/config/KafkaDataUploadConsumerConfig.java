...
public class KafkaDataUploadConsumerConfig {
  ...
  /**
   * DataUploadEvent용 ConsumerFactory - 타입 헤더를 사용하지 않는 환경에서 안전하게 역직렬화되도록 기본 타입/신뢰 패키지 지정 -
  ...
  /**
   * 공통 에러 핸들러 (블로킹 리트라이 + DLT) - 1s → 2s → 4s → 8s → 16s, 최대 5회 - 포이즌/역직렬화류는 즉시 DLT
   *
  ...
  public DefaultErrorHandler dataUploadErrorHandler(
    ...
    DeadLetterPublishingRecoverer recoverer =
        new DeadLetterPublishingRecoverer(
            dltTemplate, (rec, ex) -> new TopicPartition(rec.topic() + "-dlt", rec.partition()));
  ...
  /**
   * 리스너 컨테이너 팩토리 - 공통 에러 핸들러 연결 → 모든 리스너에서 @RetryableTopic 없이 재시도/백오프/ DLT 적용 - 동시성은 트래픽에 따라 yml로
   * 조절
   */
