...
public interface DataJpaRepository extends JpaRepository<DataEntity, Long> {
  /**
   * 삭제된 데이터를 포함하여 지정된 dataId에 해당하는 DataEntity를 반환합니다. 네이티브 쿼리를 사용하여 soft delete 등 엔티티 레벨의 필터링을 무시하고
   * 데이터를 조회합니다.
   *
  ...
  /**
   * 지정된 데이터 ID에 해당하는 데이터 파일 URL을 반환합니다. 데이터의 삭제 상태와 무관하게 `data` 테이블에서 직접 조회하며, 해당 ID의 URL이 없으면 빈
   * Optional을 반환합니다.
   *
