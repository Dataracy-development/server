...
public interface FindConnectedDataSetsPort {
   ...
   *
   * <p>각 페이지 항목은 데이터셋 정보와 해당 데이터셋이 연결된 프로젝트 수를 포함한 DataWithProjectCountDto로 구성됩니다.
   *
   ...
   */
  Page<DataWithProjectCountDto> findConnectedDataSetsAssociatedWithProject(
      Long projectId, Pageable pageable);

  /**
   * 주어진 데이터셋 ID 목록에 대응하는 데이터셋과 각 데이터셋에 연결된 프로젝트 수를 조회하여 반환합니다.
   *
   * <p>입력한 ID들에 해당하는 데이터셋 각각에 대해 DataWithProjectCountDto(데이터셋 정보 + 연결된 프로젝트 수)를 포함한 리스트를 반환합니다. 요청한
   * ID 중 존재하지 않는 항목이 있으면 해당 항목은 결과에 포함되지 않을 수 있습니다.
   *
