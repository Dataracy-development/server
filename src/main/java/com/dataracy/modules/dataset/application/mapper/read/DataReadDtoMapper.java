...
public class DataReadDtoMapper {
  /**
   * Data 도메인 객체와 보조 정보(업로더 닉네임·프로필 이미지, 레이블, 연결된 프로젝트 수)를 결합하여 ConnectedDataResponse DTO 인스턴스를 생성하여
   * 반환합니다.
   *
   ...
   *
   * <p>반환되는 DTO는 데이터의 ID, 제목, 업로더 ID/닉네임/프로필 이미지 URL, 썸네일 URL, 생성일시만 포함합니다.
   *
   * @param data 변환할 Data 도메인 객체
   * @param username 업로더의 표시 이름(닉네임)
  ...
  public RecentMinimalDataResponse toResponseDto(
      Data data, String username, String userProfileImageUrl) {
   ...
   *
   * <p>전달된 Data 객체의 식별자·제목·기간·설명·썸네일·다운로드수·크기(bytes)·메타데이터(행/열 수)·생성일과 추가로 제공된 username,
   * topicLabel, dataSourceLabel, dataTypeLabel, 연결된 프로젝트 수를 포함한 DTO를 반환합니다.
   *
   * @param data 변환할 Data 도메인 객체
   ...
   *
   * <p>상세 응답에 필요한 필드(작성자 표시명·프로필·소개, 각종 라벨, 기간·설명·미리보기 등)를 Data 객체와 전달된 문자열들을 이용해 조합해 반환합니다.
   *
   * <p>참고: 이 메서드는 9개의 파라미터를 가지지만, 데이터 상세 조회 DTO가 복잡한 화면 정보를 포함하고 Mapper 레이어에서 도메인 객체와 여러 라벨 정보를
   * 조합하는 역할을 수행하므로 허용됩니다.
   *
   * @param data 도메인 엔티티
   ...
   *
   * <p>지정된 Data의 식별자·제목·기간·썸네일·다운로드/크기·메타데이터(행/열 수)·생성일자를 응답 DTO에 복사하고, 전달된 토픽·데이터 타입 라벨 및 프로젝트 연결
   * 수를 추가합니다.
   *
   * @param data 변환할 Data 도메인 객체
  ...
  public UserDataResponse toResponseDto(
      Data data, String topicLabel, String dataTypeLabel, Long countConnectedProjects) {
