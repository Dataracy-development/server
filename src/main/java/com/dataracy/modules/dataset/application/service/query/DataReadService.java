...
public class DataReadService
    implements GetPopularDataSetsUseCase,
  ...
  private static final String GET_RECENT_MINIMAL_DATA_SETS_USE_CASE =
      "GetRecentMinimalDataSetsUseCase";
  ...
  /**
   * 지정된 개수만큼 인기 데이터셋을 조회하여 사용자 닉네임, 프로필 썸네일 URL, 주제/데이터 소스/데이터 타입 라벨, 및 각 데이터셋의 연결된 프로젝트 수를 포함한 응답
   * 리스트를 반환합니다.
   *
  ...
  public List<PopularDataResponse> getPopularDataSets(int size) {
    Instant startTime =
        LoggerFactory.service()
            .logStart(GET_POPULAR_DATA_SETS_USE_CASE, "인기 데이터셋 목록 조회 서비스 시작 size=" + size);
    ...
    LoggerFactory.service()
        .logInfo(
            GET_POPULAR_DATA_SETS_USE_CASE,
            "저장소 캐시 조회 결과: " + (cachedResult.isPresent() ? "데이터 존재" : "데이터 없음"));
    if (cachedResult.isPresent()) {
      List<PopularDataResponse> cachedData = cachedResult.get();
      List<PopularDataResponse> result = cachedData.stream().limit(size).toList();

      LoggerFactory.service()
          .logSuccess(
              GET_POPULAR_DATA_SETS_USE_CASE,
              "인기 데이터셋 저장소 조회 성공 size=" + size + " cachedCount=" + cachedData.size(),
              startTime);
      return result;
    }
    ...
    LoggerFactory.service()
        .logInfo(GET_POPULAR_DATA_SETS_USE_CASE, "저장소에 데이터가 없어 DB에서 조회합니다. size=" + size);
    ...
    List<PopularDataResponse> popularDataResponses =
    ...
    LoggerFactory.service()
        .logSuccess(GET_POPULAR_DATA_SETS_USE_CASE, "인기 데이터셋 DB 조회 서비스 종료 size=" + size, startTime);
    return popularDataResponses;
  }

  /**
   * 주어진 데이터 ID에 해당하는 데이터셋의 상세 정보를 반환합니다. 데이터셋의 기본 정보, 작성자 닉네임, 작성자 등급 및 직업 라벨, 주제/데이터 소스/데이터 타입 라벨,
   * 기간, 설명, 분석 가이드, 썸네일 URL, 다운로드 수, 메타데이터(행/열 개수, 미리보기 JSON), 생성일시를 포함한 상세 정보를 제공합니다.
   *
  ...
  public DataDetailResponse getDataDetail(Long dataId) {
    Instant startTime =
        LoggerFactory.service()
            .logStart(GET_DATA_DETAIL_USE_CASE, "데이터셋 상세 정보 조회 서비스 시작 dataId=" + dataId);

    Data data =
        findDataWithMetadataPort
            .findDataWithMetadataById(dataId)
            .orElseThrow(
                () -> {
                  LoggerFactory.service()
                      .logWarning(GET_DATA_DETAIL_USE_CASE, DATA_NOT_FOUND_MESSAGE + dataId);
    ...
    String authorLabel =
        dataUser.authorLevelId() == null
            ? null
            : getAuthorLevelLabelFromIdUseCase.getLabelById(dataUser.authorLevelId());
    String occupationLabel =
        dataUser.occupationId() == null
            ? null
            : getOccupationLabelFromIdUseCase.getLabelById(dataUser.occupationId());

    DataDetailResponse dataDetailResponse =
        dataReadDtoMapper.toResponseDto(
            data,
            dataUser.nickname(),
    ...
    LoggerFactory.service()
        .logSuccess(GET_DATA_DETAIL_USE_CASE, "데이터셋 상세 정보 조회 서비스 종료 dataId=" + dataId, startTime);
   ...
   *
   * <p>조회된 각 데이터는 사용자 ID로부터 닉네임과 썸네일 URL을 조회하여 DTO로 매핑됩니다.
   *
  ...
  public List<RecentMinimalDataResponse> getRecentDataSets(int size) {
    Instant startTime =
        LoggerFactory.service()
            .logStart(
                GET_RECENT_MINIMAL_DATA_SETS_USE_CASE, "최신 미니 데이터셋 목록 조회 서비스 시작 size=" + size);

    List<Data> recentDataSets = getRecentDataSetsPort.getRecentDataSets(size);
    List<Long> userIds = recentDataSets.stream().map(Data::getUserId).toList();
    ...
    LoggerFactory.service()
        .logSuccess(
            GET_RECENT_MINIMAL_DATA_SETS_USE_CASE,
            "최신 미니 데이터셋 목록 조회 서비스 종료 size=" + size,
            startTime);
  ...
  public List<DataGroupCountResponse> getDataGroupCountByTopicLabel() {
    Instant startTime =
        LoggerFactory.service()
            .logStart(
                GET_DATA_GROUP_COUNT_USE_CASE, "데이터셋을 주제별로 그룹화하여 각 주제에 속한 데이터셋의 개수를 반환 서비스 시작");
    List<DataGroupCountResponse> dataGroupCountResponses =
        getDataGroupCountPort.getDataGroupCount();
    LoggerFactory.service()
        .logSuccess(
            GET_DATA_GROUP_COUNT_USE_CASE,
            "데이터셋을 주제별로 그룹화하여 각 주제에 속한 데이터셋의 개수를 반환 서비스 종료",
            startTime);
    return dataGroupCountResponses;
  }

  /**
   * 지정된 프로젝트와 연결된 데이터셋들을 페이지 단위로 조회하여 라벨(작성자 닉네임, 프로필 URL, 주제, 데이터 타입)과 각 데이터셋의 연결된 프로젝트 수를 포함한
   * ConnectedDataResponse 페이지로 반환합니다.
   *
  ...
  @Override
  @Transactional(readOnly = true)
  public Page<ConnectedDataResponse> findConnectedDataSetsAssociatedWithProject(
      Long projectId, Pageable pageable) {
    Instant startTime =
        LoggerFactory.service()
            .logStart(
                FIND_CONNECTED_DATA_SETS_USE_CASE,
                "프로젝트와 연결된 데이터셋 목록 조회 서비스 시작 projectId=" + projectId);

    Page<DataWithProjectCountDto> savedDataSets =
        findConnectedDataSetsPort.findConnectedDataSetsAssociatedWithProject(projectId, pageable);
    DataLabelMapResponse labelResponse =
        findDataLabelMapUseCase.labelMapping(savedDataSets.getContent());
    Page<ConnectedDataResponse> connectedDataResponses =
        savedDataSets.map(
            wrapper -> {
              Data data = wrapper.data();
              return dataReadDtoMapper.toResponseDto(
    ...
    LoggerFactory.service()
        .logSuccess(
            FIND_CONNECTED_DATA_SETS_USE_CASE,
            "프로젝트와 연결된 데이터셋 목록 조회 서비스 종료 projectId=" + projectId,
            startTime);
   ...
   *
   * <p>주어진 ID 목록이 null이거나 비어 있으면 빈 리스트를 반환하며, 내부적으로 중복 ID는 제거됩니다. 반환되는 각 항목은 데이터 엔티티와 함께 작성자명/프로필
   * URL, 토픽 라벨, 데이터 타입 라벨 및 연결된 프로젝트 수를 포함한 ConnectedDataResponse로 매핑됩니다.
   *
  ...
  public List<ConnectedDataResponse> findDataSetsByIds(List<Long> dataIds) {
    Instant startTime =
        LoggerFactory.service()
            .logStart(
                FIND_CONNECTED_DATA_SETS_USE_CASE,
                "아이디 목록을 통한 데이터셋 목록 조회 서비스 시작 dataIds=" + dataIds);

    if (dataIds == null || dataIds.isEmpty()) {
      LoggerFactory.service()
          .logSuccess(FIND_CONNECTED_DATA_SETS_USE_CASE, "빈 ID 목록으로 빈 결과 반환", startTime);
      return List.of();
    }
    List<Long> distinctIds = dataIds.stream().distinct().toList();
    List<DataWithProjectCountDto> savedDataSets =
        findConnectedDataSetsPort.findConnectedDataSetsAssociatedWithProjectByIds(distinctIds);

    DataLabelMapResponse labelResponse = findDataLabelMapUseCase.labelMapping(savedDataSets);
    List<ConnectedDataResponse> connectedDataResponses =
        savedDataSets.stream()
            .map(
                wrapper -> {
                  Data data = wrapper.data();
    ...
    LoggerFactory.service()
        .logSuccess(
            FIND_CONNECTED_DATA_SETS_USE_CASE,
            "아이디 목록을 통한 데이터셋 목록 조회 서비스 종료 dataIds=" + dataIds,
            startTime);
    return connectedDataResponses;
  }
}
