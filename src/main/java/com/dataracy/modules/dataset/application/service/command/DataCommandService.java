...
public class DataCommandService implements UploadDataUseCase, ModifyDataUseCase {
  ...
  private static final String DATA_NOT_FOUND_AFTER_FILE_UPLOAD_MESSAGE =
      "파일 업로드 후 데이터를 찾을 수 없습니다. dataId=";
   ...
   *
   * <p>요청한 메타데이터를 저장하고(트랜잭션 내), 파일을 스토리지에 업로드하여 저장된 데이터셋의 파일 URL을 갱신합니다. 데이터 파일이 존재하면 업로드 완료 이벤트를
   * 발행합니다.
   *
  ...
  @Override
  @Transactional
  public UploadDataResponse uploadData(
      Long userId,
    ...
    Instant startTime =
        LoggerFactory.service()
            .logStart(UPLOAD_DATA_USE_CASE, "데이터셋 업로드 서비스 시작 title=" + requestDto.title());
    ...
    // 데이터셋 저장
    Data data = createDataDtoMapper.toDomain(requestDto, userId);
    ...
    Data updatedFileUrlData =
        findDataPort
            .findDataById(saveData.getId())
            .orElseThrow(
                () -> {
                  LoggerFactory.service()
                      .logWarning(
                          UPLOAD_DATA_USE_CASE,
                          DATA_NOT_FOUND_AFTER_FILE_UPLOAD_MESSAGE + saveData.getId());
    ...
    LoggerFactory.service()
        .logSuccess(UPLOAD_DATA_USE_CASE, "데이터셋 업로드 서비스 종료 title=" + requestDto.title(), startTime);
   ...
   *
   * <p>데이터셋의 제목, 설명, 날짜 범위, 주제, 데이터 소스, 데이터 타입 등 메타데이터를 갱신하고, 새로운 데이터셋 파일 또는 썸네일 파일이 제공된 경우 파일을
   * 업로드하여 해당 URL로 갱신합니다. 데이터셋이 존재하지 않으면 예외를 발생시킵니다. 파일 업로드 실패 시 트랜잭션이 롤백되며, 데이터셋 파일이 수정된 경우 업로드
   * 이벤트가 발행됩니다.
   *
  ...
  public void modifyData(
    ...
    Instant startTime =
        LoggerFactory.service().logStart(MODIFY_DATA_USE_CASE, "데이터셋 수정 서비스 시작 dataId=" + dataId);
    ...
    Data savedData =
        findDataPort
            ...
            .orElseThrow(
                () -> {
                  LoggerFactory.service()
                      .logWarning(MODIFY_DATA_USE_CASE, DATA_NOT_FOUND_MESSAGE + dataId);
                  return new DataException(DataErrorStatus.NOT_FOUND_DATA);
                });
    ...
    if (dataFile != null && !dataFile.isEmpty()) {
      dataUploadEventPort.sendUploadEvent(
          savedData.getId(), savedData.getDataFileUrl(), dataFile.getOriginalFilename());
    }

    LoggerFactory.service()
        .logSuccess(MODIFY_DATA_USE_CASE, "데이터셋 수정 서비스 종료 dataId=" + dataId, startTime);
  }

  /**
   * 데이터셋 파일, 썸네일 파일, 날짜, 주제/데이터소스/데이터타입 ID의 유효성을 검증합니다. 시작일이 종료일보다 늦을 경우 예외를 발생시키며, 파일 형식 및 각 ID의
   * 존재 여부를 확인합니다.
   *
   * <p>참고: 8개의 파라미터를 가지지만, 데이터 업로드/수정 시 필요한 모든 검증을 수행하는 통합 검증 메서드입니다.
   *
  ...
  private void validateDataRequest(
    ...
    if (startDate.isAfter(endDate)) {
      LoggerFactory.service()
          .logWarning(useCase, "시작일이 종료일보다 늦습니다. startDate=" + startDate + ", endDate=" + endDate);
   ...
   *
   * <p>업로드 중 오류가 발생하면 로그를 남기고 RuntimeException을 던져 트랜잭션을 롤백합니다.
   *
  ...
  private void dataFileUpload(MultipartFile dataFile, Long dataId, String useCase) {
    ...
    if (dataFile != null && !dataFile.isEmpty()) {
      try {
        ...
        LoggerFactory.service()
            .logException(useCase, "데이터셋 파일 업로드 실패. fileName=" + dataFile.getOriginalFilename(), e);
  ...
  private void thumbnailFileUpload(MultipartFile thumbnailFile, Long dataId, String useCase) {
    ...
    if (thumbnailFile != null && !thumbnailFile.isEmpty()) {
      try {
        String key =
            S3KeyGeneratorUtil.generateThumbnailKey(
                "data", dataId, thumbnailFile.getOriginalFilename());
        ...
        LoggerFactory.service()
            .logException(
                useCase, "데이터셋 썸네일 파일 업로드 실패. fileName=" + thumbnailFile.getOriginalFilename(), e);
