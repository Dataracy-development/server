...
public class DataValidateService implements ValidateDataUseCase {
  ...
  /**
   * 주어진 데이터 ID에 해당하는 데이터가 존재하는지 검증합니다. 데이터가 존재하지 않을 경우 DataException을 NOT_FOUND_DATA 상태로 발생시킵니다.
   *
  ...
  public void validateData(Long dataId) {
    Instant startTime =
        LoggerFactory.service()
            .logStart(
                VALIDATE_DATA_USE_CASE, "주어진 데이터 ID에 해당하는 데이터의 존재 여부를 검증 서비스 시작 dataId=" + dataId);
    ...
    LoggerFactory.service()
        .logSuccess(
            VALIDATE_DATA_USE_CASE,
            "주어진 데이터 ID에 해당하는 데이터의 존재 여부를 검증 서비스 종료 dataId=" + dataId,
            startTime);
  }
}
