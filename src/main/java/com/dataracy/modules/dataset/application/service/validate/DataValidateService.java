...
public class DataValidateService implements ValidateDataUseCase {
    ...
    // Use Case 상수 정의
    private static final String VALIDATE_DATA_USE_CASE = "ValidateDataUseCase";
    ...
    public void validateData(Long dataId) {
        Instant startTime = LoggerFactory.service().logStart(VALIDATE_DATA_USE_CASE, "주어진 데이터 ID에 해당하는 데이터의 존재 여부를 검증 서비스 시작 dataId=" + dataId);
        boolean isExist = checkDataExistsByIdPort.existsDataById(dataId);
        if (!isExist) {
            LoggerFactory.service().logWarning(VALIDATE_DATA_USE_CASE, DATA_NOT_FOUND_MESSAGE + dataId);
            throw new DataException(DataErrorStatus.NOT_FOUND_DATA);
        }
        LoggerFactory.service().logSuccess(VALIDATE_DATA_USE_CASE, "주어진 데이터 ID에 해당하는 데이터의 존재 여부를 검증 서비스 종료 dataId=" + dataId, startTime);
    }
}
