...
public class EmailCommandService implements SendEmailUseCase {
   ...
   *
   * <p>생성된 인스턴스는 주입된 포트를 사용해 이메일 전송 및 이메일 인증 코드 관리를 수행합니다.
   */
   ...
   *
   * <p>이 메서드는 인증 코드를 생성한 뒤 전송을 시도하고, 전송에 실패하면 EmailException을 던집니다. 전송이 성공하면 이메일과 목적(type)에 맞춰 생성한
   * 코드를 영속/캐시 계층에 저장합니다.
   *
  ...
  public void sendEmailVerificationCode(String email, EmailVerificationType type) {
    Instant startTime =
        LoggerFactory.service().logStart(SEND_EMAIL_USE_CASE, "이메일 인증 코드 전송 서비스 시작 email=" + email);
    ...
    LoggerFactory.service()
        .logSuccess(SEND_EMAIL_USE_CASE, "이메일 인증 코드 전송 서비스 종료 email=" + email, startTime);
  }
