...
public class SesConfig {
     ...
     *
     * AWS 자격 증명은 다음 순서로 시도됩니다:
     * 1. 설정된 access-key, secret-key가 있으면 AWSStaticCredentialsProvider 사용
     * 2. 그렇지 않으면 DefaultAWSCredentialsProviderChain 사용 (환경변수, IAM 역할 등)
     * 
     * 보안상 권장사항:
     * - 프로덕션 환경에서는 IAM 역할이나 환경변수 사용 권장
     * - 하드코딩된 자격 증명은 보안 위험을 초래할 수 있음
     *
    ...
    public AmazonSimpleEmailService amazonSimpleEmailService() {
        try {
            ...
            if (!accessKey.isBlank() && !secretKey.isBlank()) {
                LoggerFactory.common().logWarning("AWS SES 설정", "하드코딩된 AWS 자격 증명 사용 중 - 프로덕션 환경에서는 IAM 역할 사용 권장");
