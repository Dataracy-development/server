...
public final class UserEntityMapper {
  ...
  /**
   * UserEntity 객체를 User 도메인 모델로 변환합니다. 입력된 UserEntity가 null이면 null을 반환하며, 연관된 토픽 ID 목록과 소개글 등 모든 주요
   * 필드를 User 도메인 모델에 매핑합니다.
   *
  ...
  public static User toDomain(UserEntity userEntity) {
    ...
    List<Long> topicIds =
        Optional.ofNullable(userEntity.getUserTopicEntities())
   ...
   *
   * <p>User의 필드와 토픽 ID 목록을 기반으로 UserEntity와 연관된 UserTopicEntity 목록을 생성하여 연결합니다. 입력이 null이면 null을
   * 반환합니다.
   *
  ...
  public static UserEntity toEntity(User user) {
    ...
    List<UserTopicEntity> userTopicEntities =
        Optional.ofNullable(user.getTopicIds()).orElseGet(Collections::emptyList).stream()
            .distinct()
