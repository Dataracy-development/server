...
import com.dataracy.modules.user.domain.enums.InterestDomainType;
...
public record SelfSignUpWebRequest(
        ...
        @ValidEnumValue(enumClass = AuthorLevelType.class, required = true, message = "작성자 유형은 초심자, 실무자, 전문가, GPT활용만 가능하다.")
        String authorLevel,

        @Schema(description = "직업", allowableValues = {"학생", "개발자", "기획자", "디자이너", "마케터", "기타"})
        @ValidEnumValue(enumClass = OccupationType.class, required = false, message = "직업은 학생, 개발자, 기획자, 디자이너, 마케너, 기타만 가능하다.")
        String occupation,

        @Schema(description = "흥미 도메인", example = "[\"프론트엔드\", \"디자인\"]",
                allowableValues = {"프론트엔드", "백엔드", "인공지능", "데이터분석", "보안", "디자인", "스타트업"})
        @ValidEnumValue(enumClass = InterestDomainType.class, required = false, message = "흥미 도메인은 프론트엔드, 백엔드, 인공지능, 데이터분석, 보안, 디자인, 스타트업이 가능하다.")
        List<String> topics,

        @Schema(description = "방문 경로", allowableValues = {"SNS", "검색엔진", "지인추천", "커뮤니티", "블로그", "광고", "기타"})
        @ValidEnumValue(enumClass = VisitSourceType.class, required = false, message = "방문 경로는 SNS, 검색엔진, 지인추천, 커뮤니티, 블로그, 광고, 기타만 가능하다.")
        String visitSource,
