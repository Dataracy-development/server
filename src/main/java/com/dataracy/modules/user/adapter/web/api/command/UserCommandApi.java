...
public interface UserCommandApi {
   ...
   * 회원 정보를 수정합니다.
   *
   * <p>요청 본문으로 회원 수정 정보(검증된 ModifyUserInfoWebRequest)와 선택적 프로필 이미지 파일을 받아 회원 정보를 업데이트하고 성공 시 200
   * OK와 SuccessResponse<Void>를 반환합니다.
   *
  ...
  ResponseEntity<SuccessResponse<Void>> modifyUserInfo(
      ...
      @RequestPart @Validated ModifyUserInfoWebRequest webRequest);

  /**
   * 회원을 소프트 탈퇴 처리합니다.
   *
   * <p>현재 인증된 사용자의 계정을 소프트 삭제(탈퇴)하고 성공 시 200 OK와 빈 SuccessResponse를 반환합니다.
   *
  ...
  /**
   * 현재 로그인한 사용자의 세션과 리프레시 토큰을 무효화하여 로그아웃을 수행합니다. 모든 인증 관련 쿠키(accessToken, refreshToken,
   * accessTokenExpiration, resetToken)를 삭제합니다.
   *
