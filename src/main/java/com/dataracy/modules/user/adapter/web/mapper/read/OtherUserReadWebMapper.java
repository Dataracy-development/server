...
public class OtherUserReadWebMapper {
     ...
     *
     * 변환된 웹 DTO에는 프로젝트 식별자(id), 제목, 내용, 썸네일 URL, 주제 레이블(topicLabel),
     * 작성자 레벨 레이블(authorLevelLabel), 댓글/좋아요/조회수, 생성일(createdAt) 등이 포함된다.
     *
     ...
     *
     * 요청된 데이터의 식별자, 제목, 토픽/데이터 타입 라벨, 기간(start/end), 썸네일 URL, 다운로드/크기/행·열 수,
     * 생성일(createdAt) 및 연관 프로젝트 수를 대응하는 웹 DTO 필드로 그대로 복사하여 새 웹 DTO를 생성해 반환한다.
     *
