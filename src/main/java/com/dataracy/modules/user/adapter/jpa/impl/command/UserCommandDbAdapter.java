...
public class UserCommandDbAdapter implements UserCommandPort {
     ...
     *
     * 사용자 엔티티가 존재하지 않으면 UserErrorStatus.NOT_FOUND_USER를 가진 UserException을 던집니다.
     *
     ...
     *
     * 요청한 사용자 ID의 엔티티를 조회하여 존재하지 않으면 UserException(UserErrorStatus.NOT_FOUND_USER)를 던진다.
     * 조회된 엔티티에 요청 DTO의 값으로 사용자 정보를 적용(modifyUserInfo)하고,
     * 기존에 저장된 모든 관심 주제 레코드를 삭제한 뒤 요청 DTO에 포함된 topicIds로 새 관심 주제를 생성·저장하고
     * 최종적으로 사용자 엔티티를 저장한다.
     *
