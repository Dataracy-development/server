...
public class OtherUserProfileService implements GetOtherUserInfoUseCase {
   ...
   *
   * <p>반환되는 응답에는 사용자의 ID, 닉네임, 프로필 이미지, 자기소개, authorLevel 및 occupation 라벨(해당 ID가 없으면 null), 그리고 대표
   * 프로젝트 목록과 데이터셋 목록이 포함된다.
   *
  ...
  public GetOtherUserInfoResponse getOtherUserInfo(Long userId) {
    Instant startTime =
        LoggerFactory.service()
            .logStart(
                GET_OTHER_USER_INFO_USE_CASE, "주어진 사용자 ID에 대한 타인 유저 정보 조회 서비스 시작 userId=" + userId);
        ...
        userQueryPort
            ...
            .orElseThrow(
                () -> {
                  LoggerFactory.service()
                      .logWarning(
                          GET_OTHER_USER_INFO_USE_CASE,
                          "[타인 유저 정보 조회] " + USER_NOT_FOUND_MESSAGE + userId);
    ...
    LoggerFactory.service()
        .logSuccess(
            GET_OTHER_USER_INFO_USE_CASE,
            "주어진 사용자 ID에 대한 타인 정보 조회 서비스 성공 userId=" + userId,
            startTime);
   ...
   *
   * <p>주어진 사용자 ID에 대해 페이지네이션된 프로젝트들을 조회하여 {@code GetOtherUserProjectResponse}로 매핑한 결과를 반환한다.
   *
  ...
  public Page<GetOtherUserProjectResponse> getOtherExtraProjects(Long userId, Pageable pageable) {
    Instant startTime =
        LoggerFactory.service()
            .logStart(
                GET_OTHER_USER_INFO_USE_CASE, "타인이 업로드한 프로젝트 목록 추가 조회 서비스 시작 userId=" + userId);
    Page<GetOtherUserProjectResponse> projects =
        findUserProjectsUseCase
            .findUserProjects(userId, pageable)
            .map(otherUserInfoMapper::toOtherUserProject);
    LoggerFactory.service()
        .logSuccess(
            GET_OTHER_USER_INFO_USE_CASE,
            "타인이 업로드한 프로젝트 목록 추가 조회 서비스 성공 userId=" + userId,
            startTime);
    return projects;
  }
  ...
  public Page<GetOtherUserDataResponse> getOtherExtraDataSets(Long userId, Pageable pageable) {
    Instant startTime =
        LoggerFactory.service()
            .logStart(
                GET_OTHER_USER_INFO_USE_CASE, "타인이 업로드한 데이터셋 목록 추가 조회 서비스 시작 userId=" + userId);
    Page<GetOtherUserDataResponse> datasets =
        findUserDataSetsUseCase
            .findUserDataSets(userId, pageable)
            .map(otherUserInfoMapper::toOtherUserData);
    LoggerFactory.service()
        .logSuccess(
            GET_OTHER_USER_INFO_USE_CASE,
            "타인이 업로드한 데이터셋 목록 추가 조회 서비스 성공 userId=" + userId,
            startTime);
    return datasets;
  }
}
