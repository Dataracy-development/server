...
public class UserCommandService
    implements ModifyUserInfoUseCase, WithdrawUserUseCase, LogoutUserUseCase {
  ...
  public void modifyUserInfo(
    ...
    Instant startTime =
        LoggerFactory.service()
            .logStart(MODIFY_USER_INFO_USE_CASE, "회원 정보 수정 서비스 시작 userId=" + userId);
        ...
        userQueryPort
            ...
            .orElseThrow(
                () -> {
                  LoggerFactory.service()
                      .logWarning(
                          MODIFY_USER_INFO_USE_CASE,
                          "[회원 정보 수정] " + USER_NOT_FOUND_MESSAGE + userId);
   ...
   */
  private void executeModifyUserInfo(
      Long userId,
      MultipartFile profileImageFile,
      ModifyUserInfoRequest requestDto,
      Instant startTime) {
   ...
   *
   * <p>요청된 프로필 이미지, 닉네임, 작성자 유형(필수), 선택적 직업·방문경로 및 토픽 ID들의 유효성을 확인합니다.
   *
   ...
   *
   * <p>파일이 null이거나 비어있으면 아무 동작도 수행하지 않는다.
   *
  ...
  private void modifyProfileImageFile(MultipartFile profileImageFile, Long userId, String useCase) {
    ...
    if (profileImageFile != null && !profileImageFile.isEmpty()) {
      try {
        String key =
            S3KeyGeneratorUtil.generateKey("user", userId, profileImageFile.getOriginalFilename());
        ...
        LoggerFactory.service()
            .logException(
                useCase,
                "유저 프로필 이미지 파일 업로드 실패. fileName=" + profileImageFile.getOriginalFilename(),
                e);
  ...
  public void withdrawUser(Long userId) {
    Instant startTime =
        LoggerFactory.service().logStart(WITHDRAW_USER_USE_CASE, "회원 탈퇴 서비스 시작 userId=" + userId);
    userCommandPort.withdrawalUser(userId);
    LoggerFactory.service()
        .logSuccess(WITHDRAW_USER_USE_CASE, "회원 탈퇴 서비스 성공 userId=" + userId, startTime);
  }

  /**
   * 사용자의 로그아웃을 처리한다.
   *
   * <p>전달된 리프레시 토큰으로 토큰에 포함된 사용자 ID를 검증한 뒤, 일치하면 해당 사용자의 리프레시 토큰을 삭제한다.
   *
  ...
  public void logout(Long userId, String refreshToken) {
    Instant startTime =
        LoggerFactory.service().logStart(LOGOUT_USER_USE_CASE, "회원 로그아웃 서비스 시작 userId=" + userId);
    ...
    if (!refreshTokenUserId.equals(userId)) {
      LoggerFactory.service()
          .logWarning(LOGOUT_USER_USE_CASE, "[로그아웃] 인증 요청을 한 유저와 일치하지 않는 리프레시 토큰입니다.");
    ...
    LoggerFactory.service()
        .logSuccess(LOGOUT_USER_USE_CASE, "회원 로그아웃 서비스 성공 userId=" + userId, startTime);
  }
}
