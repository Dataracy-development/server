...
public class SignUpUserService implements SelfSignUpUseCase, OAuthSignUpUseCase {
  ...
  public RefreshTokenResponse signUpSelf(SelfSignUpRequest requestDto) {
    ...
    User user = createUserDtoMapper.toDomain(requestDto, providerId, encodedPassword);
    User savedUser = userCommandPort.saveUser(user);

    // 리프레시 토큰 발급 및 저장
    String refreshToken =
        jwtGenerateUseCase.generateRefreshToken(savedUser.getId(), RoleType.ROLE_USER);
   ...
   * 사용자에 대해 리프레시 토큰을 생성·영구 저장한 뒤 토큰 값과 만료 시간을 담은 응답을 반환합니다. 닉네임 중복 방지를 위해 메서드 호출 시 닉네임 기반의 분산 락이
   * 적용됩니다.
   *
  ...
  public RefreshTokenResponse signUpOAuth(String registerToken, OnboardingRequest requestDto) {
    Instant startTime =
        LoggerFactory.service()
            .logStart(OAUTH_SIGN_UP_USE_CASE, "소셜 회원가입 서비스 시작 nickname=" + requestDto.nickname());
    ...
    User user = createUserDtoMapper.toDomain(requestDto, provider, providerId, email);
    User savedUser = userCommandPort.saveUser(user);
    ...
    LoggerFactory.service()
        .logSuccess(
            OAUTH_SIGN_UP_USE_CASE, "소셜 회원가입 서비스 성공 nickname=" + requestDto.nickname(), startTime);
    return refreshTokenResponse;
  }
   ...
   *
   * <p>이메일과 닉네임의 중복 여부를 확인하고, 작성자 유형의 존재를 필수로 검증한다. 직업, 방문 경로, 토픽 ID는 값이 제공된 경우에만 각각의 유효성을 검사한다.
   */
