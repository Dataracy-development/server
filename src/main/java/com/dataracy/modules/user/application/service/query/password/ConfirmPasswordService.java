...
public class ConfirmPasswordService implements ConfirmPasswordUseCase {
  ...
  /**
   * 주어진 사용자 ID와 비밀번호로 해당 사용자의 비밀번호가 일치하는지 확인한다. 사용자가 존재하지 않거나 비밀번호가 일치하지 않을 경우 UserException이 발생한다.
   *
  ...
  public void confirmPassword(Long userId, ConfirmPasswordRequest requestDto) {
    Instant startTime =
        LoggerFactory.service().logStart(CONFIRM_PASSWORD_USE_CASE, "유저의 비밀번호가 일치하는지 확인하는 서비스 시작");
            ...
            .orElseThrow(
                () -> {
                  LoggerFactory.service()
                      .logWarning(
                          CONFIRM_PASSWORD_USE_CASE,
                          "[비밀번호 확인] " + USER_NOT_FOUND_MESSAGE + userId);
    ...
    if (!isMatched) {
      LoggerFactory.service()
          .logWarning(CONFIRM_PASSWORD_USE_CASE, "[비밀번호 확인]제공받은 비밀번호와 실제 비밀번호가 일치하지 않습니다.");
      throw new UserException(UserErrorStatus.FAIL_CONFIRM_PASSWORD);
    }

    LoggerFactory.service()
        .logSuccess(CONFIRM_PASSWORD_USE_CASE, "유저의 비밀번호가 일치하는지 확인하는 서비스 성공", startTime);
  }
}
