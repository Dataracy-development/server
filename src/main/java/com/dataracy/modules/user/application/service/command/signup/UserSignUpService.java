...
public class UserSignUpService implements SelfSignUpUseCase, OAuthSignUpUseCase {
     ...
     *
     * 이메일, 닉네임, 비밀번호 등 필수 정보를 검증하고, 작성자 유형, 직업, 방문 경로, 토픽 등 선택 정보를 유효성 검사한 후 사용자 계정을 생성한다. 중복 가입을 방지하기 위해 이메일 기준 분산 락을 적용한다. 회원가입이 완료되면 리프레시 토큰을 생성하여 Redis에 저장하고, 토큰과 만료 시간을 포함한 응답을 반환한다.
     *
    ...
    /**
     * 소셜 로그인 회원가입을 처리하고 리프레시 토큰을 발급합니다.
     *
     * 소셜 회원가입 토큰의 유효성을 검증하고, 온보딩 정보를 기반으로 신규 사용자를 등록한 후 리프레시 토큰을 생성하여 Redis에 저장합니다.
     * 닉네임 중복 회원가입을 방지하기 위해 분산 락을 적용합니다.
     *
