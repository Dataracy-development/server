...
public class LikeCommandAdapter implements LikeCommandPort {
  ...
  public void save(Like like) {
    ...
    LoggerFactory.db()
        .logSave(
            LIKE_ENTITY,
            String.valueOf(savedLike.getId()),
            "타겟 좋아요가 완료되었습니다. likeId=" + savedLike.getId());
  }

  /**
   * 주어진 사용자 ID, 타겟 ID, 타겟 타입에 해당하는 좋아요를 취소합니다. 해당 조건에 맞는 좋아요가 존재하지 않을 경우 LikeException이 발생합니다.
   *
            ...
            .orElseThrow(
                () -> {
                  LoggerFactory.db()
                      .logWarning(
                          LIKE_ENTITY,
                          LIKE_NOT_FOUND_MESSAGE + targetType + ", targetId=" + targetId);
                  return new LikeException(LikeErrorStatus.NOT_FOUND_TARGET_LIKE);
                });
    likeJpaRepository.delete(entity);
    LoggerFactory.db()
        .logDelete(LIKE_ENTITY, String.valueOf(entity.getId()), "타겟 좋아요 취소가 완료되었습니다.");
  }
}
