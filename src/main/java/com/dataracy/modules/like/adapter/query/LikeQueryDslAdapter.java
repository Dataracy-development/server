...
public class LikeQueryDslAdapter implements ReadLikePort, ValidateLikePort {
  ...
  public boolean isLikedTarget(Long userId, Long targetId, TargetType targetType) {
    Instant startTime =
        LoggerFactory.query()
            .logQueryStart(
                LIKE_ENTITY,
                "[isLikedTarget] 사용자가 해당 타겟을 좋아요 했는지 여부 반환 시작. targetType="
                    + targetType
                    + ", targetId="
                    + targetId);
    Integer result =
    ...
    LoggerFactory.query()
        .logQueryEnd(
            LIKE_ENTITY,
            "[isLikedTarget] 사용자가 해당 타겟을 좋아요 했는지 여부 반환 완료. targetType="
                + targetType
                + ", targetId="
                + targetId,
            startTime);
  ...
  public List<Long> findLikedTargetIds(Long userId, List<Long> targetIds, TargetType targetType) {
    Instant startTime =
        LoggerFactory.query()
            .logQueryStart(
                LIKE_ENTITY,
                "[findLikedTargetIds] 사용자가 좋아요를 누른 타겟 ID 목록 반환 시작. targetType=" + targetType);
    if (userId == null || targetIds == null || targetIds.isEmpty()) {
      return List.of();
    }
    List<Long> likedTargetIds =
        queryFactory
    ...
    LoggerFactory.query()
        .logQueryEnd(
            LIKE_ENTITY,
            "[findLikedTargetIds] 사용자가 좋아요를 누른 타겟 ID 목록 반환 완료. targetType=" + targetType,
            startTime);
