...
public final class S3KeyGeneratorUtil {
   ...
   *
   * <p>입력된 도메인, 엔티티 ID, 원본 파일명에서 확장자를 추출하고, 무작위 UUID를 조합하여 `도메인/엔티티ID/UUID.확장자` 형식의 S3 키 문자열을
   * 반환합니다.
   *
   ...
   *
   * <p>반환되는 키는 `domain/entityId/thumb/uuid.extension` 형식이며, 파일 확장자는 원본 파일명에서 추출됩니다. 입력값이 null인 경우
   ...
   *
   * <p>파일명에 점(`.`)이 없거나 마지막이 점으로 끝나는 경우 기본 확장자인 "jpg"를 반환합니다.
   *
