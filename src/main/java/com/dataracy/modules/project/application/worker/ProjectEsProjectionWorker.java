...
public class ProjectEsProjectionWorker {
   ...
   *
   * <p>프로젝트 Elasticsearch 프로젝션 작업의 처리(삭제/복원, 댓글/좋아요/조회수 반영 등)에 필요한 외부 의존성을 초기화합니다.
   */
   ...
   *
   * <ul>
   *   <li>retryCount이 8 이상이면 고정 120초를 반환한다.
   *   <li>그렇지 않으면 2^(max(0, retryCount-1)) 초를 반환하되 최대 64초까지 제한된다.
   * </ul>
   *
  ...
  public void run() {
    List<ProjectEsProjectionTaskEntity> tasks =
        loadProjectProjectionTaskPort.findBatchForWork(
            LocalDateTime.now(),
   ...
   *
   * <p>주요 동작: - soft-delete/restore, 댓글/좋아요/조회수 델타를 ES에 적용한다. - 성공 시 해당 작업을 삭제한다. - 예외 발생 시 재시도 횟수를
   * 증가시키고 재시도 한도를 초과하면 DLQ로 저장한 뒤 작업을 삭제한다. - 재시도 시에는 상태를 RETRYING으로 설정하고 nextRunAt을 backoff 정책에 따라
   * 갱신한다. - 발생한 오류는 로깅된다.
   *
  ...
  public void processTask(ProjectEsProjectionTaskEntity t) {
    try {
      ...
      LoggerFactory.elastic()
          .logError("project_index", "ES 반영 실패 - projectId=" + t.getProjectId(), ex);
    }
  }
