...
public class ProjectLabelMapService implements FindProjectLabelMapUseCase {
   ...
   *
   * <p>각 Project에서 userId, topicId, analysisPurposeId, dataSourceId, authorLevelId를 수집한 뒤, 해당 ID들을
   * 관련 UseCase에 위임하여 사용자명, 사용자 썸네일, 토픽 레이블, 분석 목적 레이블, 데이터 소스 레이블 및 저자 레벨 레이블을 조회해
   * ProjectLabelMapResponse로 합칩니다.
   *
  ...
  public ProjectLabelMapResponse labelMapping(Collection<Project> savedProjects) {
    ...
    List<Long> analysisPurposeIds =
        savedProjects.stream().map(Project::getAnalysisPurposeId).toList();
