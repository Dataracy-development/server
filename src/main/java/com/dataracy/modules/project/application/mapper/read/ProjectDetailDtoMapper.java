...
public class ProjectDetailDtoMapper {
   ...
   *
   * <p>Project 객체의 필드(아이디, 제목, 작성자 아이디, 계속 여부, 부모 프로젝트 ID, 내용, 썸네일, 생성일, 댓글/좋아요/조회수 등) 를 응답 DTO의 기본
   * 값으로 사용하고, 사용자명·프로필·라벨·연결 데이터·좋아요 여부 등은 전달된 파라미터로 채웁니다.
   *
   * <p>참고: 13개의 파라미터를 가지지만, Mapper 레이어에서 도메인 객체와 여러 라벨 정보를 조합하여 상세 화면 DTO를 생성하므로 허용됩니다.
   *
