...
public class PopularProjectsBatchService implements UpdatePopularProjectsStorageUseCase {
   ...
   */
  @DistributedLock(
      key = "'lock:batch:popular-projects'",
      waitTime = 100L,
      leaseTime = 300000L,
      retry = 1)
  ...
  public void updatePopularProjectsCache() {
    ...
    try {
      ...
      ProjectLabelMapResponse labelResponse =
          findProjectLabelMapUseCase.labelMapping(savedProjects);
              ...
              .map(
                  project ->
                      popularProjectDtoMapper.toResponseDto(
                          ...
                          labelResponse
                              .analysisPurposeLabelMap()
                              .get(project.getAnalysisPurposeId()),
      ...
      LoggerFactory.scheduler()
          .logComplete("PopularProjectsBatchService - count=" + popularProjects.size());
  ...
  public void manualUpdatePopularProjectsCache(int size) {
    ...
    try {
      ...
      LoggerFactory.scheduler()
          .logComplete("PopularProjectsBatchService-Manual - count=" + popularProjects.size());
  ...
  public void warmUpCacheIfNeeded(int size) {
    ...
    LoggerFactory.scheduler()
        .logStart("PopularProjectsBatchService-WarmUpCheck - hasValidData=" + hasValidData);
    ...
    if (!hasValidData) {
      ...
      LoggerFactory.scheduler()
          .logComplete("PopularProjectsBatchService-WarmUpCheck - 캐시가 이미 존재하여 워밍업 생략");
    }
  }
}
