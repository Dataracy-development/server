        ...
        FindUserIdIncludingDeletedUseCase
{
    ...
    public Long findUserIdByProjectId(Long projectId) {
        Instant startTime = LoggerFactory.service().logStart(FIND_USER_ID_USE_CASE, "프로젝트 아이디로부터 유저 아이디 조회 서비스 시작 projectId=" + projectId);
        Long userId = extractProjectOwnerPort.findUserIdByProjectId(projectId);
        LoggerFactory.service().logSuccess(FIND_USER_ID_USE_CASE, "프로젝트 아이디로부터 유저 아이디 조회 서비스 종료 projectId=" + projectId, startTime);
        return userId;
    }
    ...
    public Long findUserIdIncludingDeleted(Long projectId) {
        Instant startTime = LoggerFactory.service().logStart(FIND_USER_ID_INCLUDING_DELETED_USE_CASE, "삭제된 프로젝트를 포함하여 프로젝트 아이디로부터 유저 아이디 조회 서비스 시작 projectId=" + projectId);
        Long userId = extractProjectOwnerPort.findUserIdIncludingDeleted(projectId);
        LoggerFactory.service().logSuccess(FIND_USER_ID_INCLUDING_DELETED_USE_CASE, "삭제된 프로젝트를 포함하여 프로젝트 아이디로부터 유저 아이디 조회 서비스 종료 projectId=" + projectId, startTime);
        return userId;
    }
}
