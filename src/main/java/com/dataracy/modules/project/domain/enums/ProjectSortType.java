...
public enum ProjectSortType {
   ...
   *
   * <p>입력값은 열거형 상수의 이름 또는 value 필드와 대소문자 구분 없이 비교하여 매칭됩니다. 일치하는 값이 없으면 ProjectException이 발생하며, 이때
   * 잘못된 입력에 대한 규칙 위반 로그가 기록됩니다.
   *
        ...
        .orElseThrow(
            () -> {
              LoggerFactory.domain()
                  .logRuleViolation(
                      "ProjectSortType",
                      "잘못된 ENUM 타입입니다. LATEST, OLDEST, MOST_LIKED, MOST_VIEWED, MOST_COMMENTED, LEAST_COMMENTED만 가능합니다.");
