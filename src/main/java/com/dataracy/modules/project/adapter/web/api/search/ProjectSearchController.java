...
public class ProjectSearchController implements ProjectSearchApi {
   ...
   * ProjectSuccessStatus.FIND_REAL_TIME_PROJECTS 상태를 가진 SuccessResponse로 래핑되고, HTTP 200 OK 응답으로
   * 반환됩니다. 발생한 예외는 호출자에게 전파됩니다.
   *
   ...
   */
  @Override
  public ResponseEntity<SuccessResponse<List<RealTimeProjectWebResponse>>> searchRealTimeProjects(
      String keyword, int size) {
    Instant startTime =
        LoggerFactory.api()
            .logRequest("[SearchRealTimeProjects] 자동완성을 위한 실시간 프로젝트 목록 조회 API 요청 시작");
    List<RealTimeProjectWebResponse> webResponse;

    try {
      List<RealTimeProjectResponse> responseDto =
          searchRealTimeProjectsUseCase.searchByKeyword(keyword, size);
      webResponse = responseDto.stream().map(projectSearchWebMapper::toWebDto).toList();
    } finally {
      LoggerFactory.api()
          .logResponse("[SearchRealTimeProjects] 자동완성을 위한 실시간 프로젝트 목록 조회 API 응답 완료", startTime);
    }
   ...
   *
   * <p>조회 결과는 {@code ProjectSuccessStatus.FIND_SIMILAR_PROJECTS} 상태와 함께 {@code
   * SuccessResponse<List<SimilarProjectWebResponse>>} 형태로 래핑됩니다. 메서드는 내부에서 요청/응답 로그를 남기며, 처리 중 발생한
   * 예외는 호출자에게 그대로 전파됩니다.
   *
   ...
   */
  @Override
  public ResponseEntity<SuccessResponse<List<SimilarProjectWebResponse>>> searchSimilarProjects(
      Long projectId, int size) {
    Instant startTime =
        LoggerFactory.api().logRequest("[SearchSimilarProjects] 유사 프로젝트 목록 조회 API 요청 시작");
    List<SimilarProjectWebResponse> webResponse;

    try {
      List<SimilarProjectResponse> responseDto =
          searchSimilarProjectsUseCase.searchSimilarProjects(projectId, size);
      webResponse = responseDto.stream().map(projectSearchWebMapper::toWebDto).toList();
    } finally {
   ...
   */
  @Override
  public ResponseEntity<SuccessResponse<Page<FilteredProjectWebResponse>>> searchFilteredProjects(
      FilteringProjectWebRequest webRequest, Pageable pageable) {
    Instant startTime =
        LoggerFactory.api().logRequest("[SearchFilteredProjects] 필터링된 프로젝트 목록 조회 API 요청 시작");
    ...
    try {
      ...
      Page<FilteredProjectResponse> responseDto =
          searchFilteredProjectsUsecase.searchByFilters(requestDto, pageable);
      webResponse = responseDto.map(projectFilterWebMapper::toWebDto);
    } finally {
      LoggerFactory.api()
          .logResponse("[SearchFilteredProjects] 필터링된 프로젝트 목록 조회 API 응답 완료", startTime);
    }
