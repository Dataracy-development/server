...
public class ProjectEntity extends BaseTimeEntity {
  ...
  public Set<ProjectEntity> getChildProjects() {
    return Collections.unmodifiableSet(childProjects);
  }
  ...
  // SpotBugs EI_EXPOSE_REP 경고 해결을 위한 방어적 복사
  public Set<ProjectDataEntity> getProjectDataEntities() {
    return Collections.unmodifiableSet(projectDataEntities);
  }
   ...
   *
   * <p>이 메서드는 소프트 삭제된 프로젝트의 `isDeleted` 플래그를 `false`로 변경하여 프로젝트를 다시 활성화합니다.
   */
  public void restore() {
   ...
   * @return 생성된 ProjectEntity 객체 참고: 10개의 파라미터를 가지지만, ProjectEntity가 복잡한 도메인 엔티티이고 Builder 패턴을
   *     내부적으로 사용하므로 허용됩니다.
   */
