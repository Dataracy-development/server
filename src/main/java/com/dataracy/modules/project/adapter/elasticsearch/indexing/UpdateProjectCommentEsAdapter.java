...
public class UpdateProjectCommentEsAdapter implements UpdateProjectCommentPort {
   ...
   *
   * <p>commentCount가 null이면 1로 초기화하고, 값이 있으면 1을 더합니다.
   *
    ...
    try {
      client.update(
          u ->
              u.index(INDEX)
                  ...
                  .script(
                      s -> s.inline(i -> i.lang("painless").source(INCREASE_COMMENT_COUNT_SCRIPT)))
                  .upsert(
      ...
      LoggerFactory.elastic()
          .logUpdate(
              ...
              "프로젝트 commentCount 증분 업데이트 완료 - projectId=" + projectId);
    } catch (IOException e) {
      LoggerFactory.elastic()
          .logError(INDEX, "프로젝트 commentCount 증분 업데이트 실패 - projectId=" + projectId, e);
   ...
   *
   * <p>commentCount가 null이거나 0 이하인 경우 0으로 설정됩니다.
   *
    ...
    try {
      client.update(
          u ->
              u.index(INDEX)
                  .id(String.valueOf(projectId))
                  .script(
                      s -> s.inline(i -> i.lang("painless").source(DECREASE_COMMENT_COUNT_SCRIPT)))
                  .upsert(
                      ProjectSearchDocument.builder()
                          .id(projectId)
                          .commentCount(0L)
      ...
      LoggerFactory.elastic()
          .logUpdate(
              INDEX,
              String.valueOf(projectId),
              "프로젝트 commentCount 감분 업데이트 완료 - projectId=" + projectId);
    } catch (IOException e) {
      LoggerFactory.elastic()
          .logError(INDEX, "프로젝트 commentCount 감분 업데이트 실패 - projectId=" + projectId, e);
      throw new EsUpdateException("ES update failed: projectId=" + projectId, e);
    }
  }
}
