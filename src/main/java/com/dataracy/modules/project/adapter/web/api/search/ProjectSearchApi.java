...
public interface ProjectSearchApi {
  ...
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
            description = "실시간 프로젝트 리스트 조회에 성공했습니다.",
   ...
   */
  @Operation(summary = "유사 프로젝트 리스트를 조회한다.", description = "해당하는 프로젝트와 유사한 프로젝트 리스트를 조회한다.")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
            description = "유사 프로젝트 리스트 조회에 성공했습니다.",
            useReturnTypeSchema = true)
      })
  @GetMapping("/{projectId}/similar")
  ResponseEntity<SuccessResponse<List<SimilarProjectWebResponse>>> searchSimilarProjects(
      @PathVariable(name = "projectId") @Min(1) Long projectId,
      @RequestParam(name = "size") @Min(1) int size);
   ...
   */
  @Operation(summary = "필터링된 프로젝트 리스트를 조회한다.", description = "필터링된 프로젝트 리스트를 조회한다.")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
            description = "필터링된 프로젝트 리스트 조회에 성공했습니다.",
            useReturnTypeSchema = true)
      })
  @GetMapping("/filter")
  ResponseEntity<SuccessResponse<Page<FilteredProjectWebResponse>>> searchFilteredProjects(
      @Validated @ModelAttribute FilteringProjectWebRequest webRequest,
