...
public class ReadProjectQueryDslAdapter
  ...
  public Optional<Project> findProjectById(Long projectId) {
    Instant startTime =
        LoggerFactory.query()
            .logQueryStart(
                PROJECT_ENTITY,
                "[findProjectById] 아이디를 통해 삭제되지 않은 프로젝트를 조회 시작. projectId=" + projectId);
    ProjectEntity entity =
        queryFactory
            .selectFrom(project)
            .where(
                ProjectFilterPredicate.projectIdEq(projectId), ProjectFilterPredicate.notDeleted())
            .fetchOne();
    Optional<Project> savedProject = Optional.ofNullable(ProjectEntityMapper.toMinimal(entity));
    LoggerFactory.query()
        .logQueryEnd(
            PROJECT_ENTITY,
            "[findProjectById] 아이디를 통해 삭제되지 않은 프로젝트를 조회 완료. projectId=" + projectId,
            startTime);
    return savedProject;
  }
   ...
   *
   * <p>상세: - 프로젝트가 존재하지 않으면 Optional.empty()를 반환합니다. - 존재하면 프로젝트(부모 프로젝트 정보 포함)와 그에 연결된 데이터셋 ID 목록을
   * ProjectWithDataIdsResponse로 감싸 반환합니다. - 성능 최적화: 하나의 쿼리로 프로젝트와 연결된 데이터 ID를 함께 조회하여 N+1 문제 해결
   *
   * @param projectId 조회할 프로젝트의 ID
  ...
  public Optional<ProjectWithDataIdsResponse> findProjectWithDataById(Long projectId) {
    Instant startTime =
        LoggerFactory.query()
            .logQueryStart(
                PROJECT_ENTITY,
                "[findProjectWithDataById] 아이디를 통해 삭제되지 않은 프로젝트를 연결된 데이터셋과 함께 조회 시작. projectId="
                    + projectId);
    ...
    List<Tuple> results =
        queryFactory
            ...
            .leftJoin(projectData)
            .on(projectData.project.id.eq(project.id).and(ProjectDataFilterPredicate.notDeleted()))
            .where(
                ProjectFilterPredicate.projectIdEq(projectId), ProjectFilterPredicate.notDeleted())
            .fetch();
    ...
    if (results.isEmpty()) {
      LoggerFactory.query()
          .logQueryEnd(
              PROJECT_ENTITY,
              "[findProjectWithDataById] 해당하는 프로젝트 리소스가 존재하지 않습니다. projectId=" + projectId,
              startTime);
    ...
    Optional<ProjectWithDataIdsResponse> projectWithDataIdsResponse =
        Optional.of(
            new ProjectWithDataIdsResponse(ProjectEntityMapper.toWithParent(entity), dataIds));
    LoggerFactory.query()
        .logQueryEnd(
            PROJECT_ENTITY,
            "[findProjectWithDataById] 아이디를 통해 삭제되지 않은 프로젝트를 연결된 데이터셋과 함께 조회 완료. projectId="
                + projectId,
            startTime);
  ...
  public Page<Project> findContinuedProjects(Long projectId, Pageable pageable) {
    Instant startTime =
        LoggerFactory.query()
            .logQueryStart(
                PROJECT_ENTITY,
                "[findContinuedProjects] 이어가기 프로젝트 목록 조회 시작. projectId=" + projectId);
    ...
    List<Project> contents = entities.stream().map(ProjectEntityMapper::toMinimal).toList();
    ...
    LoggerFactory.query()
        .logQueryEnd(
            PROJECT_ENTITY,
            "[findContinuedProjects] 이어가기 프로젝트 목록 조회 완료. projectId=" + projectId,
            startTime);
   ...
   *
   * <p>프로젝트는 연결된 ProjectData 엔티티를 기준으로 프로젝트 생성일(createdAt) 내림차순으로 정렬되어 반환됩니다. 조회는 soft-delete(삭제
   * 플래그가 설정되지 않은) 된 연결만 대상으로 하며, 결과 콘텐츠는 최소 정보(minimal) 형태로 매핑됩니다. 총건수는 해당 데이터와 연결된 서로 다른 프로젝트
   * 수(distinct)를 기준으로 계산됩니다.
   *
  ...
  public Page<Project> findConnectedProjectsAssociatedWithDataset(Long dataId, Pageable pageable) {
    Instant startTime =
        LoggerFactory.query()
            .logQueryStart(
                PROJECT_ENTITY,
                "[findConnectedProjectsAssociatedWithDataset] 데이터셋과 연결된 프로젝트 목록 조회 시작. dataId="
                    + dataId);
    ...
    List<ProjectEntity> entities =
        queryFactory.selectFrom(project).where(project.id.in(projectIds)).fetch();

    Map<Long, ProjectEntity> entityMap =
        entities.stream().collect(Collectors.toMap(ProjectEntity::getId, Function.identity()));
    ...
    LoggerFactory.query()
        .logQueryEnd(
            PROJECT_ENTITY,
            "[findConnectedProjectsAssociatedWithDataset] 데이터셋과 연결된 프로젝트 목록 조회 완료. dataId="
                + dataId,
            startTime);
    return new PageImpl<>(contents, pageable, total);
  }
   ...
   *
   * <p>논리적으로 삭제되지 않은 프로젝트만 포함되며, 내부적으로 인기 순 정렬을 적용하고 결과를 도메인 모델의 최소 표현(Project)으로 매핑합니다.
   *
  ...
  public List<Project> getPopularProjects(int size) {
    Instant startTime =
        LoggerFactory.query()
            .logQueryStart(PROJECT_ENTITY, "[getPopularProjects] 인기있는 프로젝트 목록 조회 시작.");
    List<Project> popularProjects =
        queryFactory
            .selectFrom(project)
    ...
    LoggerFactory.query()
        .logQueryEnd(PROJECT_ENTITY, "[getPopularProjects] 인기있는 프로젝트 목록 조회 완료.", startTime);
   ...
   *
   * <p>작성일 기준 최신순으로 정렬된 프로젝트들의 최소 정보(minimal)를 반환하며, 전달된 Pageable이 null이면 기본 페이지(PageRequest.of(0,
   * 5))를 사용한다. 결과의 total은 해당 사용자에 대한 비삭제 프로젝트 총 개수를 반영한다.
   *
  ...
  public Page<Project> findUserProjects(Long userId, Pageable pageable) {
    ...
    Pageable effectivePageable = (pageable == null) ? PageRequest.of(0, 5) : pageable;

    Instant startTime =
        LoggerFactory.query()
            .logQueryStart(
                PROJECT_ENTITY, "[findUserProjects] 해당 회원이 작성한 프로젝트 목록 조회 시작. userId=" + userId);

    List<ProjectEntity> entities =
        queryFactory
            .selectFrom(project)
            .orderBy(ProjectSortBuilder.fromSortOption(ProjectSortType.LATEST))
            .where(ProjectFilterPredicate.userIdEq(userId), ProjectFilterPredicate.notDeleted())
   ...
   *
   * <p>요청된 페이지의 좋아요 기록에서 프로젝트 ID들을 먼저 가져오고, 해당 ID에 대응하는 삭제되지 않은 프로젝트들을 로드한 뒤 좋아요 순서를 유지하여 최소 정보
   * 형태(Project 최소 도메인)로 매핑해 반환한다.
   *
   * @param userId 조회할 사용자의 식별자
   * @param pageable 결과 페이징 및 정렬을 지정하는 Pageable 객체(널이 아니어야 함)
   * @return 페이징된 프로젝트 목록(Page<Project>) — 콘텐츠는 Project의 최소 정보이며 전체 항목 수(total)는 해당 사용자의 좋아요 총 개수에
   *     기반함
   */
  @Override
  public Page<Project> findLikeProjects(Long userId, Pageable pageable) {
    Instant startTime =
        LoggerFactory.query()
            .logQueryStart(
                PROJECT_ENTITY, "[findLikeProjects] 해당 회원이 좋아요한 프로젝트 목록 조회 시작. userId=" + userId);

    // 좋아요 ID 페이징
    List<Long> likeProjectIds =
        queryFactory
    ...
    Map<Long, ProjectEntity> projectMap =
        entities.stream().collect(Collectors.toMap(ProjectEntity::getId, e -> e));

    List<Project> contents =
        likeProjectIds.stream()
            .map(projectMap::get)
            .filter(Objects::nonNull)
            .map(ProjectEntityMapper::toMinimal)
            .toList();
