...
public class SearchSimilarProjectsEsAdapter implements SearchSimilarProjectsPort {
    ...
    try {
      Instant startTime =
          LoggerFactory.elastic()
              .logQueryStart(
                  INDEX, "유사 프로젝트 검색 시작: projectId=" + project.getId() + ", size=" + size);
      ...
      List<SimilarProjectResponse> similarProjectResponses =
          response.hits().hits().stream()
              .map(
                  hit ->
                      mapToSimilarResponse(
                          Objects.requireNonNull(hit.source(), "검색된 document가 null값입니다.")))
              .toList();
      LoggerFactory.elastic()
          .logQueryEnd(
              INDEX, "유사 프로젝트 검색 종료: projectId=" + project.getId() + ", size=" + size, startTime);
      return similarProjectResponses;
    } catch (IOException e) {
      LoggerFactory.elastic()
          .logError(INDEX, "유사 프로젝트 조회 실패: projectId=" + project.getId() + ", size=" + size, e);
   ...
   *
   * <p>제목과 내용을 활용한 유사 문서 검색, 주제 ID 및 분석 목적 ID의 가중치 부여, 동일 프로젝트 제외, 삭제되지 않은 프로젝트만 포함하는 조건을 결합합니다.
   *
   * @param q 쿼리 빌더 객체
                        ...
                        sb.moreLikeThis(
                            mlt ->
                                mlt.fields("title", "content")
                                    ...
                                    .minDocFreq(1)))
                .should(
                    sb -> sb.term(t -> t.field("topicId").value(project.getTopicId()).boost(1.5f)))
                .should(
                    sb ->
                        sb.term(
                            t ->
                                t.field("analysisPurposeId")
                                    .value(project.getAnalysisPurposeId())
                                    .boost(1.3f)))
                .mustNot(mn -> mn.term(t -> t.field("id").value(project.getId())))
   ...
   *
   * <p>변환된 응답 객체는 프로젝트의 식별자와 텍스트 정보(제목, 내용), 작성자 정보(userId, username), 썸네일·분류 레이블(topic, analysis
   * purpose, data source, author level) 및 통계(commentCount, likeCount, viewCount)를 포함한다.
   *
