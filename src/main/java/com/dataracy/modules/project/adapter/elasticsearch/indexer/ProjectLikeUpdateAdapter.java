...
public class ProjectLikeUpdateAdapter implements ProjectLikeUpdatePort {
    ...
    public void increaseLikeCount(Long projectId) {
        try {
            log.info("Elasticsearch: likeCount 증분 업데이트 시작 - projectId={}", projectId);
            client.update(u -> u
                            .index(INDEX)
            ...
            log.info("Elasticsearch: likeCount 증분 업데이트 완료 - projectId={}", projectId);
        } catch (IOException e) {
            log.error("Elasticsearch: likeCount 증분 업데이트 실패 - projectId={}", projectId, e);
        }
    }
    ...
    public void decreaseLikeCount(Long projectId) {
        try {
            log.info("Elasticsearch: likeCount 감분 업데이트 시작 - projectId={}", projectId);
            client.update(u -> u
                            .index(INDEX)
                            .id(projectId.toString())
                            .script(s -> s
            ...
            log.info("Elasticsearch: likeCount 감분 업데이트 완료 - projectId={}", projectId);
        } catch (IOException e) {
            log.error("Elasticsearch: likeCount 감분 업데이트 실패 - projectId={}", projectId, e);
        }
    }
}
