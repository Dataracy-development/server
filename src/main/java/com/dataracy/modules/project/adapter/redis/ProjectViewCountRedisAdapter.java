...
public class ProjectViewCountRedisAdapter implements ManageProjectViewCountPort {
   ...
   *
   * <p>동일 뷰어가 5분 내에 여러 번 조회해도 조회수는 한 번만 증가합니다.
   *
  ...
  public void increaseViewCount(Long projectId, String viewerId, String targetType) {
    try {
      ...
      LoggerFactory.redis()
          .logError(
              VIEW_COUNT_PREFIX + targetType + ":" + projectId,
              REDIS_CONNECTION_FAILURE_MESSAGE,
              e);
      throw new CommonException(CommonErrorStatus.REDIS_CONNECTION_FAILURE);
    } catch (DataAccessException e) {
      LoggerFactory.redis()
          .logError(
              VIEW_COUNT_PREFIX + targetType + ":" + projectId, DATA_ACCESS_FAILURE_MESSAGE, e);
   ...
   *
   * <p>키 `viewCount:{targetType}:{projectId}`의 값을 읽어 Long으로 반환합니다. 키가 존재하지 않으면 0L을 반환합니다.
   *
    ...
    try {
      Instant startTime =
          LoggerFactory.redis()
              .logQueryStart(
                  VIEW_COUNT_PREFIX + targetType + ":" + projectId,
                  "해당 프로젝트의 조회수 조회 시작. projectId=" + projectId);
      ...
      LoggerFactory.redis()
          .logQueryEnd(
              VIEW_COUNT_PREFIX + targetType + ":" + projectId,
              "해당 프로젝트의 조회수 조회 종료. projectId=" + projectId,
              startTime);
      return viewCount;
    } catch (RedisConnectionFailureException e) {
      LoggerFactory.redis()
  ...
  public Set<String> getAllViewCountKeys(String targetType) {
    try {
      Instant startTime =
          LoggerFactory.redis()
              .logQueryStart(
                  VIEW_COUNT_PREFIX + targetType + ":*",
                  "지정된 타겟 타입에 해당하는 모든 조회수 Redis 키를 반환. targetType=" + targetType);
      ...
      LoggerFactory.redis()
          .logError(VIEW_COUNT_PREFIX + targetType + ":*", REDIS_CONNECTION_FAILURE_MESSAGE, e);
      throw new CommonException(CommonErrorStatus.REDIS_CONNECTION_FAILURE);
    } catch (DataAccessException e) {
      LoggerFactory.redis()
          .logError(VIEW_COUNT_PREFIX + targetType + ":*", DATA_ACCESS_FAILURE_MESSAGE, e);
      throw new CommonException(CommonErrorStatus.DATA_ACCESS_EXCEPTION);
    }
  }
   ...
   *
   * <p>삭제 작업 중 Redis 연결 실패 또는 데이터 접근 오류가 발생하면 CommonException으로 래핑되어 전파됩니다.
   *
  ...
  public void clearViewCount(Long targetId, String targetType) {
    try {
      ...
      LoggerFactory.redis()
          .logDelete(
              VIEW_COUNT_PREFIX + targetType + ":" + targetId,
              "지정된 대상 ID와 대상 타입에 해당하는 조회수 카운트 Redis 키를 삭제. targetId=" + targetId);
    } catch (RedisConnectionFailureException e) {
      LoggerFactory.redis()
          .logError(
              VIEW_COUNT_PREFIX + targetType + ":" + targetId, REDIS_CONNECTION_FAILURE_MESSAGE, e);
      throw new CommonException(CommonErrorStatus.REDIS_CONNECTION_FAILURE);
    } catch (DataAccessException e) {
      LoggerFactory.redis()
          .logError(
              VIEW_COUNT_PREFIX + targetType + ":" + targetId, DATA_ACCESS_FAILURE_MESSAGE, e);
      throw new CommonException(CommonErrorStatus.DATA_ACCESS_EXCEPTION);
    }
  }
   ...
   *
   * <p>대상 키(viewCount:{targetType}:{projectId})에 저장된 값을 가져와 키를 삭제한 뒤 해당 값을 Long으로 반환합니다. 값이 존재하지
   * 않거나 숫자 파싱에 실패하면 0L을 반환합니다.
   *
    ...
    try {
      Instant startTime =
          LoggerFactory.redis()
              .logQueryStart(
                  VIEW_COUNT_PREFIX + targetType + ":" + projectId,
                  "조회수 pop 작업 시작. projectId=" + projectId);
      ...
      LoggerFactory.redis()
          .logQueryEnd(
              VIEW_COUNT_PREFIX + targetType + ":" + projectId,
              "조회수 pop 작업 종료. projectId=" + projectId,
              startTime);
      return viewCount;
    } catch (RedisConnectionFailureException e) {
