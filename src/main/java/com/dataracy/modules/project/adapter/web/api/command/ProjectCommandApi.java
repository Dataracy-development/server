...
public interface ProjectCommandApi {
   ...
   */
  @Operation(summary = "프로젝트를 업로드한다.", description = "제공받은 웹 요청 DTO의 프로젝트 정보를 통해 프로젝트 정보를 db에 저장한다")
  @Parameter(
  ...
  ResponseEntity<SuccessResponse<UploadProjectWebResponse>> uploadProject(
      ...
      @RequestPart @Validated UploadProjectWebRequest webRequest);
   ...
   */
  @Operation(summary = "프로젝트를 수정한다.", description = "제공받은 웹 요청 DTO의 프로젝트 정보를 통해 기존 프로젝트를 수정한다.")
  @Parameter(
      in = ParameterIn.HEADER,
      name = "Authorization",
      required = true,
  ...
  ResponseEntity<SuccessResponse<Void>> modifyProject(
      ...
      @RequestPart(value = "thumbnailFile", required = false) MultipartFile thumbnailFile,
      @RequestPart @Validated ModifyProjectWebRequest webRequest);
   ...
   */
  @Operation(summary = "프로젝트를 삭제한다.", description = "해당하는 프로젝트를 삭제한다.")
  @Parameter(
      in = ParameterIn.HEADER,
      name = "Authorization",
      required = true,
  ...
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
            description = "해당하는 프로젝트 삭제에  성공했습니다.",
            useReturnTypeSchema = true)
      })
  @DeleteMapping("/{projectId}")
  ResponseEntity<SuccessResponse<Void>> deleteProject(@PathVariable @Min(1) Long projectId);
   ...
   */
  @Operation(summary = "프로젝트를 복원한다.", description = "해당하는 프로젝트를 복원한다.")
  @Parameter(
      in = ParameterIn.HEADER,
      name = "Authorization",
      required = true,
  ...
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
            description = "해당하는 프로젝트 복원에  성공했습니다.",
            useReturnTypeSchema = true)
      })
  @PatchMapping("/{projectId}/restore")
  ResponseEntity<SuccessResponse<Void>> restoreProject(@PathVariable @Min(1) Long projectId);
}
