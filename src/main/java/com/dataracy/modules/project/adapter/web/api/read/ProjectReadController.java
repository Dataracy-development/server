...
public class ProjectReadController implements ProjectReadApi {
   ...
   *
   * <p>HTTP 요청 및 응답에서 인증된 사용자 ID와 뷰어 ID를 추출한 후, 해당 정보를 기반으로 프로젝트의 상세 정보를 조회하여 반환합니다.
   *
  ...
  public ResponseEntity<SuccessResponse<ProjectDetailWebResponse>> getProjectDetail(
    ...
    Instant startTime =
        LoggerFactory.api().logRequest("[GetProjectDetail] 프로젝트 상세 정보 조회 API 요청 시작");
    ...
    try {
      ...
      ProjectDetailResponse responseDto =
          getProjectDetailUseCase.getProjectDetail(projectId, userId, viewerId);
   ...
   */
  @Override
  public ResponseEntity<SuccessResponse<Page<ContinuedProjectWebResponse>>> findContinueProjects(
      Long projectId, Pageable pageable) {
    Instant startTime =
        LoggerFactory.api().logRequest("[FindContinueProjects] 이어가기 프로젝트 목록 조회 API 요청 시작");
    Page<ContinuedProjectWebResponse> webResponse;

    try {
      Page<ContinuedProjectResponse> responseDto =
          findContinuedProjectsUseCase.findContinuedProjects(projectId, pageable);
      webResponse = responseDto.map(projectReadWebMapper::toWebDto);
    } finally {
      LoggerFactory.api()
          .logResponse("[FindContinueProjects] 이어가기 프로젝트 목록 조회 API 응답 완료", startTime);
    }
   ...
   */
  @Override
  public ResponseEntity<SuccessResponse<Page<ConnectedProjectWebResponse>>>
      findConnectedProjectsAssociatedWithData(Long dataId, Pageable pageable) {
    Instant startTime =
        LoggerFactory.api()
            .logRequest("[FindConnectedProjectsAssociatedWithData] 데이터셋과 연결된 프로젝트 목록 조회 API 요청 시작");
    Page<ConnectedProjectWebResponse> webResponse;

    try {
      Page<ConnectedProjectResponse> responseDto =
          findConnectedProjectsUseCase.findConnectedProjects(dataId, pageable);
      webResponse = responseDto.map(projectReadWebMapper::toWebDto);
    } finally {
      LoggerFactory.api()
          .logResponse(
              "[FindConnectedProjectsAssociatedWithData] 데이터셋과 연결된 프로젝트 목록 조회 API 응답 완료",
              startTime);
    }
    ...
    return ResponseEntity.status(HttpStatus.OK)
        .body(
            SuccessResponse.of(
                ProjectSuccessStatus.GET_CONNECTED_PROJECTS_ASSOCIATED_DATA, webResponse));
  }
   ...
   */
  @Override
  public ResponseEntity<SuccessResponse<List<PopularProjectWebResponse>>> getPopularProjects(
      int size) {
    Instant startTime =
        LoggerFactory.api().logRequest("[GetPopularProjects] 인기 프로젝트 목록 조회 API 요청 시작 size=" + size);
    ...
    try {
      ...
      webResponse = responseDto.stream().map(projectReadWebMapper::toWebDto).toList();
   ...
   *
   * <p>요청된 페이지 정보에 따라 FindUserProjectsUseCase로부터 프로젝트 목록을 조회하고, 각 도메인 응답을 UserProjectWebResponse로
   * 매핑한 결과를 SuccessResponse(GET_USER_PROJECTS)로 감싸 HTTP 200 응답으로 반환합니다.
   *
   ...
   */
  @Override
  public ResponseEntity<SuccessResponse<Page<UserProjectWebResponse>>> findUserProjects(
      Long userId, Pageable pageable) {
    Instant startTime =
        LoggerFactory.api().logRequest("[FindUserProjects] 로그인한 회원이 업로드한 프로젝트 리스트를 조회 API 요청 시작");
    Page<UserProjectWebResponse> webResponse;

    try {
      Page<UserProjectResponse> responseDto =
          findUserProjectsUseCase.findUserProjects(userId, pageable);
      webResponse = responseDto.map(projectReadWebMapper::toWebDto);
    } finally {
      LoggerFactory.api()
          .logResponse("[FindUserProjects] 로그인한 회원이 업로드한 프로젝트 리스트를 조회 API 응답 완료", startTime);
    }
   ...
   */
  @Override
  public ResponseEntity<SuccessResponse<Page<UserProjectWebResponse>>> findLikeProjects(
      Long userId, Pageable pageable) {
    Instant startTime =
        LoggerFactory.api().logRequest("[FindLikeProjects] 로그인한 회원이 좋아요한 프로젝트 리스트를 조회 API 요청 시작");
    Page<UserProjectWebResponse> webResponse;

    try {
      Page<UserProjectResponse> responseDto =
          findUserProjectsUseCase.findLikeProjects(userId, pageable);
      webResponse = responseDto.map(projectReadWebMapper::toWebDto);
    } finally {
      LoggerFactory.api()
          .logResponse("[FindLikeProjects] 로그인한 회원이 좋아요한 프로젝트 리스트를 조회 API 응답 완료", startTime);
    }
