...
public class PopularProjectsRedisAdapter implements PopularProjectsStoragePort {
  ...
  public Optional<List<PopularProjectResponse>> getPopularProjects() {
    LoggerFactory.redis()
        .logInfo(POPULAR_PROJECTS_KEY, "PopularProjectsRedisAdapter.getPopularProjects() 호출됨");
    Instant startTime =
        LoggerFactory.redis().logQueryStart(POPULAR_PROJECTS_KEY, "인기 프로젝트 캐시 조회 시작");
    ...
    try {
      ...
      List<PopularProjectResponse> popularProjects =
          objectMapper.readValue(cachedData, new TypeReference<List<PopularProjectResponse>>() {});
  ...
  public void setPopularProjects(List<PopularProjectResponse> popularProjects) {
    LoggerFactory.redis()
        .logInfo(
            POPULAR_PROJECTS_KEY,
            "PopularProjectsRedisAdapter.setPopularProjects() 호출됨 - count="
                + popularProjects.size());
    try {
      String jsonData = objectMapper.writeValueAsString(popularProjects);
      LoggerFactory.redis()
          .logInfo(POPULAR_PROJECTS_KEY, "JSON 직렬화 완료 - size=" + jsonData.length());

      redisTemplate.opsForValue().set(POPULAR_PROJECTS_KEY, jsonData, CACHE_TTL);
      LoggerFactory.redis()
          .logInfo(POPULAR_PROJECTS_KEY, "Redis 저장 완료 - key=" + POPULAR_PROJECTS_KEY);
      ...
      LoggerFactory.redis()
          .logInfo(POPULAR_PROJECTS_KEY, "메타데이터 저장 완료 - key=" + POPULAR_PROJECTS_METADATA_KEY);

      LoggerFactory.redis()
          .logSaveOrUpdate(
              POPULAR_PROJECTS_KEY, "인기 프로젝트 캐시 저장 성공 count=" + popularProjects.size());
  ...
  public boolean hasValidData() {
    return redisTemplate.hasKey(POPULAR_PROJECTS_KEY)
        && redisTemplate.hasKey(POPULAR_PROJECTS_METADATA_KEY);
  }
}
