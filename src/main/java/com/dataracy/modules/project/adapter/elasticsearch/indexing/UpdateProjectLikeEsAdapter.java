...
public class UpdateProjectLikeEsAdapter implements UpdateProjectLikePort {
    ...
    try {
      client.update(
          u ->
              u.index(INDEX)
                  ...
                  .script(s -> s.inline(i -> i.lang("painless").source(INCREASE_LIKE_COUNT_SCRIPT)))
                  .upsert(
      ...
      LoggerFactory.elastic()
          .logUpdate(
              ...
              "프로젝트 likeCount 증분 업데이트 완료 - projectId=" + projectId);
    } catch (IOException e) {
      LoggerFactory.elastic()
          .logError(INDEX, "프로젝트 likeCount 증분 업데이트 실패 - projectId=" + projectId, e);
   ...
   *
   * <p>likeCount가 0보다 크면 1 감소시키고, 그렇지 않으면 0으로 설정합니다.
   *
    ...
    try {
      client.update(
          u ->
              u.index(INDEX)
                  .id(String.valueOf(projectId))
                  .script(s -> s.inline(i -> i.lang("painless").source(DECREASE_LIKE_COUNT_SCRIPT)))
                  .upsert(
                      ProjectSearchDocument.builder()
                          .id(projectId)
                          .likeCount(0L)
      ...
      LoggerFactory.elastic()
          .logUpdate(
              INDEX,
              String.valueOf(projectId),
              "프로젝트 likeCount 감분 업데이트 완료 - projectId=" + projectId);
    } catch (IOException e) {
      LoggerFactory.elastic()
          .logError(INDEX, "프로젝트 likeCount 감분 업데이트 실패 - projectId=" + projectId, e);
      throw new EsUpdateException("ES update failed: projectId=" + projectId, e);
    }
  }
}
