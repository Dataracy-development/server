...
public class ProjectCommandController implements ProjectCommandApi {
   ...
   * 새 프로젝트를 생성한다.
   *
   * <p>사용자 ID, 썸네일 이미지 및 업로드 정보를 받아 프로젝트를 생성하고 생성된 프로젝트 정보(웹 DTO)를 반환한다.
   *
  ...
  public ResponseEntity<SuccessResponse<UploadProjectWebResponse>> uploadProject(
      Long userId, MultipartFile thumbnailFile, UploadProjectWebRequest webRequest) {
    ...
    try {
      ...
      UploadProjectResponse responseDto =
  ...
  public ResponseEntity<SuccessResponse<Void>> modifyProject(
      Long projectId, MultipartFile thumbnailFile, ModifyProjectWebRequest webRequest) {
