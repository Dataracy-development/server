...
public class SoftDeleteProjectEsAdapter implements SoftDeleteProjectPort {
  ...
  private void updateDeletedStatus(Long projectId, ProjectDeletedUpdate update, String operation) {
    try {
      client.update(
          u -> u.index(INDEX).id(String.valueOf(projectId)).doc(update),
          ProjectSearchDocument.class);
      LoggerFactory.elastic()
          .logUpdate(
              INDEX, String.valueOf(projectId), "프로젝트 " + operation + "완료: projectId=" + projectId);
    } catch (IOException e) {
      LoggerFactory.elastic()
          .logError(INDEX, "프로젝트 " + operation + "실패: projectId=" + projectId, e);
