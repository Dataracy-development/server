...
public record ProjectDetailWebResponse(
    ...
    @Schema(description = "작성자 소개글", example = "안녕하세요. 박준형입니다.") String userIntroductionText,
    @Schema(description = "작성자 유형 라벨", example = "실무자") String authorLevelLabel,
    @Schema(description = "직업 라벨", example = "디자이너") String occupationLabel,
    ...
    @Schema(description = "데이터 출처 라벨", example = "한국데이터협회") String dataSourceLabel,
    @Schema(description = "이어가기 프로젝트인지 여부", example = "false") boolean isContinue,
    @Schema(description = "부모 프로젝트 아이디", example = "3") Long parentProjectId,
    ...
    @Schema(description = "자식 프로젝트 존재 유무", example = "false") boolean hasChild,
    @Schema(description = "연결된 데이터셋 목록") List<ProjectConnectedDataWebResponse> connectedDataSets,
    @Schema(description = "부모 프로젝트") ParentProjectWebResponse parentProject) {}
