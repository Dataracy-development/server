...
public class ManageProjectEsProjectionTaskDbAdapter implements ManageProjectProjectionTaskPort {
     ...
     *
     * 이 메서드는 댓글 수 변경을 나타내는 projection task 엔티티를 생성해 저장소에 저장함으로써
     * 이후 프로젝션 적용 시 해당 프로젝트의 댓글 수를 조정할 수 있도록 합니다.
     *
     ...
     *
     * deltaLike는 증가(양수) 또는 감소(음수)를 나타내는 정수 값이다.
     *
     ...
     *
     * 영속 저장소에 프로젝션 작업 엔티티를 저장하여 조회수 변경 처리가 나중에 적용되도록 대기열에 추가한다.
     *
     ...
     *
     * 영속 저장소에 "삭제 상태 변경" 작업 엔티티를 생성해 저장하므로, 이후 프로젝션 처리 파이프라인에서
     * 해당 프로젝트의 삭제 플래그가 반영된다.
     *
