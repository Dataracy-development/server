...
public final class ProjectEntityMapper {
   ...
   *
   * <p>변환 대상이 null이면 null을 반환한다. 자식 프로젝트, 데이터 목록 등 다른 연관 정보는 포함하지 않는다.
   *
   ...
   *
   * <p>부모 프로젝트 정보와 데이터 ID 목록은 포함하지 않습니다.
   *
  ...
  /**
   * ProjectEntity를 Project 도메인 객체로 변환하며, 연관된 프로젝트 데이터 ID 목록만 포함합니다. 하위 프로젝트 정보는 포함하지 않습니다.
   *
   * @param entity 변환할 ProjectEntity 객체
   ...
   *
   * <p>입력 엔티티가 null이면 null을 반환합니다. includeParent가 true이면 부모 프로젝트의 ID를 parentProjectId로 설정하고,
   * includeData가 true이면 연관된 데이터 ID 목록을 포함하며, includeChildren이 true이면 자식 프로젝트들을 최소 정보 형태로 최대
   * childrenCount개까지 포함합니다. includeChildren이 false이면 자식 목록은 비어있습니다.
   *
   ...
   */
  private static Project toDomain(
    ...
    Long parentProjectId =
        includeParent
            ? Optional.ofNullable(entity.getParentProject()).map(ProjectEntity::getId).orElse(null)
            : null;
    ...
    List<Project> childProjects =
        includeChildren
            ? Optional.ofNullable(entity.getChildProjects()).orElse(Collections.emptySet()).stream()
                .limit(childrenCount)
   ...
   *
   * <p>프로젝트에 부모 프로젝트 ID가 있으면 해당 ID로 부모 엔티티를 생성하여 연관시킵니다. 프로젝트에 연결된 데이터 ID 목록이 있으면 각각 {@link
   * ProjectDataEntity}로 매핑하여 엔티티에 추가합니다.
   *
  ...
  public static ProjectEntity toEntity(Project project) {
    ...
    ProjectEntity parentProject =
        Optional.ofNullable(project.getParentProjectId())
            .map(parentProjectId -> ProjectEntity.builder().id(parentProjectId).build())
            .orElse(null);
    ...
    Optional.ofNullable(project.getDataIds()).orElse(Collections.emptyList()).stream()
