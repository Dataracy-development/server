...
public interface ProjectEsProjectionTaskRepository
    extends JpaRepository<ProjectEsProjectionTaskEntity, Long> {
  /**
   * 지정된 상태이고 nextRunAt이 주어진 시점 이전(이하)인 프로젝션 작업을 페이징 단위로 페시미스틱 쓰기 잠금하여 조회한다.
   *
   * <p>조회 시 선택된 행에는 PESSIMISTIC_WRITE 잠금을 적용하고, 다른 트랜잭션에 의해 잠긴 행은 건너뛴다 (JPA 힌트: SKIP LOCKED). 결과는
   * nextRunAt 오름차순, 동일 시각일 경우 id 오름차순으로 정렬된다.
   *
   ...
   *
   * <p>삭제 쿼리를 직접 실행하며, 실행 후 영속성 컨텍스트를 플러시하고(clearAutomatically=false가 아님) 자동으로 초기화합니다. 일치하는 엔티티가
   * 없으면 아무것도 삭제되지 않습니다.
   *
