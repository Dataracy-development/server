...
public class ProjectFilterWebMapper {
   ...
   *
   * <p>응답의 기본 필드(id, title, content 등)와 생성자 정보(creatorId, creatorName)를 그대로 매핑하며, 하위 프로젝트 목록은
   * ChildProjectWebMapper를 통해 ChildProjectWebResponse 목록으로 변환하여 포함합니다.
   *
  ...
  public FilteredProjectWebResponse toWebDto(FilteredProjectResponse responseDto) {
    ...
    List<ChildProjectWebResponse> childProjectWebResponses =
        responseDto.childProjects().stream().map(childProjectWebMapper::toWebDto).toList();
