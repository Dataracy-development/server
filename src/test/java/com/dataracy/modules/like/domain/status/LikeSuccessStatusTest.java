...
class LikeSuccessStatusTest {
    ...
    void likeSuccessStatuses_ShouldHaveCorrectMessage() {
        // Then
        assertAll(
                () -> assertThat(LikeSuccessStatus.LIKE_PROJECT.getMessage()).contains("프로젝트에 대한 좋아요 처리에 성공했습니다"),
                () -> assertThat(LikeSuccessStatus.UNLIKE_PROJECT.getMessage()).contains("프로젝트에 대한 좋아요 취소 처리에 성공했습니다"),
                () -> assertThat(LikeSuccessStatus.LIKE_COMMENT.getMessage()).contains("댓글에 대한 좋아요 처리에 성공했습니다"),
                () -> assertThat(LikeSuccessStatus.UNLIKE_COMMENT.getMessage()).contains("댓글에 대한 좋아요 취소 처리에 성공했습니다")
        );
    }
}
