...
class UserProjectReadServiceTest {
  ...
  @Mock private UserProjectDtoMapper userProjectDtoMapper;

  @Mock private FindUserProjectsPort findUserProjectsPort;
  ...
  @InjectMocks private UserProjectReadService service;
  ...
  class FindUserProjectsTest {
    ...
    void findUserProjectsSuccess() {
      ...
      given(getAuthorLevelLabelFromIdUseCase.getLabelsByIds(List.of(2L, 4L)))
          .willReturn(authorLevelLabelMap);
      given(userProjectDtoMapper.toResponseDto(projects.get(0), "AI", "Expert"))
          .willReturn(response1);
      given(userProjectDtoMapper.toResponseDto(projects.get(1), "Data Science", "Beginner"))
      ...
      then(loggerService)
          .should()
          .logStart(
              eq("FindUserProjectsUseCase"), eq("해당 회원이 작성한 프로젝트 목록 조회 서비스 시작 userId=" + userId));
      then(loggerService)
          .should()
          .logSuccess(
              eq("FindUserProjectsUseCase"),
              eq("해당 회원이 작성한 프로젝트 목록 조회 서비스 종료 userId=" + userId),
              any(Instant.class));
    }
  ...
  class FindLikeProjectsTest {
    ...
    void findLikeProjectsSuccess() {
      ...
      given(getAuthorLevelLabelFromIdUseCase.getLabelsByIds(List.of(6L, 8L)))
          .willReturn(authorLevelLabelMap);
      given(userProjectDtoMapper.toResponseDto(projects.get(0), "Machine Learning", "Advanced"))
          .willReturn(response1);
      given(userProjectDtoMapper.toResponseDto(projects.get(1), "Deep Learning", "Intermediate"))
          .willReturn(response2);
      ...
      then(loggerService)
          .should()
          .logStart(
              eq("FindLikeProjectsUseCase"), eq("해당 회원이 좋아요한 프로젝트 목록 조회 서비스 시작 userId=" + userId));
      then(loggerService)
          .should()
          .logSuccess(
              eq("FindLikeProjectsUseCase"),
              eq("해당 회원이 좋아요한 프로젝트 목록 조회 서비스 종료 userId=" + userId),
              any(Instant.class));
    }
  ...
  private Project createProject(
      Long id, String title, Long topicId, Long userId, Long authorLevelId) {
    return Project.of(
        id,
        title,
        topicId,
