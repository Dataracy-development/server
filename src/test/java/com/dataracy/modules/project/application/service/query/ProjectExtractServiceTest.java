...
class ProjectExtractServiceTest {
  ...
  @InjectMocks private ProjectExtractService service;
  ...
  class FindUserIdByProjectIdTest {
    ...
    void findUserIdByProjectIdSuccess() {
      ...
      then(loggerService)
          .should()
          .logStart(
              eq("FindUserIdUseCase"),
              contains("프로젝트 아이디로부터 유저 아이디 조회 서비스 시작 projectId=" + projectId));
      then(loggerService)
          .should()
          .logSuccess(
              eq("FindUserIdUseCase"),
              contains("프로젝트 아이디로부터 유저 아이디 조회 서비스 종료 projectId=" + projectId),
              any(Instant.class));
    }
  ...
  class FindUserIdIncludingDeletedTest {
    ...
    void findUserIdIncludingDeletedSuccess() {
      ...
      given(extractProjectOwnerPort.findUserIdIncludingDeleted(projectId))
      ...
      then(loggerService)
          .should()
          .logStart(
              eq("FindUserIdIncludingDeletedUseCase"),
              contains("삭제된 프로젝트를 포함하여 프로젝트 아이디로부터 유저 아이디 조회 서비스 시작 projectId=" + projectId));
      then(loggerService)
          .should()
          .logSuccess(
              eq("FindUserIdIncludingDeletedUseCase"),
              contains("삭제된 프로젝트를 포함하여 프로젝트 아이디로부터 유저 아이디 조회 서비스 종료 projectId=" + projectId),
              any(Instant.class));
    }
    ...
    void findUserIdIncludingDeletedWithDeletedProject() {
      ...
      given(extractProjectOwnerPort.findUserIdIncludingDeleted(deletedProjectId))
          .willReturn(expectedUserId);
      ...
      then(loggerService)
          ...
          .logStart(
              eq("FindUserIdIncludingDeletedUseCase"),
              contains(
                  "삭제된 프로젝트를 포함하여 프로젝트 아이디로부터 유저 아이디 조회 서비스 시작 projectId=" + deletedProjectId));
      then(loggerService)
          .should()
          .logSuccess(
              eq("FindUserIdIncludingDeletedUseCase"),
              contains("삭제된 프로젝트를 포함하여 프로젝트 아이디로부터 유저 아이디 조회 서비스 종료 projectId=" + deletedProjectId),
              any(Instant.class));
    }
