...
class UpdateProjectViewEsAdapterTest {
  ...
  void increaseViewCountSuccess() throws IOException {
    ...
    try (MockedStatic<LoggerFactory> loggerFactoryMock = mockStatic(LoggerFactory.class)) {
      ...
      then(elasticLogger)
          .should()
          .logUpdate("project_index", "123", "프로젝트 viewCount 증분 업데이트 완료 - projectId=123");
    }
  }
    ...
    try (MockedStatic<LoggerFactory> loggerFactoryMock = mockStatic(LoggerFactory.class)) {
      ...
      EsUpdateException exception =
          catchThrowableOfType(
              () -> adapter.increaseViewCount(projectId, increment), EsUpdateException.class);
      assertAll(
          () ->
      ...
      then(elasticLogger)
          ...
          .logError(
              eq("project_index"),
              eq("프로젝트 viewCount 증분 업데이트 실패 - projectId=123"),
  ...
  void increaseViewCountBatchSuccess() throws IOException {
    ...
    try (MockedStatic<LoggerFactory> loggerFactoryMock = mockStatic(LoggerFactory.class)) {
      ...
      then(elasticLogger)
          .should()
          .logUpdate("project_index", "배치 처리", "프로젝트 viewCount 배치 업데이트 완료. 처리된 프로젝트 수: 3");
    }
  }
  ...
  void increaseViewCountWithLargeIncrement() throws IOException {
    ...
    try (MockedStatic<LoggerFactory> loggerFactoryMock = mockStatic(LoggerFactory.class)) {
      ...
      then(elasticLogger)
          .should()
          .logUpdate("project_index", "456", "프로젝트 viewCount 증분 업데이트 완료 - projectId=456");
    }
  }
  ...
  void increaseViewCountBatchWithPartialFailure() throws IOException {
    ...
    try (MockedStatic<LoggerFactory> loggerFactoryMock = mockStatic(LoggerFactory.class)) {
      ...
      then(elasticLogger)
          ...
          .logError(
              eq("project_index"),
              eq("프로젝트 viewCount 증분 업데이트 실패 - projectId=1"),
              any(IOException.class));
      then(elasticLogger)
          .should()
          .logError(
              eq("project_index"),
              eq("프로젝트 viewCount 증분 업데이트 실패 - projectId=2"),
              any(IOException.class));
      ...
      then(elasticLogger)
          ...
          .logError(
              eq("project_index"),
              eq("배치 처리 중 개별 프로젝트 업데이트 실패 - projectId=1"),
              any(Exception.class));
      then(elasticLogger)
          .should()
          .logError(
              eq("project_index"),
              eq("배치 처리 중 개별 프로젝트 업데이트 실패 - projectId=2"),
              any(Exception.class));
      then(elasticLogger)
          .should()
          .logUpdate("project_index", "배치 처리", "프로젝트 viewCount 배치 업데이트 완료. 처리된 프로젝트 수: 2");
    }
  }
}
