...
class ProjectReadControllerTest {
  ...
  @MockBean private ProjectReadWebMapper projectReadWebMapper;

  @MockBean private GetProjectDetailUseCase getProjectDetailUseCase;

  @MockBean private GetPopularProjectsUseCase getPopularProjectsUseCase;

  @MockBean private FindUserProjectsUseCase findUserProjectsUseCase;

  @MockBean private FindContinuedProjectsUseCase findContinuedProjectsUseCase;

  @MockBean private FindConnectedProjectsUseCase findConnectedProjectsUseCase;

  @MockBean private ExtractHeaderUtil extractHeaderUtil;
  ...
  @MockBean
  private com.dataracy.modules.common.config.web.MultipartJackson2HttpMessageConverter
      multipartJackson2HttpMessageConverter;

  @MockBean
  private com.dataracy.modules.common.support.resolver.CurrentUserIdArgumentResolver
  ...
  void getProjectDetailSuccess() throws Exception {
    ...
    given(extractHeaderUtil.extractViewerIdFromRequest(any(), any()))
        .willReturn(String.valueOf(viewerId));
    given(
            getProjectDetailUseCase.getProjectDetail(
                projectId, authenticatedUserId, String.valueOf(viewerId)))
        .willReturn(responseDto);
    given(projectReadWebMapper.toWebDto(responseDto)).willReturn(webResponse);

    // when & then
    mockMvc
        .perform(get("/api/v1/projects/{projectId}", projectId).accept(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.code").value(ProjectSuccessStatus.GET_PROJECT_DETAIL.getCode()))
        .andExpect(
            jsonPath("$.message").value(ProjectSuccessStatus.GET_PROJECT_DETAIL.getMessage()))
    ...
    then(getProjectDetailUseCase)
        .should()
        .getProjectDetail(projectId, authenticatedUserId, String.valueOf(viewerId));
  ...
  void getPopularProjectsSuccess() throws Exception {
    // given
    PopularProjectResponse project1 =
    ...
    PopularProjectWebResponse webProject1 =
        new PopularProjectWebResponse(
            1L,
            "Popular Project 1",
            "Content 1",
            1L,
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath("$.message").value(ProjectSuccessStatus.FIND_POPULAR_PROJECTS.getMessage()))
  ...
  void getUserProjectsSuccess() throws Exception {
    ...
    UserProjectWebResponse webProject1 =
        new UserProjectWebResponse(
            1L,
            "User Project 1",
            "Content 1",
            "thumb1.jpg",
    ...
    given(findUserProjectsUseCase.findUserProjects(any(), any(Pageable.class)))
