...
class UpdateProjectCommentEsAdapterTest {
    ...
    void increaseCommentCountSuccess() throws IOException {
        ...
        try (MockedStatic<LoggerFactory> loggerFactoryMock = mockStatic(LoggerFactory.class)) {
            ...
            then(elasticLogger).should().logUpdate("project_index", "123", "프로젝트 commentCount 증분 업데이트 완료 - projectId=123");
        }
    }
    ...
    void decreaseCommentCountSuccess() throws IOException {
        ...
        try (MockedStatic<LoggerFactory> loggerFactoryMock = mockStatic(LoggerFactory.class)) {
            ...
            // then
            then(elasticsearchClient).should().update(any(Function.class), eq(ProjectSearchDocument.class));
            then(elasticLogger).should().logUpdate("project_index", "456", "프로젝트 commentCount 감분 업데이트 완료 - projectId=456");
        }
    }
    ...
    void increaseCommentCountWithIOException() throws IOException {
        ...
        try (MockedStatic<LoggerFactory> loggerFactoryMock = mockStatic(LoggerFactory.class)) {
            ...
            // when & then
            assertThatThrownBy(() -> adapter.increaseCommentCount(projectId))
                .isInstanceOf(EsUpdateException.class)
                .hasMessage("ES update failed: projectId=123")
                .hasCause(ioException);
            
            then(elasticLogger).should().logError(eq("project_index"), eq("프로젝트 commentCount 증분 업데이트 실패 - projectId=123"), any(IOException.class));
        }
    }
    ...
    void decreaseCommentCountWithIOException() throws IOException {
        ...
        try (MockedStatic<LoggerFactory> loggerFactoryMock = mockStatic(LoggerFactory.class)) {
            ...
            // when & then
            assertThatThrownBy(() -> adapter.decreaseCommentCount(projectId))
                .isInstanceOf(EsUpdateException.class)
                .hasMessage("ES update failed: projectId=456")
                .hasCause(ioException);
            
            then(elasticLogger).should().logError(eq("project_index"), eq("프로젝트 commentCount 감분 업데이트 실패 - projectId=456"), any(IOException.class));
        }
    }
}
