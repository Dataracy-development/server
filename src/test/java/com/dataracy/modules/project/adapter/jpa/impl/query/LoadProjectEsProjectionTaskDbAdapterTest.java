...
class LoadProjectEsProjectionTaskDbAdapterTest {
  ...
  @Mock private ProjectEsProjectionTaskRepository repo;

  @InjectMocks private LoadProjectEsProjectionTaskDbAdapter adapter;
  ...
  void findBatchForWorkSuccess() {
    ...
    given(repo.findBatchForWork(now, statuses, pageable)).willReturn(List.of(task));
  ...
  void findBatchForWorkEmpty() {
    ...
    given(repo.findBatchForWork(now, statuses, pageable)).willReturn(List.of());
