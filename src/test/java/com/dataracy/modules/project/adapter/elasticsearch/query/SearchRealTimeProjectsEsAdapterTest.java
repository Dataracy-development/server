...
class SearchRealTimeProjectsEsAdapterTest {
  ...
  void searchByKeywordWithIOException() throws IOException {
    ...
    willThrow(ioException)
        ...
        .search(any(Function.class), eq(ProjectSearchDocument.class));
    ...
    try (MockedStatic<LoggerFactory> loggerFactoryMock = mockStatic(LoggerFactory.class)) {
      ...
      ProjectException exception =
          catchThrowableOfType(
              () -> adapter.searchByKeyword(keyword, size), ProjectException.class);
      assertAll(
          () ->
      ...
      then(elasticLogger)
          ...
          .logError(
              eq("project_index"),
              eq("프로젝트 실시간 검색 실패: keyword=test, size=5"),
