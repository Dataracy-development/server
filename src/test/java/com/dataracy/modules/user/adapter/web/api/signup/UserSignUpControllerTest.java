...
class UserSignUpControllerTest {
    ...
    void signUpUserSelfSuccess() throws Exception {
        ...
        SelfSignUpWebRequest webRequest = new SelfSignUpWebRequest(
                "test@example.com", "Password123!", "Password123!", "TestUser", 1L, 2L, List.of(1L, 2L), 3L, true
        );

        SelfSignUpRequest requestDto = new SelfSignUpRequest(
                "test@example.com", "Password123!", "Password123!", "TestUser", 1L, 2L, List.of(1L, 2L), 3L, true
        );
    ...
    void onboardingSuccess() throws Exception {
        ...
        OnboardingWebRequest webRequest = new OnboardingWebRequest(
                "TestUser", 1L, 2L, List.of(1L, 2L), 3L, true
        );

        OnboardingRequest requestDto = new OnboardingRequest(
                "TestUser", 1L, 2L, List.of(1L, 2L), 3L, true
        );
        ...
        given(userSignUpWebMapper.toApplicationDto(webRequest)).willReturn(requestDto);
        given(oauthSignUpUseCase.signUpOAuth(anyString(), any(OnboardingRequest.class))).willReturn(responseDto);
        willDoNothing().given(cookieUtil).setCookie(any(), any(), any(), any(), anyInt());
    ...
    @Test
    @DisplayName("signUpUserOAuth API: 성공 - 201 Created와 JSON 응답 검증")
    void signUpUserOAuthSuccess() throws Exception {
        // given
        SelfSignUpWebRequest webRequest = new SelfSignUpWebRequest(
                "test@example.com", "Password123!", "Password123!", "TestUser", 1L, 2L, List.of(1L, 2L), 3L, true
        );
