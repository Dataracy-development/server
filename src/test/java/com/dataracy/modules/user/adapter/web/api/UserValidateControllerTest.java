...
class UserValidateControllerTest {
    ...
    @MockBean
    private UserValidationWebMapper userWebMapper;
    ...
    void duplicateNicknameSuccess() throws Exception {
        ...
        given(userWebMapper.toApplicationDto(any(DuplicateNicknameWebRequest.class))).willReturn(reqDto);
        willDoNothing().given(duplicateNicknameUseCase).validateDuplicatedNickname("nick");

        // when & then
        mockMvc.perform(post("/api/v1/nickname/check")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(webReq)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(UserSuccessStatus.OK_NOT_DUPLICATED_NICKNAME.getCode()));
