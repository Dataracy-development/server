...
class ChangePasswordServiceTest {
  ...
  void changePassword_userNotFound() {
    ...
    // when
    UserException ex =
        catchThrowableOfType(() -> service.changePassword(99L, req), UserException.class);
  ...
  void changePassword_socialForbidden() {
    ...
    // when
    UserException ex =
        catchThrowableOfType(() -> service.changePassword(10L, req), UserException.class);
  ...
  class ResetPasswordTests {
    ...
    void resetPassword_success() {
      ...
      given(userQueryPort.findUserByEmail("u@test.com"))
          .willReturn(Optional.of(localUser(ProviderType.LOCAL)));
    ...
    void resetPassword_invalidToken() {
      ...
      // when
      UserException ex =
          catchThrowableOfType(() -> service.resetPassword(req), UserException.class);

      // then
      assertThat(ex.getErrorCode())
          .isEqualTo(UserErrorStatus.INVALID_OR_EXPIRED_RESET_PASSWORD_TOKEN);
      then(userQueryPort).shouldHaveNoInteractions();
      then(userCommandPort).shouldHaveNoInteractions();
    }
