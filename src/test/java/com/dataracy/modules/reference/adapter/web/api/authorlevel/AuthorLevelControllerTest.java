...
@WebMvcTest(
    controllers = AuthorLevelController.class,
    includeFilters =
...
class AuthorLevelControllerTest {
  ...
  @MockBean private FindAllAuthorLevelsUseCase findAllAuthorLevelsUseCase;

  @MockBean private AuthorLevelWebMapper webMapper;
  ...
  void findAllAuthorLevelsSuccess() throws Exception {
    ...
    AllAuthorLevelsWebResponse web =
        new AllAuthorLevelsWebResponse(List.of(new AuthorLevelWebResponse(1L, "VAL_A", "라벨A")));
    ...
    // when & then
    mockMvc
        .perform(get("/api/v1/references/author-levels").accept(MediaType.APPLICATION_JSON))
