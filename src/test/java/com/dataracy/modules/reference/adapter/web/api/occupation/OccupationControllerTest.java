...
@WebMvcTest(
    controllers = OccupationController.class,
    includeFilters =
...
class OccupationControllerTest {
  ...
  @MockBean private FindAllOccupationsUseCase findAllOccupationsUseCase;

  @MockBean private OccupationWebMapper webMapper;
  ...
  void findAllOccupationsSuccess() throws Exception {
    ...
    AllOccupationsWebResponse web =
        new AllOccupationsWebResponse(List.of(new OccupationWebResponse(1L, "DEV", "개발자")));
    ...
    // when & then
    mockMvc
        .perform(get("/api/v1/references/occupations").accept(MediaType.APPLICATION_JSON))
