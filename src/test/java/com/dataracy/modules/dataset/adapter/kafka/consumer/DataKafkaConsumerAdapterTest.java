...
class DataKafkaConsumerAdapterTest {
    ...
    @Mock
    private ParseMetadataUseCase parseMetadataUseCase;
    ...
    void consumeDataUploadEventSuccess() {
        ...
        try (MockedStatic<LoggerFactory> loggerFactoryMock = mockStatic(LoggerFactory.class)) {
            ...
            then(kafkaLogger).should().logConsume("data-uploaded-topic", "데이터셋 업로드 이벤트 처리 완료: dataId=123");
        }
    }
    ...
    void consumeDataUploadEventFailure() {
        // given
        DataUploadEvent event = new DataUploadEvent(456L, "http://example.com/data.xlsx", "dataset.xlsx");
        RuntimeException exception = new RuntimeException("Metadata parsing failed");
        willThrow(exception).given(parseMetadataUseCase).parseAndSaveMetadata(any(ParseMetadataRequest.class));
