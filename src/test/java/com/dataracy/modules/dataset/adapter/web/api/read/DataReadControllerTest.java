...
class DataReadControllerTest {
  ...
  @MockBean private DataReadWebMapper mapper;

  @MockBean private GetPopularDataSetsUseCase getPopularDataSetsUseCase;

  @MockBean private GetDataDetailUseCase getDataDetailUseCase;

  @MockBean private GetRecentMinimalDataSetsUseCase getRecentMinimalDataSetsUseCase;

  @MockBean private GetDataGroupCountUseCase getDataGroupCountUseCase;

  @MockBean private FindConnectedDataSetsUseCase findConnectedDataSetsUseCase;

  @MockBean private FindUserDataSetsUseCase findUserDataSetsUseCase;
  ...
  @MockBean private SecurityPathConfig securityPathConfig;
    ...
    this.mockMvc =
        MockMvcBuilders.standaloneSetup(
            ...
            .setCustomArgumentResolvers(
                new PageableHandlerMethodArgumentResolver(), currentUserIdArgumentResolver)
            .build();
  ...
  void getPopularDataSetsShouldReturnOk() throws Exception {
    PopularDataResponse resDto =
        new PopularDataResponse(
            ...
            5L);
    PopularDataWebResponse webRes =
        new PopularDataWebResponse(
            1L,
            "데이터1",
            1L,
            "userA",
    ...
    mockMvc
        .perform(
            get("/api/v1/datasets/popular").param("size", "3").accept(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.code").value(DataSuccessStatus.FIND_POPULAR_DATASETS.getCode()))
        .andExpect(
            jsonPath("$.message").value(DataSuccessStatus.FIND_POPULAR_DATASETS.getMessage()))
  ...
  void getDataDetailShouldReturnOk() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/datasets/{dataId}", 2L).accept(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
  ...
  void getRecentDataSetsShouldReturnOk() throws Exception {
    RecentMinimalDataResponse resDto =
        new RecentMinimalDataResponse(
            10L, "데이터10", 1L, "userA", "https://~~", "thumb.png", LocalDateTime.now());
    RecentMinimalDataWebResponse webRes =
        new RecentMinimalDataWebResponse(
            10L, "데이터10", 1L, "userA", "https://~~", "thumb.png", LocalDateTime.now());
    ...
    mockMvc
        .perform(
            get("/api/v1/datasets/recent").param("size", "2").accept(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
  ...
  void getDataCountByTopicLabelShouldReturnOk() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/datasets/group-by/topic").accept(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(
            jsonPath("$.code").value(DataSuccessStatus.COUNT_DATASETS_GROUP_BY_TOPIC.getCode()))
        .andExpect(
            jsonPath("$.message")
                .value(DataSuccessStatus.COUNT_DATASETS_GROUP_BY_TOPIC.getMessage()))
  ...
  void findConnectedDataSetsShouldReturnOk() throws Exception {
    ...
    ConnectedDataResponse resDto =
        new ConnectedDataResponse(
            ...
            4L);
    ConnectedDataWebResponse webRes =
        new ConnectedDataWebResponse(
            7L,
            "연결된데이터",
            1L,
            "userA",
    ...
    mockMvc
        .perform(
            get("/api/v1/datasets/connected-to-project")
                .param("projectId", "99")
                .param("page", "0")
                .param("size", "1")
        ...
        .andExpect(
            jsonPath("$.code")
                .value(DataSuccessStatus.GET_CONNECTED_DATASETS_ASSOCIATED_PROJECT.getCode()))
        .andExpect(
            jsonPath("$.message")
                .value(DataSuccessStatus.GET_CONNECTED_DATASETS_ASSOCIATED_PROJECT.getMessage()))
  ...
  void findUserDataSetsShouldReturnOk() throws Exception {
    UserDataResponse resDto =
        new UserDataResponse(
            ...
            LocalDateTime.of(2023, 3, 1, 10, 0),
            4L);
    UserDataWebResponse webRes =
        new UserDataWebResponse(
            7L,
            "연결된데이터",
            "경제",
            "CSV",
    ...
    Page<UserDataResponse> pageRes =
        new PageImpl<>(new ArrayList<>(List.of(resDto)), PageRequest.of(0, 1), 1);
    given(findUserDataSetsUseCase.findUserDataSets(any(), any())).willReturn(pageRes);
    given(mapper.toWebDto(resDto)).willReturn(webRes);

    mockMvc
        .perform(
            get("/api/v1/datasets/me")
                .param("page", "0")
                .param("size", "5")
                .accept(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
