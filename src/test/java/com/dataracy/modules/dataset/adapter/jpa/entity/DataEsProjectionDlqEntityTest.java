...
class DataEsProjectionDlqEntityTest {
  ...
  void builderShouldApplyDefaultValues() {
    // when
    DataEsProjectionDlqEntity entity =
        DataEsProjectionDlqEntity.builder().dataId(100L).lastError("error message").build();
  ...
  void allArgsConstructorShouldSetAllFields() {
    // when
    DataEsProjectionDlqEntity entity =
        new DataEsProjectionDlqEntity(10L, 200L, 3, true, "dlq error");
