...
class SearchDataQueryDslAdapterIntegrationTest {
  ...
  void setUp() {
    ...
    DataMetadataEntity metadata =
        DataMetadataEntity.builder().data(savedData).columnCount(10).rowCount(1000).build();
  ...
  class ComplexScenarioTest {
    ...
    void data_with_multiple_project_connections() {
      ...
      // then
      assertAll(
          () -> assertThat(result).isNotEmpty(), () -> assertThat(result.getContent()).hasSize(3));

      // 프로젝트 연결 수 확인
      List<DataWithProjectCountDto> content = result.getContent();
      boolean foundTwoConnections =
          content.stream().anyMatch(dto -> dto.countConnectedProjects() == 2);
      boolean foundOneConnection =
          content.stream().anyMatch(dto -> dto.countConnectedProjects() == 1);
      boolean foundZeroConnection =
          content.stream().anyMatch(dto -> dto.countConnectedProjects() == 0);
    ...
    void data_with_metadata_retrieval() {
      ...
      // then
      assertAll(
          () -> assertThat(result).isNotEmpty(), () -> assertThat(result.getContent()).hasSize(1));
