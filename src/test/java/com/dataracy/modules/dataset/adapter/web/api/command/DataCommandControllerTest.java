...
class DataCommandControllerTest {
    ...
    @MockBean
    private DataCommandWebMapper dataCommandWebMapper;

    @MockBean
    private DataDownloadWebMapper dataDownloadWebMapper;

    @MockBean
    private UploadDataUseCase uploadDataUseCase;

    @MockBean
    private ModifyDataUseCase modifyDataUseCase;
    ...
    void uploadDataShouldReturnCreated() throws Exception {
        ...
        UploadDataResponse appRes =
                new UploadDataResponse(100L);
        UploadDataWebResponse webRes =
                new UploadDataWebResponse(100L);
        ...
        given(uploadDataUseCase.uploadData(anyLong(), any(), any(), any(UploadDataRequest.class)))
                .willReturn(appRes);
        given(dataCommandWebMapper.toWebDto(appRes))
                .willReturn(webRes);

        // MockMultipartFile 준비
        MockMultipartFile dataFile =
                new MockMultipartFile("dataFile", "data.csv", "text/csv", "dummy".getBytes());
        MockMultipartFile thumbnailFile =
                new MockMultipartFile("thumbnailFile", "thumb.png", "image/png", "dummy-thumb".getBytes());
        MockMultipartFile webRequestPart =
                new MockMultipartFile("webRequest", "", "application/json",
        ...
        mockMvc.perform(multipart("/api/v1/datasets")
                ...
                .andExpect(jsonPath("$.code")
                        .value(DataSuccessStatus.CREATED_DATASET.getCode()))
                .andExpect(jsonPath("$.message")
                        .value(DataSuccessStatus.CREATED_DATASET.getMessage()))
    ...
    void modifyDataShouldReturnOk() throws Exception {
        ...
        given(dataCommandWebMapper.toApplicationDto(any(ModifyDataWebRequest.class)))
                .willReturn(appReq);
        willDoNothing().given(modifyDataUseCase)
        ...
        // when & then
        mockMvc.perform(multipart("/api/v1/datasets/{dataId}", 100L)
                        .file(dataFile)
                ...
                .andExpect(jsonPath("$.code")
                        .value(DataSuccessStatus.MODIFY_DATASET.getCode()))
                .andExpect(jsonPath("$.message")
                        .value(DataSuccessStatus.MODIFY_DATASET.getMessage()));
    }
    ...
    void deleteDataShouldReturnOk() throws Exception {
        // given
        willDoNothing().given(deleteDataUseCase)
                .deleteData(100L);

        // when & then
        mockMvc.perform(delete("/api/v1/datasets/{dataId}", 100L))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code")
                        .value(DataSuccessStatus.DELETE_DATASET.getCode()))
                .andExpect(jsonPath("$.message")
                        .value(DataSuccessStatus.DELETE_DATASET.getMessage()));
    }
    ...
    void restoreDataShouldReturnOk() throws Exception {
        // given
        willDoNothing().given(restoreDataUseCase)
                .restoreData(100L);

        // when & then
        mockMvc.perform(patch("/api/v1/datasets/{dataId}/restore", 100L))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code")
                        .value(DataSuccessStatus.RESTORE_DATASET.getCode()))
                .andExpect(jsonPath("$.message")
                        .value(DataSuccessStatus.RESTORE_DATASET.getMessage()));
    }
    ...
    void getPreSignedDataUrlShouldReturnOk() throws Exception {
        // given
        GetDataPreSignedUrlResponse appRes =
                new GetDataPreSignedUrlResponse("http://signed-url");

        given(downloadDataFileUseCase.downloadDataFile(anyLong(), anyInt()))
                .willReturn(appRes);
        ...
        // when & then
        mockMvc.perform(get("/api/v1/datasets/{dataId}/download", 100L))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code")
                        .value(DataSuccessStatus.DOWNLOAD_DATASET.getCode()))
                .andExpect(jsonPath("$.message")
                        .value(DataSuccessStatus.DOWNLOAD_DATASET.getMessage()))
