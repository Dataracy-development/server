...
class UserDataReadServiceTest {
  @InjectMocks private UserDataReadService service;

  @Mock private DataReadDtoMapper mapper;

  @Mock private FindUserDataSetsPort findUserDataSetsPort;
  ...
  void getUserDataSetsEmpty() {
    // given
    given(findUserDataSetsPort.findUserDataSets(eq(1L), any())).willReturn(Page.empty());
  ...
  void getUserDataSetsWithLabels() {
    ...
    Page<DataWithProjectCountDto> page =
        new PageImpl<>(
            List.of(
                new DataWithProjectCountDto(data1, 2L), new DataWithProjectCountDto(data2, 3L)));
