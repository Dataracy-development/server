...
class PopularDataSetsBatchServiceTest {
  ...
  @Mock private DataReadDtoMapper dataReadDtoMapper;

  @Mock private FindDataLabelMapUseCase findDataLabelMapUseCase;

  @Mock private PopularDataSetsStoragePort popularDataSetsStoragePort;

  @Mock private GetPopularDataSetsPort getPopularDataSetsPort;

  @InjectMocks private PopularDataSetsBatchService popularDataSetsBatchService;

  private Data createSampleData(
      Long id, Long userId, Long topicId, Long dataSourceId, Long dataTypeId) {
    return Data.of(
        id,
        "title",
        userId,
  ...
  class UpdatePopularDataSetsCache {
    ...
    void updatePopularDataSetsCacheSuccess() {
      ...
      List<DataWithProjectCountDto> savedDataSets =
          List.of(new DataWithProjectCountDto(data1, 5L), new DataWithProjectCountDto(data2, 3L));
      ...
      then(dataReadDtoMapper)
          .should(times(2))
          .toResponseDto(any(), any(), any(), any(), any(), any(), any());
    ...
    void updatePopularDataSetsCacheFailure() {
      ...
      willThrow(new RuntimeException("Database error"))
          .given(getPopularDataSetsPort)
          .getPopularDataSets(20);
  ...
  class ManualUpdatePopularDataSetsCache {
    ...
    void manualUpdatePopularDataSetsCacheSuccess() {
      ...
      List<DataWithProjectCountDto> savedDataSets = List.of(new DataWithProjectCountDto(data, 5L));
