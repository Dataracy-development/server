...
class DataReadDtoMapperTest {
  ...
  void toConnectedDataResponseShouldMapCorrectly() {
    ...
    ConnectedDataResponse res =
        mapper.toResponseDto(data, "userA", "profile.png", "topic", "type", 5L);
  ...
  void toPopularDataResponseShouldMapCorrectly() {
    ...
    // when
    PopularDataResponse res =
        mapper.toResponseDto(data, "user", "profile.png", "topic", "src", "type", 7L);
  ...
  void toDataDetailResponseShouldMapCorrectly() {
    ...
    // when
    DataDetailResponse res =
        mapper.toResponseDto(
            data, "nick", "profile", "intro", "author", "occ", "topic", "src", "type");
  ...
  void toUserDataResponseResponseShouldMapCorrectly() {
    ...
    // when
    UserDataResponse res = mapper.toResponseDto(data, "topic", "type", 3L);
