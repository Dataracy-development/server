...
class CommentReadControllerTest {
  ...
  @MockBean private ReadCommentWebMapper readCommentWebMapper;

  @MockBean private FindCommentListUseCase findCommentListUseCase;

  @MockBean private FindReplyCommentListUseCase findReplyCommentListUseCase;
  ...
  void findCommentsSuccess() throws Exception {
    // given
    FindCommentResponse appRes =
        new FindCommentResponse(
            1L, 1L, "닉네임", "등급", "url", "내용", 0L, 0L, LocalDateTime.now(), false);
    FindCommentWebResponse webRes =
        new FindCommentWebResponse(
            1L, 1L, "닉네임", "등급", "url", "내용", 0L, 0L, LocalDateTime.now(), false);
    ...
    given(findCommentListUseCase.findComments(any(), any(), any(Pageable.class)))
        .willReturn(new PageImpl<>(List.of(appRes), PageRequest.of(0, 10), 1));
    given(readCommentWebMapper.toWebDto(appRes)).willReturn(webRes);

    // when & then
    mockMvc
        .perform(
            get("/api/v1/projects/{projectId}/comments", 1L)
                .param("page", "0")
                .param("size", "10")
  ...
  void findReplyCommentsSuccess() throws Exception {
    // given
    FindReplyCommentResponse appRes =
        new FindReplyCommentResponse(
            2L, 1L, "닉네임", "등급", "url", "내용", 0L, LocalDateTime.now(), false);
    FindReplyCommentWebResponse webRes =
        new FindReplyCommentWebResponse(
            2L, 1L, "닉네임", "등급", "url", "내용", 0L, LocalDateTime.now(), false);
    ...
    mockMvc
        .perform(
            get("/api/v1/projects/{projectId}/comments/{commentId}", 1L, 2L)
                .param("page", "0")
                .param("size", "10")
                .contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
