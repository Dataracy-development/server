...
class CommentKafkaProducerAdapterTest {
    ...
    void sendCommentUploadedEventSuccess() {
        ...
        try (MockedStatic<LoggerFactory> loggerFactoryMock = mockStatic(LoggerFactory.class)) {
            ...
            then(kafkaLogger).should().logProduce("comment-uploaded-topic", "댓글 작성 이벤트 발송됨: projectId=123");
        }
    }
    ...
    void sendCommentUploadedEventFailure() {
        ...
        try (MockedStatic<LoggerFactory> loggerFactoryMock = mockStatic(LoggerFactory.class)) {
            ...
            // then
            then(kafkaTemplate).should().send("comment-uploaded-topic", "123", 123L);
            then(kafkaLogger).should().logError(eq("comment-uploaded-topic"), eq("댓글 작성 이벤트 발송 처리 실패: projectId=123"), any(RuntimeException.class));
        }
    }
    ...
    void sendCommentDeletedEventSuccess() {
        ...
        try (MockedStatic<LoggerFactory> loggerFactoryMock = mockStatic(LoggerFactory.class)) {
            ...
            then(kafkaLogger).should().logProduce("comment-deleted-topic", "댓글 삭제 이벤트 발송됨: projectId=456");
        }
    }
    ...
    void sendCommentDeletedEventFailure() {
        ...
        try (MockedStatic<LoggerFactory> loggerFactoryMock = mockStatic(LoggerFactory.class)) {
            ...
            // then
            then(kafkaTemplate).should().send("comment-deleted-topic", "456", 456L);
            then(kafkaLogger).should().logError(eq("comment-deleted-topic"), eq("댓글 삭제 이벤트 발송 처리 실패: projectId=456"), any(RuntimeException.class));
        }
    }
}
