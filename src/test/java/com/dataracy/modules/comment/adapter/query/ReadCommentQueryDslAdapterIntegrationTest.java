...
class ReadCommentQueryDslAdapterIntegrationTest {
  ...
  @Autowired private ReadCommentPortAdapter readCommentAdapter;
  ...
  void setUp() {
    ...
    replyComment =
        CommentEntity.of(savedProject.getId(), savedUser.getId(), "답글 내용", rootComment.getId());
    entityManager.persist(replyComment);

    anotherRootComment =
        CommentEntity.of(savedProject.getId(), savedUser.getId(), "다른 루트 댓글", null);
  ...
  class ReadCommentPortTest {
    ...
    void findCommentsByProjectIdReturnsRootComments() {
      ...
      // when
      Page<FindCommentWithReplyCountResponse> result =
          readCommentAdapter.findComments(savedProject.getId(), pageable);
      ...
      assertAll(
          ...
          () ->
              assertThat(result.getContent())
                  .hasSize(2) // rootComment, anotherRootComment (replyComment 제외)
          );

      // 답글 수 확인
      FindCommentWithReplyCountResponse rootCommentResponse =
    ...
    void findCommentsWithNonExistentProjectIdReturnsEmpty() {
      ...
      // when
      Page<FindCommentWithReplyCountResponse> result =
          readCommentAdapter.findComments(999L, pageable);

      // then
      assertAll(
          () -> assertThat(result).isEmpty(), () -> assertThat(result.getTotalElements()).isZero());
    }
    ...
    void findReplyCommentsByParentCommentIdReturnsReplies() {
      ...
      // when
      Page<Comment> result =
          readCommentAdapter.findReplyComments(savedProject.getId(), rootComment.getId(), pageable);
    ...
    void findReplyCommentsWithNoRepliesReturnsEmpty() {
      ...
      // when
      Page<Comment> result =
          readCommentAdapter.findReplyComments(
              savedProject.getId(), anotherRootComment.getId(), pageable);
    ...
    void findReplyCommentsWithNonExistentProjectIdReturnsEmpty() {
      ...
      // when
      Page<Comment> result =
          readCommentAdapter.findReplyComments(999L, rootComment.getId(), pageable);
  ...
  class ComplexScenarioTest {
    ...
    void multiplerepliescountcalculation() {
      ...
      CommentEntity reply3 =
          CommentEntity.of(savedProject.getId(), savedUser.getId(), "답글 3", rootComment.getId());
      ...
      assertAll(
          () -> assertThat(rootCommentResponse).isNotNull(),
          () ->
              assertThat(rootCommentResponse.replyCount())
                  .isEqualTo(3) // replyComment, reply2, reply3
          );
    }
    ...
    void commentsFromDifferentProjectsSeparation() {
      ...
      CommentEntity anotherProjectComment =
          CommentEntity.of(anotherProject.getId(), savedUser.getId(), "다른 프로젝트 댓글", null);
      ...
      assertAll(
          ...
          () ->
              assertThat(
                      result.getContent().stream()
                          .anyMatch(
                              response -> response.comment().getContent().equals("다른 프로젝트 댓글")))
                  .isFalse());
    }
    ...
    void commentsfromdifferentusers() {
      ...
      CommentEntity anotherUserComment =
          CommentEntity.of(savedProject.getId(), anotherUser.getId(), "다른 사용자 댓글", null);
      ...
      assertAll(
          ...
          () ->
              assertThat(result.getContent())
                  .hasSize(3) // rootComment, anotherRootComment, anotherUserComment
          );

      // 다른 사용자의 댓글 확인
      boolean foundAnotherUserComment =
          result.getContent().stream()
    ...
    void emptycommentlisthandling() {
      ...
      // when
      Page<FindCommentWithReplyCountResponse> result =
          readCommentAdapter.findComments(emptyProject.getId(), pageable);
