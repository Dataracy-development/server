...
class FileDeleteProducerTest {
    ...
    void sendDeleteEventFailure() {
        ...
        try (MockedStatic<LoggerFactory> loggerFactoryMock = mockStatic(LoggerFactory.class)) {
            ...
            // then
            then(kafkaTemplate).should().send("file-delete-topic", fileUrl);
            then(kafkaLogger).should().logError(eq("file-delete-topic"), eq("파일 삭제 이벤트 발송 처리 실패"), any(RuntimeException.class));
        }
    }
    ...
    void sendDeleteEventWithNullFileUrl() {
        ...
        try (MockedStatic<LoggerFactory> loggerFactoryMock = mockStatic(LoggerFactory.class)) {
            ...
            then(kafkaLogger).should().logWarning("fileDeleteTopic", "파일 삭제 이벤트 발송을 위한 파일 url이 존재하지 않습니다.");
        }
    }
