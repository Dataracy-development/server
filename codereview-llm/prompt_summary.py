...
🧠 아래 PR diff를 위 정책에 따라 리뷰하라. 모든 항목은 **한국어로 실무적으로, 매우 구체적**으로 작성할 것:
아래 1번과 2번, 7번은 필수항목이고 나머지는 있다면 작성해주고 아예 없다면 생략한다.

1️⃣ 변경 파일 요약 (표 형태)  
2️⃣ 주요 변경 목적  
3️⃣ 잠재적 위험 요소 (동시성, 트랜잭션 누락, 락 부재 등)  
4️⃣ 리팩토링 제안 (책임 분리, 메서드 축소 등)  
5️⃣ 누락 가능 항목 (모니터링, fallback, 재시도 정책 등)  
6️⃣ Docstring 추천 (JavaDoc 스타일)  
7️⃣ 시퀀스 다이어그램 예시  
