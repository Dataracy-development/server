...
✅ 분석 기준 (해당하는 항목만 작성):
- 비즈니스 로직이 너무 많은 Service일 경우에 출력 → 메서드 분리, 객체 분리
- 도메인 객체가 무의미하게 컨트롤러에 노출되는 구조일 경우에 출력 → 캡슐화 유도
- DDD 규칙 위반 (도메인 → 인프라 의존 등)한 경우에 출력
- 트랜잭션 범위가 너무 넓거나 여러 책임을 포함한 경우일 경우에 출력
- AOP로 추출 가능한 관심사 분리가 필요할 경우에 출력
- 유틸/헬퍼 클래스가 너무 비대해졌을 경우에 출력
